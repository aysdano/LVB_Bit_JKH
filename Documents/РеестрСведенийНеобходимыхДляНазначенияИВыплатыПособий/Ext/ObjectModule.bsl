...
Процедура ПроверитьТребованияФормата(Отказ)
	...
	СообщенияПользователюБЗК.ПроверитьЗаполнениеРеквизитаОбъекта(Отказ, ЭтотОбъект, "РеестрСоставил");
	СообщенияПользователюБЗК.ПроверитьЗаполнениеРеквизитаОбъекта(Отказ, ЭтотОбъект, "ТелефонСоставителя");
	СообщенияПользователюБЗК.ПроверитьЗаполнениеЭлектроннойПочтыОбъекта(Отказ, ЭтотОбъект, "АдресЭлектроннойПочтыСоставителя");
	...
	Для Каждого Строка Из СведенияНеобходимыеДляНазначенияПособий Цикл
		...
		Если Не СтрНачинаетсяС(Строка.НомерЛисткаНетрудоспособности, "9990")
			И ЗначениеЗаполнено(Строка.Заявление) Тогда
...
Процедура ПроверитьКППВШапке(Отказ, ИмяРеквизита)
	...
	Если Не Успех Тогда
		СообщенияПользователюБЗК.СообщитьОбОшибкеВОбъекте(Отказ, ЭтотОбъект, ИмяРеквизита, Текст);
	КонецЕсли;
КонецПроцедуры
...
Процедура ПроверитьОГРНВШапке(Отказ, ИмяРеквизита, ЭтоЮридическоеЛицо)
	Текст = "";
	ОГРН = ЭтотОбъект[ИмяРеквизита];
	Успех = РегламентированныеДанныеКлиентСервер.ОГРНСоответствуетТребованиям(ОГРН, ЭтоЮридическоеЛицо, Текст);
	Если Не Успех Тогда
		СообщенияПользователюБЗК.СообщитьОбОшибкеВОбъекте(Отказ, ЭтотОбъект, ИмяРеквизита, Текст);
	КонецЕсли;
КонецПроцедуры
...
Процедура ПроверитьРегистрационныйНомерФСС(Отказ, ИмяРеквизита, ТребоватьЗаполнить)
	...
	Если Не ЗначениеЗаполнено(РегистрационныйНомер) Тогда
		Если ТребоватьЗаполнить Тогда
			СообщенияПользователюБЗК.ПроверитьЗаполнениеРеквизитаОбъекта(Отказ, ЭтотОбъект, ИмяРеквизита);
		КонецЕсли;
...
Процедура ПроверитьЗаполнениеСведений(Отказ, СведенияОбъект)
	...
	Если Не ПроверкаЗаполненияПройдена Тогда
		...
		Если СообщенияЗаявления.Количество() = 0 Тогда
			...
			СообщенияПользователюБЗК.СообщитьОбОшибкеВОбъекте(Неопределено, СведенияОбъект.Ссылка, "", Текст);
		КонецЕсли;
	КонецЕсли;
	Если СообщенияЗаявления.Количество() > 0 Тогда
...
Функция ПроверитьЗаполнениеПоРегламентуФСС() Экспорт
	...
	ПроверитьРеквизитыФизическихЛиц(Отказ);
...
Процедура ПроверитьЗаполнениеСтрокСведенийПоВременнойНетрудоспособности(Отказ)
	...
	Менеджер = Документы.РеестрСведенийНеобходимыхДляНазначенияИВыплатыПособий;
	НомераТребующиеРеестровЭЛН = НомераТребующиеРеестровЭЛН();
	...
	Для Каждого Строка Из СведенияНеобходимыеДляНазначенияПособий Цикл
		...
		Если ЭтоБольничный И НомераТребующиеРеестровЭЛН.Найти(Строка.НомерЛисткаНетрудоспособности) <> Неопределено Тогда
			Текст = НСтр("ru = 'Перед отправкой реестра прямых выплат для ЭЛН требуется отправить реестр ЭЛН.'");
			СообщитьОбОшибкеВСтрокеТаблицы(Отказ, ИмяТаблицы, Строка, "ПервичныйДокумент", Текст);
		КонецЕсли;
...
Функция НомераТребующиеРеестровЭЛН()
	НомераЭЛН = Новый Массив;
	Менеджер = Документы.РеестрСведенийНеобходимыхДляНазначенияИВыплатыПособий;
	Для Каждого Строка Из СведенияНеобходимыеДляНазначенияПособий Цикл
		Если ЗначениеЗаполнено(Строка.Заявление)
			И ЗначениеЗаполнено(Строка.ПервичныйДокумент)
			И Не Менеджер.ТребуетсяЗаполнитьСведенияОЛисткеНетрудоспособности(Строка) Тогда
			НомераЭЛН.Добавить(Строка.НомерЛисткаНетрудоспособности);
		КонецЕсли;
	КонецЦикла;
	Если НомераЭЛН.Количество() = 0 Тогда
		Возврат НомераЭЛН;
	КонецЕсли;
	
	ГоловнаяОрганизация = ЗарплатаКадры.ГоловнаяОрганизация(Организация);
	
	ОрганизацииИспользующиеОбменФСС = ЭлектронныйДокументооборотСФСС.ОрганизацииИспользующиеОбменФСС();
	Если ОрганизацииИспользующиеОбменФСС.Найти(Организация) = Неопределено
		И ОрганизацииИспользующиеОбменФСС.Найти(ГоловнаяОрганизация) = Неопределено Тогда
		Возврат Новый Массив;
	КонецЕсли;
	
	Возврат РегистрыСведений.СведенияОбЭЛН.НомераЭЛНТребующихПервичнуюОтправку(ГоловнаяОрганизация, НомераЭЛН);
КонецФункции
...
Процедура ПроверитьОГРНВСтрокеТаблицы(Отказ, ИмяТаблицы, Строка, ИмяРеквизита)
	...
	Если ЗначениеЗаполнено(ОГРНСтроки) Тогда
		Текст = "";
		Успех = РегламентированныеДанныеКлиентСервер.ОГРНСоответствуетТребованиям(ОГРНСтроки, СтрДлина(ОГРНСтроки) <> 15, Текст);
		Если Не Успех Тогда
			СообщитьОбОшибкеВСтрокеТаблицы(Отказ, ИмяТаблицы, Строка, ИмяРеквизита, Текст);
		КонецЕсли;
	Иначе
		СообщенияПользователюБЗК.ПроверитьЗаполнениеРеквизитаСтрокиТаблицы(
			Отказ,
			ЭтотОбъект,
			ИмяТаблицы,
			Строка,
		...
		Если РеквизитыЗаявления <> Неопределено Тогда
			Если Строка.ОчередностьРожденияРебенка > 1
				...
				СообщенияПользователюБЗК.СообщитьОбОшибкеВОбъекте(
					...
					"Объект.НаличиеОдногоИзДокументовНаПредыдущихДетей",
					НСтр("ru = 'Для второго и последующих детей должен быть включен один из флажков: ""Наличие документов на предыдущих детей"", ""Наличие свидетельства о его (их) смерти"".'"));
			КонецЕсли;
		КонецЕсли;
...
Процедура ПроверитьРеквизитыФизическихЛиц(Отказ)
	ИмяТаблицы = "СведенияНеобходимыеДляНазначенияПособий";
	
	МинимальнаяДатаПолучения = НачалоДня(Мин(Дата, ТекущаяДатаСеанса()) - 5*86400);
	
	РеквизитыФизическихЛиц = РеквизитыФизическихЛиц();
	
	Для Каждого Строка Из СведенияНеобходимыеДляНазначенияПособий Цикл
		
		РеквизитыФизическогоЛица = РеквизитыФизическихЛиц.Получить(Строка.ФизическоеЛицо);
		Если Не ЗначениеЗаполнено(РеквизитыФизическогоЛица.ИНН) Тогда
			Текст = НСтр("ru = 'Не заполнен ИНН.'");
			СообщитьОбОшибкеВСтрокеТаблицы(Отказ, ИмяТаблицы, Строка, "ФизическоеЛицо", Текст);
		КонецЕсли;
...
Функция РеквизитыФизическихЛиц()
	МассивСсылок = СведенияНеобходимыеДляНазначенияПособий.ВыгрузитьКолонку("ФизическоеЛицо");
	ИменаРеквизитов = Новый Массив;
	ИменаРеквизитов.Добавить("ИНН");
	УстановитьПривилегированныйРежим(Истина);
	РеквизитыФизическихЛиц = ОбщегоНазначения.ЗначенияРеквизитовОбъектов(МассивСсылок, ИменаРеквизитов);
	УстановитьПривилегированныйРежим(Ложь);
	Возврат РеквизитыФизическихЛиц;
КонецФункции
...
Процедура СообщитьОбОшибкеВСтрокеТаблицы(Отказ, ИмяТаблицы, Строка, ИмяРеквизита, Текст)
	СообщенияПользователюБЗК.СообщитьОбОшибкеВСтрокеТаблицы(
		Отказ,
		ЭтотОбъект,
		ИмяТаблицы,
		Строка,
...
Функция ОбновитьВторичныеДанныеДокумента() Экспорт
	...
	Если ОбновитьРегламентированныеСведенияОбОрганизации(ПараметрыФиксации) Тогда
		Модифицирован = Истина;
	КонецЕсли;
...
Функция ОбновитьДанныеОрганизации(ПараметрыФиксации)
	...
	Реквизиты = Новый Структура(ИменаРеквизитовОрганизации);
	...
	ЗаполнитьЗначенияСвойств(Реквизиты, ЗначенияРеквизитовОрганизации, ИменаРеквизитовОрганизации);
...
Функция ОбновитьРегламентированныеСведенияОбОрганизации(ПараметрыФиксации)
	Если Не ЗначениеЗаполнено(Организация) Тогда
		Возврат Ложь;
	КонецЕсли;
	
	Реквизиты = Новый Структура("ТелефонСоставителя, АдресЭлектроннойПочтыСоставителя, ИНН, КПП");
	Если ФиксацияВторичныхДанныхВДокументах.РеквизитыШапкиЗафиксированы(ЭтотОбъект, Реквизиты) Тогда
		Возврат Ложь;
	КонецЕсли;
	...
	ОргСведения.Свойство("ТелОрганизации",                   Реквизиты.ТелефонСоставителя);
	ОргСведения.Свойство("АдресЭлектроннойПочтыОрганизации", Реквизиты.АдресЭлектроннойПочтыСоставителя);
