...
Функция СформироватьСтруктуруПараметров() Экспорт
	...
	Для НомСтр = 201 По 210 Цикл
		КодПоказателя = "П0000201" + НомСтр + "01";
		ПараметрыВыгрузки.Вставить("Зн_" + КодПоказателя, ТабДокументРаздел1_2[КодПоказателя]);
	КонецЦикла;
	
	Для НомСтр = 401 По 402 Цикл
		Для Гр = 1 По 2 Цикл
			КодПоказателя = "П0000401" + НомСтр + "0" + Гр;
			ПараметрыВыгрузки.Вставить("Зн_" + КодПоказателя, ТабДокументРаздел3_4[КодПоказателя]);
		КонецЦикла;
	КонецЦикла;
	ПараметрыВыгрузки.Вставить("Зн_П000040140301", ТабДокументРаздел3_4["П000040140301"]);
	
	Для НомСтр = 501 По 526 Цикл
		КодПоказателя = "П0000501" + НомСтр + "01";
		ПараметрыВыгрузки.Вставить("Зн_" + КодПоказателя, ТабДокументРаздел5[КодПоказателя]);
	КонецЦикла;
	
	Для НомСтр = 601 По 671 Цикл
		КодПоказателя = "П0000601" + НомСтр + "01";
		ПараметрыВыгрузки.Вставить("Зн_" + КодПоказателя, ТабДокументРаздел6[КодПоказателя]);
	КонецЦикла;
	
	Для НомСтр = 701 По 737 Цикл
		КодПоказателя = "П0000701" + НомСтр + "01";
		ПараметрыВыгрузки.Вставить("Зн_" + КодПоказателя, ТабДокументРаздел7_8[КодПоказателя]);
	КонецЦикла;
