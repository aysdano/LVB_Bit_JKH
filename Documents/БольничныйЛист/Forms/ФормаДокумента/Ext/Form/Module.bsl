...
&НаКлиенте
Процедура ЗаголовокСсылкиСведенийДляПрямыхВыплатНажатие(Элемент, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
...
Процедура ВыгрузитьФайл(Команда)
	ОписаниеФайла = ОписаниеФайлаЭЛН(Объект.НомерЛисткаНетрудоспособности, Объект.Организация, УникальныйИдентификатор);
...
Процедура ПриПолученииДанныхНаСервере()
	...
	ВидОплатыЗаСчетРаботодателя = Документы.БольничныйЛист.ВидОплатыЗаСчетРаботодателя(Кэш);
	ВидОплатыПособия = Документы.БольничныйЛист.ВидОплатыПособия(Объект, Кэш);
	ВидНеоплачиваемогоВремени = Документы.БольничныйЛист.ВидНеоплачиваемогоВремени(Кэш);
...
Процедура ЗаполнитьТекущиеДанныеСотрудника(ЗаполнитьОбъект = Ложь)
	...
	Если ЗначениеЗаполнено(Объект.Сотрудник) Тогда
		...
		КадровыеДанные = КадровыйУчет.КадровыеДанныеСотрудников(Истина, Объект.Сотрудник, "ФизическоеЛицо,Подразделение,ДатаУвольнения,ВидЗанятости,ДоляНеполногоРабочегоВремени", Объект.ДатаНачалаСобытия);
		Если КадровыеДанные.Количество() > 0 Тогда 
			...
			Объект.ДоляНеполногоВремени = УчетПособийСоциальногоСтрахования.ДоляНеполногоРабочегоВремени(
				КадровыеДанные[0].ДоляНеполногоРабочегоВремени);
		КонецЕсли;
...
&НаСервере
Процедура ПричинаНетрудоспособностиПриИзмененииНаСервере(КорректироватьДниОплаты = Истина)
...
Процедура ОбновитьГиперссылкиПрямыхВыплатИЭЛН()
	...
	Если ЗначениеЗаполнено(Объект.НомерЛисткаНетрудоспособности)
		...
		ЗаголовокСсылкиЛН = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Заполнены данные ЛН %1 от %2'"),
			Объект.НомерЛисткаНетрудоспособности,
			Формат(Объект.ДатаВыдачиЛисткаНетрудоспособности, "ДЛФ=D"));
	Иначе
		ЗаголовокСсылкиЛН = НСтр("ru = 'Заполнить данные ЛН'");
	КонецЕсли;
	
	Если Элементы.ГруппаДанныеДляФСС.Видимость Тогда
		ЗаголовокСсылкиСведенийДляПрямыхВыплат = ПрямыеВыплатыПособийСоциальногоСтрахования.ЗаголовокГиперссылкиЗаявления(
			Объект.Ссылка);
	КонецЕсли;
	
	Элементы.ПодменюЭЛНВыгрузитьФайл.Видимость = ОбменЛисткамиНетрудоспособностиФСС.ДоступенИсходныйXML(
		Объект.НомерЛисткаНетрудоспособности,
		ЗарплатаКадры.ГоловнаяОрганизация(Объект.Организация));
...
Функция ЗагрузитьОтветСервисаФСС(Знач АдресРасшифрованногоОтветаSOAP, Знач ЗагрузкаИзФайла)
	...
	Если ОбменВыполненУспешно Тогда
		ЗначениеВРеквизитФормы(ДокументОбъект, "Объект");
		ЗаполнитьПоПервичномуБольничномуЛисту();
		ПричинаНетрудоспособностиПриИзмененииНаСервере(Ложь);
		ЯвляетсяПродолжениемБолезниПриИзмененииНаСервере();
		ДатаНачалаПриИзмененииНаСервере();
		ДатаОкончанияПриИзмененииНаСервере();
		ОбновитьГиперссылкиПрямыхВыплатИЭЛН();
...
Функция ОписаниеФайлаЭЛН(Знач НомерЛН, Знач Организация, Знач ИдентификаторФормы)
...
&НаСервере
Процедура ЗаполнитьПараметрыОплаты(Знач ФлажокНазначитьПособие = Неопределено)
	Кэш = Новый Соответствие;
	Документы.БольничныйЛист.ЗаполнитьПараметрыОплаты(Объект, Кэш);
...
Процедура ЗаполнитьДнейОплатыНаОснованииПериодаОплаты()
	Объект.ДнейОплаты = Документы.БольничныйЛист.МаксимальноеЧислоДнейОплаты(Объект);
КонецПроцедуры
