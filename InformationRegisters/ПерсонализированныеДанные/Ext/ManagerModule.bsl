...
Процедура ОбновитьДанные(Организация, Дата = Неопределено, ИнтервалОбновления = 0, РазделыПерсонализированныхДанных = Неопределено) Экспорт
	...
	Для каждого Раздел из РазделыПерсонализированныхДанных Цикл
		...
		ИначеЕсли Раздел = Перечисления.РазделыПерсонализированныхДанных.ИспользуетеКредиты Тогда
			ПерсонализированныеПредложенияСервисов.ОбновитьДанныеИспользуетеКредиты(ДатаПолученияДанных);
			Продолжить;

		ИначеЕсли Раздел = Перечисления.РазделыПерсонализированныхДанных.ФинОтчетность Тогда
			ПерсонализированныеПредложенияСервисов.ОбновитьДанныеФинОтчетность(ДатаПолученияДанных);
			Продолжить;
