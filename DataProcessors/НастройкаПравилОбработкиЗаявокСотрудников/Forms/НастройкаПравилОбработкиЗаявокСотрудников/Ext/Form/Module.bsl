...
Процедура ЗаполнитьНастройкиМаршрутов()
	...
	ТаблицаПравилНастроек = ТаблицаПравилНастроек();
	ТаблицаПользователей = ТаблицаПользователей();
...
Функция ТаблицаПравилНастроек()
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ВременнаяТаблица.РолиИсполнителей.Ссылка КАК РольИсполнителя,
	|	ВременнаяТаблица.СтруктураПредприятия.Ссылка КАК Подразделение,
	...
	|			И (ПравилаОбработкиЗаявокСотрудников.Подразделение = ЗНАЧЕНИЕ(#Таблица.ПустаяСсылка)
	...
	|			И (ПравилаОбработкиЗаявокСотрудников2.Подразделение = ЗНАЧЕНИЕ(#Таблица.ПустаяСсылка)
	...
	|		ПО ИсполнителиЗадач.ОсновнойОбъектАдресации = ВременнаяТаблица.СтруктураПредприятия
	|			И ИсполнителиЗадач.РольИсполнителя = ВременнаяТаблица.РолиИсполнителей
	|
	|УПОРЯДОЧИТЬ ПО
	|	РольИсполнителя,
	|	ВременнаяТаблица.СтруктураПредприятия.Ссылка ИЕРАРХИЯ";
...
Процедура ЗаписатьНаСервере(Отказ = Ложь)
	...
	Для каждого Настройка Из НастройкиПравил Цикл
		...
		ПравилаОбработкиЗаявок = ПолучитьЭлементы(Настройка.СтруктураПредприятия, Истина);
		
		Для каждого ПравилоОбработкиЗаявки Из ПравилаОбработкиЗаявок Цикл
			Подразделение = ?(ВидНастройки = 1, ПравилоОбработкиЗаявки.Подразделение,
...
Функция ВидНастройки()
	...
	|ГДЕ
	|	ПравилаОбработкиЗаявокСотрудников.Подразделение <> ЗНАЧЕНИЕ(#Таблица.ПустаяСсылка)";
	СтруктураПредприятия = Новый(БизнесПроцессыЗаявокСотрудниковВнутренний.ТипСтруктураПредприятия());
	ИмяТаблицы = СтруктураПредприятия.Метаданные().ПолноеИмя();
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "#Таблица", ИмяТаблицы);
