...
Процедура ЗаполнитьОписаниеРазделаПроизводство(ОписаниеФункциональности)
	...
	Функциональность = ДобавитьОписаниеФункциональности(ОписаниеФункциональности, Раздел, "ИспользоватьВыпускПродукции");
	Функциональность.ДоступнаВПростомИнтерфейсе = Истина;
	Функциональность.ТребуетсяДляОбменов        = Истина;
...
Процедура ЗаполнитьОписаниеРазделаЗарплатаКадры(ОписаниеФункциональности, ИмяРаздела = Неопределено)
	...
	Если РезультатЗапросаЮридическиеЛица().Пустой() Тогда
		...
		Функциональность = ДобавитьОписаниеФункциональности(ОписаниеФункциональности, Раздел, "ИспользоватьРаботуНаНеполнуюСтавку");
		Функциональность.Доступна = НЕ ПолучитьФункциональнуюОпцию("УчетЗарплатыИКадровВоВнешнейПрограмме");
		Функциональность.ДоступнаВПростомИнтерфейсе  = Истина;
		ДобавитьРодителя(Функциональность, "ВедетсяУчетЗарплатыКадров");
...
Функция ОтказСнятьВедетсяПроизводственнаяДеятельность()
	...
	НоваяСтрока = ТаблицаЗапросов.Добавить();
	НоваяСтрока.ТекстЗапроса = ТекстЗапросаИспользуетсяДокумент("ОтчетПроизводстваЗаСмену", Истина);
	НоваяСтрока.ВидыОпераций = Новый Массив;
	НоваяСтрока.ВидыОпераций.Добавить(Перечисления.ВидыОперацийОтчетПроизводстваЗаСмену.ОтчетПроизводстваЗаСмену);
...
Функция ОтказСнятьИспользоватьВыпускПродукции()
	...
	Запрос = Новый Запрос;
	Запрос.Текст = ТекстЗапросаИспользуетсяДокумент("ОтчетПроизводстваЗаСмену");
...
Функция ТаблицаЗапросовВедетсяПроизводственнаяДеятельность()
	...
	НоваяСтрока.ТекстЗапроса = ТекстЗапросаИспользуетсяДокумент("ОтчетПроизводстваЗаСмену", Истина, Ложь);
	НоваяСтрока.ВидыОпераций = Новый Массив;
	НоваяСтрока.ВидыОпераций.Добавить(Перечисления.ВидыОперацийОтчетПроизводстваЗаСмену.ОтчетПроизводстваЗаСмену);
...
Функция ТаблицаЗапросовИспользуетсяВыпускПродукции()
...
Функция ДокументыПрепятствующиеОтключениюИспользуетсяВыпускПродукции()
	...
	ТаблицаЗапросов = ТаблицаЗапросовИспользуетсяВыпускПродукции();
...
Процедура СформироватьСписокДокументовПрепятствующихОтключениюОпции(Параметры, АдресХранилища) Экспорт
	...
	ИначеЕсли Параметры.Свойство("ВедетсяПроизводственнаяДеятельность") Тогда
		...
		ОбластьШапка.Параметры.Заголовок = НСтр("ru = 'Комплексный учет производства'");
...
Процедура СформироватьСписокОбъектовПрепятствующихОтключениюРасширенногоФункционала(Параметры, АдресХранилища) Экспорт
	...
	Пояснение = НСтр("ru = 'Для отключения расширенного функционала в учетной политике организации (раздел ""Себестоимость выпуска"")
                      |измените значение переключателя ""Отклонения от плановой стоимости""'");
	ЗаполнитьПечатнуюФормуПоУчетнойПолитике(Выборка, Заголовок, Пояснение, СтруктураРезультата);
...
Функция РезультатЗапросаРасширенныйФункционал(ТолькоПроверка = Истина)
	...
	|	РегистрСведений.УчетнаяПолитика КАК УчетнаяПолитика
	|ГДЕ
	|	УчетнаяПолитика.СпособУчетаВыпускаГотовойПродукции = ЗНАЧЕНИЕ(Перечисление.ВариантыУчетаСтоимостиВыпуска.ПлановаяСтоимость)
	|
	|УПОРЯДОЧИТЬ ПО
	|	УчетнаяПолитика.Период
	|;
