...
Процедура КонтактнаяИнформацияПолеEmailОрганизацииРасширеннаяПодсказкаОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	ЭлектроннаяПочтаОбработкаНавигационнойСсылки(НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка);
...
&НаКлиенте
Процедура КонтактнаяИнформацияПолеEMailФизическиеЛицаРасширеннаяПодсказкаОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
...
Процедура СформироватьДокументы(Команда)
	...
	Если Организация.Ссылка.Пустая() Или Модифицированность
		Или ВыбраннаяФормаИзменена(Организация.Ссылка, ВыбраннаяФорма) Тогда
...
Процедура ПодготовитьФормуНаСервере()
	...
	ДанныеПомощникаРегистрации.Заявление.Свойство("ВыдатьНаБумажномНосителе", ВыдатьНаБумажномНосителе);
...
Процедура УправлениеЗакладкойВводДанных(Форма)
	...
	Если ЭтоФорма2014(Форма.ВыбраннаяФорма) Тогда
		Элементы.КонтактнаяИнформацияПолеEMailФизическиеЛица.Видимость = Истина;
		Элементы.ГруппаСпособПолученияДокументов.Видимость = Ложь;
	Иначе
		Элементы.КонтактнаяИнформацияПолеEMailФизическиеЛица.Видимость = Ложь;
...
Процедура ЗаявлениеОРегистрацииООО(ОрганизацияСсылка)
	...
	Если ЗначениеЗаполнено(ЗаявлениеОРегистрации.Ссылка) Тогда
		...
		РазблокироватьДанные(ЗаявлениеОРегистрации.Ссылка, ФормаОтчета.УникальныйИдентификатор);
...
Процедура РазместитьНавигациюНаСервере()
	...
	ИначеЕсли ТекущийЭтап = НомерЭтапаУстав() И ИспользоватьТиповыеУставы Тогда
		СтруктураПараметров = Новый Структура("НавигацияПараметрФормы", "Устав");
		Обработки.РегистрацияОрганизации.РазместитьНавигацию(ЭтотОбъект, СтруктураПараметров);
		Элементы.ГруппаНавигация.Доступность = Истина;
	Иначе
...
&НаКлиенте
Процедура ЭлектроннаяПочтаОбработкаНавигационнойСсылки(ТекстНавигационнойСсылки, СтандартнаяОбработка)
	
	Если Найти(ТекстНавигационнойСсылки, "ОбразецЗаявленияОРегистрацииОрганизации") > 0 Тогда
...
&НаСервереБезКонтекста
Функция ВыбраннаяФормаИзменена(ОрганизацияСсылка, ВыбраннаяФорма)
	
	ЗаявлениеОРегистрации = НайтиЗаявлениеОРегистрации(ОрганизацияСсылка);
	
	Если ЗначениеЗаполнено(ЗаявлениеОРегистрации.Ссылка)
		И СтрНайти(ЗаявлениеОРегистрации.ИмяФормы, ВыбраннаяФорма) = 0 Тогда
