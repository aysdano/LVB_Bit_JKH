		...
		Если ИмяСправочника = "Контрагенты"
			...
			Если РеквизитСоздаваемогоОбъекта.Имя = "Наименование" Тогда
				...
				Если РеквизитСоздаваемогоОбъекта.ВысотаКартинки > 0
					...
					ДвоичныеДанные = ПолучитьИзВременногоХранилища(РеквизитСоздаваемогоОбъекта.Картинка);
...
Функция ЗаполнитьДанныеКонтрагентаИзБазыЕГРЮЛ()
	...
	Если ЭтоЮрЛицо Тогда
		...
		Если ЗначениеЗаполнено(СведенияОКонтрагенте.ЕГРЮЛ) Тогда
			ДанныеДляЗаполнения = СведенияОКонтрагенте.ЕГРЮЛ;
		Иначе
			ДанныеДляЗаполнения = Новый Структура;
			ШаблонОписанияОшибки = НСтр("ru = 'Не удалось найти данные для заполнения реквизитов по ИНН %1.'");
			ДанныеДляЗаполнения.Вставить("ОписаниеОшибки", СтрШаблон(ШаблонОписанияОшибки, ИНН));
		КонецЕсли;
		...
		Если Не ЗначениеЗаполнено(ДанныеДляЗаполнения.ОписаниеОшибки) Тогда
			Если ЗначениеЗаполнено(ДанныеДляЗаполнения.Руководители) Тогда
				ЗаполнитьЗначенияСвойств(Руководитель, ДанныеДляЗаполнения.Руководители[0]);
			Иначе
				Руководитель = Неопределено;
			КонецЕсли;
			ДанныеДляЗаполнения.Вставить("Руководитель", Руководитель);
			ДанныеДляЗаполнения.Вставить("КодОКВЭД",
				?(ЗначениеЗаполнено(ДанныеДляЗаполнения.ВидДеятельности), ДанныеДляЗаполнения.ВидДеятельности.Код, Неопределено));
			ДанныеДляЗаполнения.Вставить("ЭтоОКВЭД2",  
				?(ЗначениеЗаполнено(ДанныеДляЗаполнения.ВидДеятельности) И ДанныеДляЗаполнения.ВидДеятельности.Классификатор = "ОКВЭД2", Истина, Ложь));
		КонецЕсли;
	Иначе
	...
	Если ДанныеДляЗаполнения.Свойство("ОткрытыеГосударственныеДанныеФНС") Тогда
		ДанныеДляЗаполнения.Удалить("ОткрытыеГосударственныеДанныеФНС");
	КонецЕсли;
	
	// Удаляем свойства типа "ТаблицаЗначений" перед передачей на клиент.
	Если ДанныеДляЗаполнения.Свойство("Руководители") Тогда
		ДанныеДляЗаполнения.Удалить("Руководители");
	КонецЕсли;
...
Функция РеквизитыСоздаваемогоОбъекта(ПараметрыОткрытия)
	...
	Если ИндексЮридическоеФизическоеЛицоВМассивеЗначенияОткрытия <> Неопределено Тогда
		ЮридическоеФизическоеЛицо = ОпределитьЮридическоеФизическоеЛицо(СтрокаИНН);
...
&НаСервереБезКонтекста
Функция ОпределитьЮридическоеФизическоеЛицо(ИНН)
