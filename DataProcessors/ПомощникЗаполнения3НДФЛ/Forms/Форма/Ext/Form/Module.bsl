...
Процедура ЗапроситьСверку(Команда)
	...
	ОткрытьФорму(
		ОписаниеДействияСверка.ИмяФормы, 
		ОписаниеДействияСверка.ПараметрыФормы, 
		ЭтотОбъект,
		,
		,
		,
...
Функция ОплатитьНаСервере(СпособОплаты)
	...
	СтруктураПараметров.Вставить("ПериодПлатежа",   КонецГода(Объект.Период));
...
Процедура ОтрисоватьПоясненияРасчетаНалога(ПоясненияРасчетаНалога)
	...
	Для СчетчикГрупп = 1 По ПоясненияРасчетаНалога.Строки.Количество() Цикл
		...
		ДекорацияОтступ = Элементы.Найти(ИмяДекорацииОтступ);
		Если ДекорацияОтступ = Неопределено Тогда
			НоваяДекорация(
				ИмяДекорацииОтступ, // Имя добавляемого элемента
				Элементы.ГруппаРасчетНалога, // Родитель
				Элементы.ОтступРасчетНалогаПоСтавке1); // Эталон
		КонецЕсли;
...
Функция ЗаполнитьДекларациюВФоне()
	...
	Если ЭтоАдресВременногоХранилища(АдресТаблицаДанныхЗаполнения) Тогда
		СтруктураПараметров.Вставить("ТаблицаДанныхЗаполнения", ПолучитьИзВременногоХранилища(АдресТаблицаДанныхЗаполнения));
	Иначе
		СтруктураПараметров.Вставить("ТаблицаДанныхЗаполнения", Неопределено);
	КонецЕсли;
	...
	СтруктураПараметров.Вставить("ЗаполнятьЗаявлениеНаВозвратНалога", ЗаполнятьЗаявлениеНаВозвратНалога);
	СтруктураПараметров.Вставить("ВариантДействияСНалогомКВозврату", ВариантДействияСНалогомКВозврату);
