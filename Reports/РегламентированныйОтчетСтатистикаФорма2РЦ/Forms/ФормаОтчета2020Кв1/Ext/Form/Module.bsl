...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Если Область.Имя = "ОргАдрес" Тогда
		РегламентированнаяОтчетностьКлиент.ОбработкаАдресаВСтатистическомОтчетеJSON(
			ЭтотОбъект, Область, СтандартнаяОбработка);
	Иначе
		Если Прав(Область.Имя, 2) = "02" или Прав(Область.Имя, 2) = "03" Тогда
		...
		ИначеЕсли Прав(Область.Имя, 2) = "13" Тогда
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(
...
Процедура ВвестиПоказательЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	Если Прав(Область.Имя, 2) = "13" Тогда
		Область.Значение = РезультатКод;
	Иначе
		Область.Значение = ?(Прав(Область.Имя, 2) = "02", РезультатНазвание, РезультатКод);
		КолонкаЗамены = ?(Прав(Область.Имя, 2) = "02", "03", "02");
		ОбластьЗамены = Лев(Область.Имя, 11) + КолонкаЗамены;
		ПолеТабличногоДокументаФормаОтчета.Области[ОбластьЗамены].Значение
			= ?(Прав(Область.Имя, 2) = "02", РезультатКод, РезультатНазвание);
	КонецЕсли;
...
Процедура ПеренумерацияСтрокТаблицы(ЭтотОбъект)
	...
	Пока Истина Цикл
		...
		Обл = Области.Найти("П00001010" + СтрНомер + "01");
		Если Обл = Неопределено Тогда
			Прервать;
		КонецЕсли;
		Если ЗначениеЗаполнено(Области["П00001010" + СтрНомер + "02"].Значение) Тогда
			НПП = НПП + 1;
			Области["П00001010" + СтрНомер + "01"].Значение = НПП;
		Иначе
			Области["П00001010" + СтрНомер + "01"].Значение = "";
		КонецЕсли;
		Стр = Стр + 1;
	КонецЦикла;
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	Для Стр = 1 По 26 Цикл
		СтрНом = Формат(Стр, "ЧЦ=2; ЧВН=");
		Области["П00001010" + СтрНом + "07"].Значение
			= Области["П00001010" + СтрНом + "08"].Значение
			+ Области["П00001010" + СтрНом + "09"].Значение
			+ Области["П00001010" + СтрНом + "10"].Значение;
		КонецЦикла;
...
Процедура УстановитьЗащитуИЦветФонаОбластейАвторассчитываемыхПоказателей()
	...
	Для Стр = 1 По 26 Цикл
		мФормаОтчета.Области["П00001010" + Формат(Стр, "ЧЦ=2; ЧВН=") + "07"].ЦветФона = ЦветФонаЗначение;
		мФормаОтчета.Области["П00001010" + Формат(Стр, "ЧЦ=2; ЧВН=") + "07"].Защита   = Не ОтклАвтоРасчетЗначение;
	КонецЦикла;
