...
//	|	СУММА(ВЫБОР КОГДА СчетНаОплатуПокупателюТовары.Цена = 0 ТОГДА 1 ИНАЧЕ Начисления.Сумма / СчетНаОплатуПокупателюТовары.Цена КОНЕЦ) КАК Количество,
//	//|	ВЫРАЗИТЬ(ВЫБОР
...
//	|	ВЫРАЗИТЬ (СчетНаОплатуПокупателюТовары.Цена КАК ЧИСЛО(15,2)) КАК Цена,
//	//|	СУММА(Начисления.Сумма / ВЫБОР
//	//|			КОГДА СчетНаОплатуПокупателюТовары.Количество = 0
//	//|				ТОГДА 1
//	//|			ИНАЧЕ СчетНаОплатуПокупателюТовары.Количество
//	//|		КОНЕЦ) КАК Цена,
//	|	СУММА(СчетНаОплатуПокупателюТовары.СуммаСкидки) КАК СуммаСкидок,
//	|	СУММА(ВЫБОР КОГДА СчетНаОплатуПокупателюТовары.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.БезНДС) ТОГДА 0 
//	|		КОГДА (СчетНаОплатуПокупателюТовары.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС10) или СчетНаОплатуПокупателюТовары.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС10_110)) ТОГДА Начисления.Сумма * 0.10 
//	|		КОГДА (СчетНаОплатуПокупателюТовары.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС18) или СчетНаОплатуПокупателюТовары.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС18_118)) ТОГДА Начисления.Сумма * 0.18 
//	|		КОГДА (СчетНаОплатуПокупателюТовары.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС20) или СчетНаОплатуПокупателюТовары.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС20_120)) ТОГДА Начисления.Сумма * 0.20 
//	|		ИНАЧЕ СчетНаОплатуПокупателюТовары.СуммаНДС КОНЕЦ) КАК СуммаНДС,
...
//	|		ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ Ссылка, Номенклатура, Содержание, СтавкаНДС, МИНИМУМ(Количество) КАК Количество, МИНИМУМ(Цена) КАК Цена, СУММА(СуммаСкидки) КАК СуммаСкидки, СУММА(СуммаНДС) КАК СуммаНДС, СУММА(Сумма) КАК Сумма ИЗ Документ.СчетНаОплатуПокупателю.Товары СГРУППИРОВАТЬ ПО Ссылка, Номенклатура, Содержание, СтавкаНДС) КАК СчетНаОплатуПокупателюТовары
...
//	|
//	|		ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ Регистратор, ЛицевойСчет, УслугаЖКХ.Номенклатура КАК Номенклатура, ВидНачисления, СУММА(Сумма) КАК Сумма ИЗ РегистрНакопления.бит_НачисленияПоУслугамЖКХПоПериоду СГРУППИРОВАТЬ ПО Регистратор, ЛицевойСчет, УслугаЖКХ.Номенклатура, ВидНачисления) КАК Начисления
//	|		ПО Начисления.Регистратор = СчетНаОплатуПокупателюТовары.Ссылка.бит_ДокументОснование 
//	|		И Начисления.ЛицевойСчет = СчетНаОплатуПокупателюТовары.Ссылка.бит_ЛицевойСчет
//	|		И Начисления.Номенклатура = СчетНаОплатуПокупателюТовары.Номенклатура 
//	|
...
//	|	ВЫРАЗИТЬ (СчетНаОплатуПокупателюТовары.Цена КАК ЧИСЛО(15,2)),
...
// БИТ ЖКХ 20.02.2021
Функция ТекстЗапросаДанныеДляПечатиЧекаСАвансом(ИменаТаблиц) Экспорт
