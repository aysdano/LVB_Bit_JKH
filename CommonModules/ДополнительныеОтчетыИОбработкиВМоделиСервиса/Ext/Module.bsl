...
Процедура ЗаблокироватьДополнительныеОтчетыИОбработкиДляОбновления() Экспорт
	...
	Запрос.УстановитьПараметр("Версия", бит_ОбщегоНазначенияПовтИсп.ПолучитьНомерРелиза_1С());
...
Функция ПроверитьСовместимостьПоставляемойОбработки(Знач ТаблицаСовместимости)
	...
	Для Каждого ДекларацияСовместимости Из ТаблицаСовместимости Цикл
		...
		Иначе
			...
			Если ДекларацияСовместимости.ConfigarationName = Метаданные.Имя И ДекларацияСовместимости.VersionNumber = бит_ОбщегоНазначенияПовтИсп.ПолучитьНомерРелиза_1С() Тогда
				Возврат Истина;
			КонецЕсли;
