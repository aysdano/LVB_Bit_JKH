...
Функция ВыводитьОбластьСведенийОПеревозке(ДатаДокумента)
	
	Возврат ДатаДокумента >= '20210101' И ДатаДокумента < '20270101';
...
Процедура ОбластьШапкиПутевогоЛистаПередВыводом(ОбластьШапка, ДатаДокумента)
	...
	// Данное требование действует с 01.01.2021 по 01.01.2027.
	
	Если Не ВыводитьОбластьСведенийОПеревозке(ДатаДокумента)
		И ОбластьШапка.Области.Найти("СведенияОПеревозке_2021_2027") <> Неопределено Тогда
		
		ОбластьШапка.УдалитьОбласть(
			ОбластьШапка.Области.СведенияОПеревозке_2021_2027,
