...
#Область ОбработчикиСобытийФормыИМодулОбъекта
...
//		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.бит_ВидыКонтактнойИнформацииЖКХ КАК ВидыКонтактнойИнформации
		...
		Иначе
			...
			Если НЕ Форма.Элементы.Найти(СтрокаТаблицы.ИмяРеквизита) = Неопределено Тогда
				...
				Если Элемент.Вид = ВидПоляФормы.ПолеНадписи И Элемент.Гиперссылка Тогда
					// БИТ_ЖКХ 17.03.2016 18:22:10 Начало правки... 
					...
					// БИТ_ЖКХ 17.03.2016 18:22:13 Конец правки. 	
						Продолжить;
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
...
Функция ПолучитьПустуюТаблицуРазмещенияКонтактнойИнформации() Экспорт
	...
	//9.11.2018
//	ТаблицаРазмещения.Колонки.Добавить("Вид",			Новый ОписаниеТипов("СправочникСсылка.бит_ВидыКонтактнойИнформацииЖКХ"));
...
Процедура ДобавитьРеквизитВОписание(Форма, Элемент, СтрокаКИ, ЭтоНовыйВидКИ, ЭтоРеквизитТабличнойЧасти = Ложь, ЭлементДобавлен = Истина)
	...
	Если Не ЭтоРеквизитТабличнойЧасти Тогда
		...
		Если ПустаяСтрока(Представление) Тогда
			Если Элемент.Вид = ВидПоляФормы.ПолеНадписи И Элемент.Гиперссылка Тогда
				// БИТ_ЖКХ 17.03.2016 18:21:48 Начало правки... 
				//Представление = УправлениеКонтактнойИнформациейКлиентСерверБП.ПредставлениеПустойГиперссылки();
				Представление = ПредставлениеПустойГиперссылки();
				// БИТ_ЖКХ 17.03.2016 18:21:49 Конец правки. 
			КонецЕсли;
		КонецЕсли;
...
// БИТ_ЖКХ 17.03.2016 18:20:51 Начало правки...
Функция ПредставлениеПустойГиперссылки()

	Возврат НСтр("ru='Заполнить'");

КонецФункции
// БИТ_ЖКХ 17.03.2016 18:20:51 Конец правки.
