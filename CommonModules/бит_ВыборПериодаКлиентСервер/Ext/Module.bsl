...
// 
// Параметры: 
// МинимальныйПериод   - Перечисление.бит_ДоступныеПериодыОтчета - минимальный вид периода, 
...
// ЗначениеПоУмолчанию - Перечисление.бит_ДоступныеПериодыОтчета - в параметре возвращает вид периода по умолчанию.
//
Процедура ЗаполнитьСписокВыбораВидПериода(Знач МинимальныйПериод, СписокВыбора, ЗначениеПоУмолчанию = Неопределено) Экспорт
	...
	Если ЭлементСписка <> Неопределено Тогда
		...
		Если Не ЗначениеЗаполнено(ЗначениеПоУмолчанию) Тогда
			ЗначениеПоУмолчанию = ПредопределенноеЗначение("Перечисление.бит_ДоступныеПериодыОтчета.ПроизвольныйПериод");
		КонецЕсли;
	Иначе
		Возврат; 
	КонецЕсли;
...
Функция НачалоПериодаОтчета(ВидПериода, ДатаПериода) Экспорт 
	...
	Если ВидПериода = ПредопределенноеЗначение("Перечисление.бит_ДоступныеПериодыОтчета.Год") Тогда
		НачалоПериода = НачалоГода(ДатаПериода);
		
	ИначеЕсли ВидПериода = ПредопределенноеЗначение("Перечисление.бит_ДоступныеПериодыОтчета.Полугодие") Тогда
	...
	ИначеЕсли ВидПериода = ПредопределенноеЗначение("Перечисление.бит_ДоступныеПериодыОтчета.Квартал") Тогда
		НачалоПериода = НачалоКвартала(ДатаПериода);
		
	ИначеЕсли ВидПериода = ПредопределенноеЗначение("Перечисление.бит_ДоступныеПериодыОтчета.Месяц") Тогда
		НачалоПериода = НачалоМесяца(ДатаПериода);
		
	ИначеЕсли ВидПериода = ПредопределенноеЗначение("Перечисление.бит_ДоступныеПериодыОтчета.Декада") Тогда
	...
	ИначеЕсли ВидПериода = ПредопределенноеЗначение("Перечисление.бит_ДоступныеПериодыОтчета.Неделя") Тогда
		НачалоПериода = НачалоНедели(ДатаПериода);
		
	ИначеЕсли ВидПериода = ПредопределенноеЗначение("Перечисление.бит_ДоступныеПериодыОтчета.День") Тогда
...
// Определяет вид периода.
Функция ПолучитьВидПериода(Знач НачалоПериода, Знач КонецПериода, Знач МинимальныйВидПериода = Неопределено) Экспорт 
	
	ВидПериода = ПредопределенноеЗначение("Перечисление.бит_ДоступныеПериодыОтчета.ПроизвольныйПериод");
	Если ЗначениеЗаполнено(НачалоПериода) И ЗначениеЗаполнено(КонецПериода) Тогда
		...
		Если Начало = НачалоДня(НачалоПериода) И Конец = КонецДня(НачалоПериода) Тогда
			ВидПериода = ПредопределенноеЗначение("Перечисление.бит_ДоступныеПериодыОтчета.День");
		ИначеЕсли Начало = НачалоНедели(НачалоПериода) И Конец = КонецНедели(НачалоПериода) Тогда
			ВидПериода = ПредопределенноеЗначение("Перечисление.бит_ДоступныеПериодыОтчета.Неделя");
		ИначеЕсли Начало = НачалоМесяца(НачалоПериода) И Конец = КонецМесяца(НачалоПериода) Тогда
			ВидПериода = ПредопределенноеЗначение("Перечисление.бит_ДоступныеПериодыОтчета.Месяц");
		ИначеЕсли Начало = НачалоКвартала(НачалоПериода) И Конец = КонецКвартала(НачалоПериода) Тогда
			ВидПериода = ПредопределенноеЗначение("Перечисление.бит_ДоступныеПериодыОтчета.Квартал");
		ИначеЕсли Начало = НачалоГода(НачалоПериода) И Конец = КонецГода(НачалоПериода) Тогда
			ВидПериода = ПредопределенноеЗначение("Перечисление.бит_ДоступныеПериодыОтчета.Год");
		ИначеЕсли Начало = Дата(Год(НачалоПериода), 1, 1) И Конец = Дата(Год(НачалоПериода), 6, 30, 23, 59, 59)
			...
			ВидПериода = ПредопределенноеЗначение("Перечисление.бит_ДоступныеПериодыОтчета.Полугодие");
		ИначеЕсли Начало = Дата(Год(НачалоПериода), Месяц(НачалоПериода), 1) 
			...
			ВидПериода = ПредопределенноеЗначение("Перечисление.бит_ДоступныеПериодыОтчета.Декада");
		КонецЕсли;
	КонецЕсли;
...
// Определяет представление периода.
...
// Определяет текущую страницу выбора периода.
...
// Определяет массив слов периода.
...
Процедура ПолучитьЗначениеПериода(ВидПериода, МассивСлов, ДанныеДляВыбора)
	...
	Если ПредопределенноеЗначение("Перечисление.бит_ДоступныеПериодыОтчета.День") = ВидПериода Тогда
	...
	ИначеЕсли ПредопределенноеЗначение("Перечисление.бит_ДоступныеПериодыОтчета.Неделя") = ВидПериода Тогда
	// Период Неделя не обрабатывается.
	ИначеЕсли ПредопределенноеЗначение("Перечисление.бит_ДоступныеПериодыОтчета.Декада") = ВидПериода Тогда
		Если День <= 10 Тогда
			НомерДекады = 1;
	...
	ИначеЕсли ПредопределенноеЗначение("Перечисление.бит_ДоступныеПериодыОтчета.Месяц") = ВидПериода Тогда
		День = 1;
	...
	ИначеЕсли ПредопределенноеЗначение("Перечисление.бит_ДоступныеПериодыОтчета.Квартал") = ВидПериода Тогда
		НомерКвартала = 1;
		Если Месяц <= 3 Тогда
			НомерКвартала = 1;
	...
	ИначеЕсли ПредопределенноеЗначение("Перечисление.бит_ДоступныеПериодыОтчета.Полугодие") = ВидПериода Тогда
		НомерПолугодия = 1;
		Если Месяц <= 6 Тогда
			НомерПолугодия = 1;
		Иначе
	...
	ИначеЕсли ПредопределенноеЗначение("Перечисление.бит_ДоступныеПериодыОтчета.Год") = ВидПериода Тогда
...
Функция ПолучитьСписокДоступныхПериодов()
	...
	СписокДоступныхПериодов.Добавить(ПредопределенноеЗначение("Перечисление.бит_ДоступныеПериодыОтчета.День"));
	СписокДоступныхПериодов.Добавить(ПредопределенноеЗначение("Перечисление.бит_ДоступныеПериодыОтчета.Месяц"));
	СписокДоступныхПериодов.Добавить(ПредопределенноеЗначение("Перечисление.бит_ДоступныеПериодыОтчета.Квартал"));
	СписокДоступныхПериодов.Добавить(ПредопределенноеЗначение("Перечисление.бит_ДоступныеПериодыОтчета.Полугодие"));
	СписокДоступныхПериодов.Добавить(ПредопределенноеЗначение("Перечисление.бит_ДоступныеПериодыОтчета.Год"));
	СписокДоступныхПериодов.Добавить(ПредопределенноеЗначение("Перечисление.бит_ДоступныеПериодыОтчета.ПроизвольныйПериод"));
