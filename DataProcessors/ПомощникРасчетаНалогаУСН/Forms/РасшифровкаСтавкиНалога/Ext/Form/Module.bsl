...
Процедура УправлениеФормой(Форма)
	...
	Для Индекс = 0 По Форма.ОтчетныеПериоды.Количество() - 1 Цикл
		...
		Если Не Форма.УчетЗарплатыВоВнешнейПрограмме
			И ДанныеОтчетногоПериода.ЧисленностьРаботников <> ДанныеОтчетногоПериода.ЧисленностьРаботниковПоДаннымУчета Тогда
...
Функция ТекстПодсказкиЧисленностьРаботников(УчетЗарплатыВоВнешнейПрограмме)
	...
	Если УчетЗарплатыВоВнешнейПрограмме Тогда
		...
		|
		|Если учет зарплаты ведется в программе ""Зарплата и управление персоналом"", численность работников можно определить с помощью отчета ""Численность и текучесть кадров""
		|
...
Функция ТекстПодсказкиПовышеннаяСтавкаНалога(Форма)
	...
	Если НомерКвартала = 2 Тогда
		ЧастиПодсказки.Добавить(НСтр("ru = 'Во 2 квартале'"));
	Иначе
		ЧастиПодсказки.Добавить(СтрШаблон(НСтр("ru = 'В %1 квартале'"), НомерКвартала));
	КонецЕсли;
	
	Если ДанныеПериодаПревышенияЛимитов.ПревышенЛимитДоходов
		И ДанныеПериодаПревышенияЛимитов.ПревышенЛимитРаботников Тогда
		ЧастиПодсказки.Добавить(СтрШаблон(
			НСтр("ru = 'сумма доходов превысила %1 млн. рублей и средняя численность наемных работников превысила%3%2%3человек.'"),
	...
	ИначеЕсли ДанныеПериодаПревышенияЛимитов.ПревышенЛимитДоходов Тогда
		ЧастиПодсказки.Добавить(СтрШаблон(НСтр("ru = 'сумма доходов превысила %1 млн. рублей.'"), ГраницаДоходов));
...
Процедура РассчитатьСтавкуНалога()
	...
	Если ОтчетныйПериод >= УчетУСНКлиентСервер.ДатаНачалаПрогрессивнойШкалы()
		И ПервыйПериодСПревышениемЛимитов(ЭтотОбъект) <> Неопределено Тогда
