...
Функция ОплатитьНаСервере(СпособОплаты)
	...
	// По умолчанию считаем более частотным случай возникновения остатка за предыдущий год
	
	ПараметрыВыполнения.НалогИсчисленныйВсего = УчетНДФЛПредпринимателя.НачисленныйНалогЗаПериод(
		Организация,
		НачалоПериода,
		КонецПериода);
	
	ПараметрыВыполнения.АвансовыеПлатежи = Обработки.ПомощникЗаполнения3НДФЛ.АвансовыеПлатежиПоНДФЛЗаПериод(
		Организация,
		НачалоПериода,
		КонецПериода);
