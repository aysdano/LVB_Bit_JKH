...
Процедура ОтправитьУведомлениеОбОбновлении(Знач ИмяПользователя, Знач АдресНазначения, Знач УспешноеОбновление)
	...
	Тема = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Тема, Метаданные.КраткаяИнформация, бит_ОбщегоНазначенияПовтИсп.ПолучитьНомерРелиза_1С());
	...
	Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1
		...
		|Версия: %3
		|Строка соединения: %4'"),
	Подробности, Метаданные.КраткаяИнформация, бит_ОбщегоНазначенияПовтИсп.ПолучитьНомерРелиза_1С(), СтрокаСоединенияИнформационнойБазы());
...
Функция СведенияОбОбновлении(Знач ИмяФайлаПоставкиОбновления) Экспорт
	...
	Для каждого ИсходнаяКонфигурация Из Описание.ИсходныеКонфигурации Цикл
		...
		Если Не Результат.Подходит
			...
			И ИсходнаяКонфигурация.Версия = бит_ОбщегоНазначенияПовтИсп.ПолучитьНомерРелиза_1С()) Тогда
