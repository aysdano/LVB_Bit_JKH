...
Процедура ПодробнееНажатие(Элемент)
	...
	ФайловаяСистемаКлиент.ОткрытьНавигационнуюСсылку(СсылкаНаСтатьюИТС);
...
Процедура УстановитьПрименяетсяУСНПатент()
	...
	Если ЗначениеЗаполнено(ОтборТекущаяОрганизация) Тогда
		ДатаПроверки = ОбщегоНазначения.ТекущаяДатаПользователя();
		ПрименяетсяУСН = УчетнаяПолитика.ПрименяетсяУСН(ОтборТекущаяОрганизация, ДатаПроверки);
		ПрименяетсяУСНПатент = УчетнаяПолитика.ПрименяетсяУСНПатент(ОтборТекущаяОрганизация, ДатаПроверки);
	Иначе
		ПрименяетсяУСН = Ложь;
...
Процедура ПоказатьИнформациюОПравеПримененияСпецрежима()
	...
	Если Не (ОтборТекущаяОрганизацияИспользование И ЗначениеЗаполнено(ОтборТекущаяОрганизация) 
		И (ПрименяетсяУСН Или ПрименяетсяУСНПатент)) Тогда
