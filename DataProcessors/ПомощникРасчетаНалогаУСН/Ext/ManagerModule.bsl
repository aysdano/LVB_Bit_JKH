...
Процедура ЗаполнитьАктуальныеПоказатели(Показатели)
	...
	ДанныеПрогрессивнойШкалыУСН = УчетУСН.ПараметрыПримененияПовышеннойСтавкиНалога(
	...
	Если ДанныеПрогрессивнойШкалыУСН.ПрименяетсяПовышеннаяСтавка Тогда
		...
		Показатели.СтавкаНалогаПредставление =
			СтрШаблон(НСтр("ru = '%1%%'"), Формат(Показатели.СтавкаНалога, "ЧН=0; БЛ="));
		
		Показатели.ПрименяетсяПовышеннаяСтавкаНалога = Истина;
		Показатели.ПериодПревышенияЛимитов = ДанныеПрогрессивнойШкалыУСН.ПериодПревышенияЛимитов;
		...
		Показатели.НалоговаяБазаДоПревышенияЛимитов = ДанныеПрогрессивнойШкалыУСН.НалоговаяБазаДоПревышенияЛимитов;
		Показатели.НалогДоПревышенияЛимитов = ДанныеПрогрессивнойШкалыУСН.НалогДоПревышенияЛимитов;
	...
	Иначе
		...
		Показатели.СтавкаНалогаПредставление = СтрШаблон(
			НСтр("ru = '%1%%%2'"),
			Формат(Показатели.СтавкаНалога, "ЧН=0; БЛ="),
			?(УчетнаяПолитика.НалоговыеКаникулыУСН(Показатели.Организация, Показатели.КонецПериода),
				 НСтр("ru = ' (налоговые каникулы)'"), ""));
...
Процедура ЗаполнитьНачальныеПоказатели(Показатели)
	...
	Иначе
		...
		Показатели.СтавкаНалогаПредставление = СтрШаблон(
			НСтр("ru = '%1%%%2'"),
			Формат(Показатели.СтавкаНалога, "ЧН=0; БЛ="),
			?(УчетнаяПолитика.НалоговыеКаникулыУСН(Организация, КонецПериода),
				 НСтр("ru = ' (налоговые каникулы)'"), ""));
...
Функция ПубликуемыеГиперссылки(Организация, Период)
	...
	Если ОбщегоНазначенияБПКлиентСервер.НомерКвартала(Период) = 4 Тогда // КУДиР и декларация нужны только при расчете налога за год.
		// КУДиР
