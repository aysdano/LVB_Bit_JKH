...
Функция СформироватьФайлРеестра(ВыборкаПоШапкеДокумента, ИдентификаторФСС)
	...
	ROWSET.version_software = Лев(бит_ОбщегоНазначенияПовтИсп.ПолучитьНомерРелиза_1С(), 15);
...
Процедура ЗаполнитьСведенияОЛисткеНетрудоспособностиВСтрокеРеестра(ROW, СтруктураДанных)
	Если Не ТребуетсяЗаполнитьСведенияОЛисткеНетрудоспособности(СтруктураДанных) Тогда
		Возврат;
	КонецЕсли;
	Если СтруктураДанных.ВидВыплаты <> "1"
		И СтруктураДанных.ВидВыплаты <> "2"
		И СтруктураДанных.ВидВыплаты <> "6" Тогда
		Возврат;
	КонецЕсли;
	...
	КонецЕсли;
	Если ЗначениеЗаполнено(СтруктураДанных.КоличествоОплаченныхДнейНетрудоспособностиИнвалида) Тогда
...
Процедура ЗаполнитьИсключаемыеПериодыВСтрокеРеестра(ROW, ВыборкаСтрок, ИсключаемыеПериоды)
	...
	Для Каждого Период Из ИсключаемыеПериодыСтроки Цикл
		...
		Если ПричинаЧислом = Неопределено Тогда
			...
			СообщенияПользователюБЗК.СообщениеОбъекта(Текст, Период.ПервичныйДокумент, "Объект.НазначитьПособие").Сообщить();
			Продолжить;
		КонецЕсли;
...
Процедура ЗаполнитьИнойДокументПодтверждающийРождениеРебенка(ROW, СтруктураДанных)
...
Функция НомерРеестраДляВыгрузкиВФайл(Реестр, Отказ) Экспорт
	...
	СообщенияПользователюБЗК.СообщитьОбОшибкеВОбъекте(Отказ, Объект, "Номер", ТекстОшибки);
...
Процедура ЗаполнитьСтатусНалогоплательщикаВСтрокеРеестра(ROW, СтруктураДанных)
	...
	// Из спецификации в части поля "RESIDENT_FLAG" - Статус налогоплательщика:
	...
	ИмяПредопределенного = ИмяСтатусаНалогоплательщика(СтруктураДанных.СтатусНалогоплательщикаПолучателя);
	
	Если СтрСравнить(ИмяПредопределенного, "Нерезидент") = 0 Тогда
		
		ROW.RESIDENT_FLAG = 0;
		
	ИначеЕсли СтрСравнить(ИмяПредопределенного, "Резидент") = 0
		Или СтрСравнить(ИмяПредопределенного, "ГражданинСтраныЕАЭС") = 0 Тогда
		
		ROW.RESIDENT_FLAG = 1;
		
	ИначеЕсли СтрСравнить(ИмяПредопределенного, "ВысококвалифицированныйИностранныйСпециалист") = 0 Тогда
		
		ROW.RESIDENT_FLAG = 2;
		
	ИначеЕсли СтрСравнить(ИмяПредопределенного, "УчастникПрограммыПоПереселениюСоотечественников") = 0
		Или СтрСравнить(ИмяПредопределенного, "ЧленЭкипажаСуднаПодФлагомРФ") = 0 Тогда
		
		ROW.RESIDENT_FLAG = 3;
		
	ИначеЕсли СтрСравнить(ИмяПредопределенного, "Беженцы") = 0 Тогда
		
		ROW.RESIDENT_FLAG = 4;
		
	ИначеЕсли СтрСравнить(ИмяПредопределенного, "НерезидентРаботающийНаОснованииПатента") = 0 Тогда
...
Функция СтруктураАдреса(Адрес) Экспорт
	...
	ИменаПолей = "Индекс, Регион, Район, Город, НаселенныйПункт, Территория, Улица, Дом, Корпуса, Квартира";
	СтруктураАдреса = ПрямыеВыплатыПособийСоциальногоСтрахования.СтруктураАдреса(Адрес, ИменаПолей);
	
	// Сворачивание корпусов.
	Литера = "";
	...
	Для Каждого Корпус Из СтруктураАдреса.Корпуса Цикл
		Если Не ЗначениеЗаполнено(Корпус.Номер) Тогда
			Продолжить;
		КонецЕсли;
		ТипКорпуса = СокрЛП(Корпус.ТипКорпуса);
		Номер = СокрЛП(Корпус.Номер);
		Если СтрСравнить(ТипКорпуса, НСтр("ru = 'Литера'")) = 0
			Или СтрСравнить(ТипКорпуса, НСтр("ru = 'Литер'")) = 0
			Или СтрСравнить(ТипКорпуса, НСтр("ru = 'Лит'")) = 0 Тогда
			Литера = Литера + Номер;
			Продолжить;
		КонецЕсли;
		Если СтрСравнить(ТипКорпуса, "Корпус") = 0 Тогда
			ТипКорпусаКратко = НСтр("ru = 'к.'");
		ИначеЕсли СтрСравнить(ТипКорпуса, "Строение") = 0 Тогда
			ТипКорпусаКратко = НСтр("ru = 'стр.'");
		ИначеЕсли СтрСравнить(ТипКорпуса, "Сооружение") = 0 Тогда
			ТипКорпусаКратко = НСтр("ru = 'соор.'");
		ИначеЕсли СтрСравнить(ТипКорпуса, "Участок") = 0 Тогда
			ТипКорпусаКратко = НСтр("ru = 'уч.'");
		Иначе
			ТипКорпусаКратко = ТипКорпуса;
		КонецЕсли;
		ПолныеПредставления.Добавить(ТипКорпуса + " " + Номер);
		КраткиеПредставления.Добавить(ТипКорпусаКратко + " " + Номер);
	КонецЦикла;
	Если Литера <> "" И ПолныеПредставления.Количество() > 0 Тогда
		ПолныеПредставления[0]  = ПолныеПредставления[0]  + Литера;
		КраткиеПредставления[0] = КраткиеПредставления[0] + Литера;
	КонецЕсли;
	Представление = СтрСоединить(ПолныеПредставления, ", ");
	Если СтрДлина(Представление) > 30 Тогда
		Представление = СтрСоединить(КраткиеПредставления, ", ");
	КонецЕсли;
	СтруктураАдреса.Вставить("Корпус", Представление);
