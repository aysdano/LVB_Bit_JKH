...
&НаКлиенте
Процедура ВвезенПослеДатыОбязательнойМаркировкиПриИзменении(Элемент)
	
	Объект.ПриобретенПроизведенПослеДатыОбязательнойМаркировки = Объект.ВвезенПослеДатыОбязательнойМаркировки;
	...
	Если СтрокиПоGTIN.Количество() Тогда
		...
		Для Каждого КлючИЗначение Из ДанныеПоEAN Цикл
			...
			Если Не ЗначениеЗаполнено(ЗначениеПоEAN.Номенклатура) Тогда
				Продолжить;
			КонецЕсли;
			Для Каждого СтрокаТовары Из СтрокиПоGTIN.Получить(КлючИЗначение.Ключ) Цикл
...
Процедура ОбновитьВидимостьДоступностьЭлементовФормы(Форма)
	...
	Если Форма.ЭтоМаркировкаОстатков Тогда
		...
		БылДоступен = ЭтоШины И (Объект.ВвезенПослеДатыОбязательнойМаркировки
			Или Объект.ПриобретенПроизведенПослеДатыОбязательнойМаркировки);
