...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	// БИТ_ЖКХ 15.05.2015{{
	Попытка
		Если ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.бит_ОплатаКУ") Тогда
		//Если Строка(Типзнч(ДокументОснование)) = "Оплата КУ"	Тогда
			
			Если ЗначениеЗаполнено(ТаблицаВзаиморасчеты) Тогда
				
				Если ТаблицаВзаиморасчеты.Количество() > 0 Тогда 
					
					Если НЕ ЗначениеЗаполнено(ТаблицаВзаиморасчеты[0].ДокументРасчетов) Тогда
						
						Для Каждого Строка из ТаблицаВзаиморасчеты Цикл
							
							Строка.ДокументРасчетов = Ссылка;
							
							// БИТ_ЖКХ 15.10.2020
						    Строка.ДокументРасчетов = Документы.ПриходныйКассовыйОрдер.ПустаяСсылка();							
	...
	// БИТ_ЖКХ добавление кода Начало: 17.02.2011 
	Если ВидОперации = Перечисления.ВидыОперацийПКО.ПрочийПриход И НомерЧекаККМ <> 0 Тогда
		
		Движение = Движения.бит_НачисленияКасс.Добавить();
		Движение.Период      = Дата;
		Движение.Регистратор = ЭтотОбъект.Ссылка;
		Движение.ЛицевойСчет = ЭтотОбъект.бит_ЛицевойСчет;
		Движение.КассаККМ    = ЭтотОбъект.бит_КассаККМ;
		Движение.СуммаОплаты = СуммаДокумента;
		
	КонецЕсли; 
	// БИТ_ЖКХ добавление кода конец : 17.02.2011
