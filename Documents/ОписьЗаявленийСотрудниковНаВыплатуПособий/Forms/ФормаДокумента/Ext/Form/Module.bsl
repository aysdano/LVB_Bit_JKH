...
Функция ПодходящиеЗаявления()
	...
	|	И &УсовиеПоОтпускСверхЕжегодногоНаПериодЛечения
	|
	|УПОРЯДОЧИТЬ ПО
	...
	Если Объект.Дата >= ДатаФорм2021 Тогда
		ТекстУсловия = "ИСТИНА";
	Иначе 
		ТекстУсловия ="ЗаявлениеСотрудника.ВидПособия <> ЗНАЧЕНИЕ(Перечисление.ПособияНазначаемыеФСС.ОтпускСверхЕжегодногоНаПериодЛечения)";
	КонецЕсли;
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&УсовиеПоОтпускСверхЕжегодногоНаПериодЛечения", ТекстУсловия);
