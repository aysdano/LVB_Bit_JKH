...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	// БИТ_ЖКХ 22.08.2016 16:31:23 Начало правки... 
	Попытка
		Если Строка(Типзнч(ДокументОснование)) = "Оплата КУ" или
			  // БИТ_ЖКХ 09.07.2019 
			  Строка(Типзнч(ДокументОснование)) = "Счет покупателю" Тогда
					...
					Если НЕ ЗначениеЗаполнено(ТаблицаВзаиморасчеты[0].ДокументРасчетов) Тогда
						...
						Для Каждого Строка из ТаблицаВзаиморасчеты Цикл
							...
							Строка.ДокументРасчетов = Ссылка;
				  		    // БИТ_ЖКХ 15.10.2020
							//Строка.ДокументРасчетов = Документы.ОплатаПлатежнойКартой.ПустаяСсылка();							
	...
	Исключение
	КонецПопытки;
	// БИТ_ЖКХ 22.08.2016 16:31:26 Конец правки. 
	...
	Если Строка(Типзнч(ДокументОснование)) = "Оплата КУ" тогда
		...
		для каждого зап из Движения.ПрочиеРасчеты цикл 
		 зап.РасчетныйДокумент = Ссылка;
	    конеццикла;
