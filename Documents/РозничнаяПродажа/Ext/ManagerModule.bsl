...
Функция СписокНалоговыхРежимовДляПечатиЧека(Организация, Склад = Неопределено, Дата = Неопределено) Экспорт
	...
	Для каждого НалоговыйРежим Из СписокНалоговыхРежимовККТ Цикл
		...
		Если НалоговыйРежим = Перечисления.ТипыСистемНалогообложенияККТ.УСНДоход 
			И УчетнаяПолитика.ПрименяетсяУСНДоходы(Организация, ДатаПолученияСведений) Тогда
		
			СписокНалоговыхРежимовДляПечатиЧека.Добавить(НалоговыйРежим);
		ИначеЕсли НалоговыйРежим = Перечисления.ТипыСистемНалогообложенияККТ.УСНДоходРасход
			И УчетнаяПолитика.ПрименяетсяУСНДоходыМинусРасходы(Организация, ДатаПолученияСведений) Тогда
			
			СписокНалоговыхРежимовДляПечатиЧека.Добавить(НалоговыйРежим);
		ИначеЕсли НалоговыйРежим = Перечисления.ТипыСистемНалогообложенияККТ.Патент
			И УчетнаяПолитика.ПрименяетсяУСНПатент(Организация, ДатаПолученияСведений) Тогда
			
			СписокНалоговыхРежимовДляПечатиЧека.Добавить(НалоговыйРежим);
		ИначеЕсли НалоговыйРежим = Перечисления.ТипыСистемНалогообложенияККТ.ОСН
			И (УчетнаяПолитика.ПлательщикНДФЛ(Организация, ДатаПолученияСведений) 
...
Процедура ОбработкаПолученияФормы(ВидФормы, Параметры, ВыбраннаяФорма, ДополнительнаяИнформация, СтандартнаяОбработка)
	...
	Если НЕ ЗначениеЗаполнено(СистемаНалогообложения) Тогда
		Если Параметры.Свойство("ЗначенияЗаполнения") 
			И ТипЗнч(Параметры.ЗначенияЗаполнения) = Тип("Структура") Тогда
			Если Параметры.ЗначенияЗаполнения.Свойство("СистемаНалогообложения") Тогда
				СистемаНалогообложения = Параметры.ЗначенияЗаполнения.СистемаНалогообложения;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
