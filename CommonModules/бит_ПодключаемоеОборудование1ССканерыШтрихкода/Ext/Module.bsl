...
#Область ОбщиеКомандыВсехОбработчиков
...
#Область СпецифичныеПоТипуОборудованияКоманды
...
Функция ВерсияСтрокаВЧисло(ВерсияСтрока) Экспорт	
Попытка
	Версия = ВерсияСтрока;	
...
Функция СПВНомер(КодСимвола) Экспорт
...
// Из формата (13)CR.
Функция СПВСимволы(СуффиксНомер, ПоУмолчанию) Экспорт
	Номер  = СуффиксНомер;
	СуффиксДрайвера = "";	
	Позиция = Найти(Номер, "#");
...
Функция СПВТекст(Номер) Экспорт
	Результат = Номер;	
