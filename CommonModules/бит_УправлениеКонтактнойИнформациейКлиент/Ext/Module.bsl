...
Функция ПредставлениеНачалоВыбора(Форма, Элемент, Модифицированность = Истина, СтандартнаяОбработка = Ложь) Экспорт
	...
	// БИТ_ЖКХ 25.11.2014{{
	ПредставлениеМетаданные = бит_общегоНазначения.ПолучитьПредставлениеМетаданные(ДанныеСтроки.Вид);
	//9.11.2018
//	Если ПредставлениеМетаданные = "бит_ВидыКонтактнойИнформацииЖКХ" Тогда
	Если ПредставлениеМетаданные = "ВидыКонтактнойИнформации" Тогда
		Если ДанныеСтроки.ЗначенияПолей  = "" Тогда
			
			
			Если Форма.Объект.КонтактнаяИнформация.Количество() > 0 Тогда
				
				Если НЕ Элемент.Имя = "ЮрАдресПроживающего" Тогда 
					//9.11.2018
//					ПредопределенноеЗначение = (ПредопределенноеЗначение("Справочник.бит_ВидыКонтактнойИнформацииЖКХ."+Элемент.Имя));
					ПредопределенноеЗначение = (ПредопределенноеЗначение("Справочник.ВидыКонтактнойИнформации."+Элемент.Имя));

					Отбор = Новый Структура("Вид", ПредопределенноеЗначение);
					
					Строки = Форма.Объект.КонтактнаяИнформация.НайтиСтроки(Отбор);
					
					Если Строки.Количество() > 0 Тогда
						
						ДанныеСтроки.ЗначенияПолей = Строки[0].ЗначенияПолей;	
						
					КонецЕсли;	
					
				Иначе
					
					Для каждого СтрокаАдрес из Форма.Объект.КонтактнаяИнформация Цикл
						
						Если Строка(СтрокаАдрес.Вид) = "Адрес по прописке" Тогда
							
							ДанныеСтроки.ЗначенияПолей = СтрокаАдрес.ЗначенияПолей;	
			...
			ИначеЕсли НЕ Лев(Форма.ИмяФормы,10) = "Справочник" Тогда
				
				Если ЗначениеЗаполнено(Форма.Объект.ОткудаПрибылАдрес) и Элемент.Имя = "ОткудаПрибылПредставление" Тогда	
					
					СтруктураАдресПрибытия = бит_УправлениеКонтактнойИнформацией.ПолучитьСтруктуруАдресаИзСтроки(Форма.Объект.ОткудаПрибылАдрес);
					
					СписокЗначений = Новый СписокЗначений;
					
					Для каждого Строка из СтруктураАдресПрибытия Цикл
						
						СписокЗначений.Добавить(Строка.Значение,Строка.Ключ);
						
					КонецЦикла;	
					
					ДанныеСтроки.ЗначенияПолей = СписокЗначений;
					
				ИначеЕсли ЗначениеЗаполнено(Форма.Объект.КудаВыписанАдрес) и Элемент.Имя = "КудаВыписанПредставление" Тогда	
					
					СтруктураАдресПрибытия = бит_УправлениеКонтактнойИнформацией.ПолучитьСтруктуруАдресаИзСтроки(Форма.Объект.КудаВыписанАдрес);
	...
	Оповещение = Новый ОписаниеОповещения("ПредставлениеНачалоВыбораЗавершение", бит_УправлениеКонтактнойИнформациейКлиент, Новый Структура);
...
Функция ПодключаемаяКоманда(Знач Форма, Знач ИмяКоманды) Экспорт
	...
	Если ИмяКоманды = "КонтактнаяИнформацияДобавитьПолеВвода" Тогда
		Оповещение = Новый ОписаниеОповещения("КонтактнаяИнформацияДобавитьПолеВводаЗавершение", бит_УправлениеКонтактнойИнформацией, Новый Структура);
...
//
//Функция ОткрытьФормуКонтактнойИнформации(Параметры, Владелец = Неопределено, Уникальность = Неопределено, Окно = Неопределено, 
//	Оповещение = Неопределено) Экспорт
//	
//	ВидИнформации = Параметры.ВидКонтактнойИнформации;
//	
//	ИмяОткрываемойФормы = бит_УправлениеКонтактнойИнформациейКлиентСерверПовтИсп.ИмяФормыВводаКонтактнойИнформации(ВидИнформации);
//	
...
Процедура ПредставлениеНачалоВыбораЗавершение(Знач РезультатЗакрытия, Знач ДополнительныеПараметры) Экспорт
	...
	Попытка
		Если ЗначениеЗаполнено(РезультатЗакрытия.ПредставлениеАдресаЗапятые) Тогда
			ДанныеЗаполнения.ПочтовыйАдресЗданияЗапятые	= РезультатЗакрытия.ПредставлениеАдресаЗапятые;
		КонецЕсли;
	Исключение
	КонецПопытки;
	...
	Попытка
		...
		Если ЗначениеЗаполнено(РезультатЗакрытия.ПредставлениеАдресаЗапятые) Тогда
			ДанныеЗаполнения[Элемент.Имя + Строка("Запятые")] = РезультатЗакрытия.ПредставлениеАдресаЗапятые;
		КонецЕсли;
...
//      ТекущееЗначение                - УникальныйИдентификатор,.
//      Строка - текущее значение - или код классификатора, или текст.
...
Процедура НачалоВыбораУлицы(Владелец, ИдентификаторНаселенногоПункта, ТекущееЗначение, ДополнительныеПараметры) Экспорт
	...
	ОткрытьФорму("Обработка.РасширенныйВводКонтактнойИнформации.Форма.ВыборУлицы", Параметры, Владелец, 
...
Процедура ПредложениеЗагрузкиКлассификатора(Знач Текст = "", Знач Регион = Неопределено) Экспорт
	...
	ОткрытьФорму("Обработка.РасширенныйВводКонтактнойИнформации.Форма.ЗагрузкаАдресногоКлассификатора", ПараметрыЗагрузки, ЭтотОбъект,,,, Оповещение, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ВвестиКомментарий(Знач Форма, Знач ИмяРеквизита, Знач НайденнаяСтрока, Знач Результат)
	...
	Оповещение = Новый ОписаниеОповещения("ВвестиКомментарийЗавершение", бит_УправлениеКонтактнойИнформацией, Новый Структура);
	Оповещение.ДополнительныеПараметры.Вставить("Форма", Форма);
