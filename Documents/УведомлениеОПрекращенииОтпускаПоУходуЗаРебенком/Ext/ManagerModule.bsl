...
Процедура ПриПечатиУведомления(КонтекстПечати)
	...
	Пока Выборка.Следующий() Цикл
		...
		ВывестУведомлениеОПрекращенииОтпускаПоУходуЗаРебенком_2021(СекцияШапка, Выборка);
...
Процедура ВывестУведомлениеОПрекращенииОтпускаПоУходуЗаРебенком_2021(Макет, Выборка)
	...
	ЗарплатаКадры.ВывестиДанныеПоБуквенно(ЗарплатаКадры.РазбитьСтрокуНаПодСтроки(
		ВРег(Выборка.НаименованиеСтрахователя), ДлиныСтрок), Макет, "ФИО_Заявителя_", 135);
	...
	Документы.ЗаявлениеСотрудникаНаВыплатуПособия.ЗаполнитьВМакетеРазрешениеНаПроживание(Макет, Выборка);
