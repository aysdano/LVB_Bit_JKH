...
Функция ПроверкаВозможностиПереходаНаКлиенте(ИмяТекущейСтраницы, НаправлениеПерехода)
	...
	ИначеЕсли ИмяТекущейСтраницы = "СтраницаПодключениеКВебИТС" Тогда
		Если НаправлениеПерехода = "Далее" Тогда
			// Проверить правильность заполнения логина / пароля.
			РезультатПроверкиЛогинаПароля = ИнтернетПоддержкаПользователейКлиентСервер.ПроверитьДанныеАутентификации(
				Новый Структура("Логин, Пароль",
				Логин, Пароль));
			Если РезультатПроверкиЛогинаПароля.Отказ = Истина Тогда
				Результат = Ложь;
				// Вывести сообщение об ошибке.
				Элементы.ПанельОшибкаПодключения.Видимость = Истина;
				Элементы.ДекорацияОшибкаПодключенияКВебИТСТекст.Заголовок = РезультатПроверкиЛогинаПароля.СообщениеОбОшибке;
			Иначе
