...
Функция СформироватьАвтоматическиРеглОтчет(ПараметрыФормирования) Экспорт
	...
	Если ФормаЗаполняетсяПомощником(ПараметрыФормирования.ИмяФормыОтчета) Тогда
		...
		СтруктураПараметров.Вставить("ТаблицаДанныхЗаполнения",       РасчетНалога.ТаблицаДанныхЗаполнения);
...
Функция РассчитатьНалог(Организация, Период, ДанныеПомощникаЗаполнения, ВыбраннаяФорма3НДФЛ)
	...
	ДобавитьИтоговыеПоказателиПредпринимательскойДеятельности(ДанныеПомощникаЗаполнения, ТаблицаДанныхЗаполнения, ДоходыВычеты, ВыбраннаяФорма3НДФЛ);
	...
	ТаблицаНалоговойБазы = ТаблицаНалоговойБазы(ТаблицаДанныхЗаполнения, ВыбраннаяФорма3НДФЛ, ВидыДоходов.Иное, НалоговыеСтавки.ПоУмолчанию);
	...
	Для Каждого СтрокаНалоговойБазы Из ТаблицаНалоговойБазы Цикл
		...
		Если СтрокаНалоговойБазы.НалоговаяСтавка = НалоговыеСтавки.ПоУмолчанию
			И СтрокаНалоговойБазы.ВидДохода = ВидыДоходов.Иное Тогда
	...
	Результат = НовыйРезультатРасчетаНалога();
...
Функция СоздатьЗаполнитьДекларацию(СтруктураПараметров)
	...
	ДоходыВычеты = СтруктураПараметров.ДанныеПомощникаЗаполнения.ДоходыВычеты;
...
Функция НовыйРезультатРасчетаНалога()
	...
	Результат.Вставить("ТаблицаДанныхЗаполнения", Отчеты.РегламентированныйОтчет3НДФЛ.НоваяТаблицаДанныхЗаполнения()); // ТаблицаЗначений - таблица, содержащая данные для заполнения декларации.
	
	Результат.Вставить("ПоясненияРасчетаНалога", НовыйПоясненияРасчетаНалогаВДекларации()); // см. НовыйПоясненияРасчетаНалогаВДекларации
...
//
Функция НовыйПоясненияРасчетаНалогаВДекларации()
...
Функция СтруктураНавигации() Экспорт
	...
	СтруктураНавигации.Структура.Вставить(НавигацияПомощниковКлиентСервер.ИмяШага(2), СтруктураШагаРасчетНалога(2));
	СтруктураНавигации.Структура.Вставить(НавигацияПомощниковКлиентСервер.ИмяШага(3), СтруктураШагаДекларация(3));
	СтруктураНавигации.Структура.Вставить(НавигацияПомощниковКлиентСервер.ИмяШага(4), СтруктураШагаОплатаНалога(4));
...
Функция ТаблицаНалоговойБазы(ТаблицаДанныхЗаполнения, ВыбраннаяФорма3НДФЛ, ВидДоходаПоУмолчанию, НалоговаяСтавкаПоУмолчанию)
...
Функция ДанныеЗаполнения_ПредпринимательскаяДеятельность(ДанныеФормы, КодВидаДеятельности)
	...
	Если ДанныеФормы.Свойство("Показатели1Квартал") Тогда
		ДанныеДекларации.Вставить("Показатели1Квартал", ДанныеФормы.Показатели1Квартал);
		ДанныеДекларации.Вставить("ПоказателиПолугодие", ДанныеФормы.ПоказателиПолугодие);
		ДанныеДекларации.Вставить("Показатели9Месяцев", ДанныеФормы.Показатели9Месяцев);
	КонецЕсли;
...
Функция ДанныеЗаполнения_СведенияОСуммахНалогаИП(РезультатРасчета, ПризнакиУплатыВозврата, ВыбраннаяФорма3НДФЛ, ТаблицаДанныхЗаполнения)
	...
	НалогЗаГод = РезультатРасчета.СуммаНалога - АвансовыеПлатежиИП.АвансовыйПлатежИП9Месяцев;
	
	Если НалогЗаГод < 0 Тогда
		ДанныеДекларации.Вставить("СуммаНалогаИПКУплатеГод", 0);
		ДанныеДекларации.Вставить("СуммаНалогаИПКУменьшениюГод", -НалогЗаГод);
	Иначе
		ДанныеДекларации.Вставить("СуммаНалогаИПКУплатеГод", НалогЗаГод);
...
Процедура ДобавитьИтоговыеПоказателиПредпринимательскойДеятельности(ДанныеПомощникаЗаполнения, ТаблицаДанныхЗаполнения, ДоходыВычеты, ВыбраннаяФорма3НДФЛ)
	...
	Для НомерПериода = 1 По 3 Цикл
		...
		ПоказателиАвансовыхПлатежей.Вставить(СтрШаблон("ДоходыИП%1", ИмяПериода), 0);
		ПоказателиАвансовыхПлатежей.Вставить(СтрШаблон("ПрофВычетИП%1", ИмяПериода), 0);
	...
	Для Каждого СтрокаДанныхЗаполнения Из НайденныеСтроки Цикл
		...
		Если СтруктураДанных.Свойство("Показатели1Квартал") Тогда
			ЗаполнятьИсчисленныеАвансовыеПлатежи = Истина;
			
			Для НомерПериода = 1 По 3 Цикл
				ИмяПериода = ПомощникЗаполнения3НДФЛКлиентСервер.ИмяПериодаНарастающимИтогом(НомерПериода);
				
				ПоказателиАвансовыхПлатежей[СтрШаблон("ДоходыИП%1", ИмяПериода)] =
					ПоказателиАвансовыхПлатежей[СтрШаблон("ДоходыИП%1", ИмяПериода)]
					+ Окр(СтруктураДанных[СтрШаблон("Показатели%1", ИмяПериода)].Доходы, 0);
				
				ПоказателиАвансовыхПлатежей[СтрШаблон("ПрофВычетИП%1", ИмяПериода)] =
					ПоказателиАвансовыхПлатежей[СтрШаблон("ПрофВычетИП%1", ИмяПериода)]
					+ Окр(СтруктураДанных[СтрШаблон("Показатели%1", ИмяПериода)].ПрофВычет, 0);
			КонецЦикла;
		КонецЕсли;
	КонецЦикла;
	...
	Если ЗаполнятьИсчисленныеАвансовыеПлатежи Тогда
		Для НомерПериода = 1 По 3 Цикл
			...
			ПоказателиАвансовыхПлатежей[СтрШаблон("СтандартныйВычетИП%1", ИмяПериода)] =
				Окр(СуммаСтандарныхВычетовЗаПериод(ДоходыВычеты, НомерПериода * 3, ВыбраннаяФорма3НДФЛ), 0);
			
			НалоговаяБаза = 
				Макс(0, ПоказателиАвансовыхПлатежей[СтрШаблон("ДоходыИП%1", ИмяПериода)]
				- ПоказателиАвансовыхПлатежей[СтрШаблон("ПрофВычетИП%1", ИмяПериода)]
				- ПоказателиАвансовыхПлатежей[СтрШаблон("СтандартныйВычетИП%1", ИмяПериода)]);
			...
			ПоказателиАвансовыхПлатежей[СтрШаблон("АвансовыйПлатежИП%1", ИмяПериода)] =
				Окр(НалоговаяБаза * ИтоговаяСтрока.НалоговаяСтавка / 100, 0);
		КонецЦикла;
...
Функция СуммаСтандарныхВычетовЗаПериод(ДоходыВычеты, КоличествоМесяцев, ВыбраннаяФорма)
	...
	НайденныеСтроки = ДоходыВычеты.НайтиСтроки(ОтборСтрок);
	Если НайденныеСтроки.Количество() > 0
		И НайденныеСтроки[0].СтруктураДоходовВычетов.СведенияОВычетах <> Неопределено Тогда
		
		Для Каждого Вычет Из НайденныеСтроки[0].СтруктураДоходовВычетов.СведенияОВычетах Цикл
			Если Вычет.НомерМесяца <= КоличествоМесяцев Тогда
				СуммаВычетов = СуммаВычетов + Вычет.СуммаВычета;
			КонецЕсли;
		КонецЦикла;
	...
	Для Каждого СтрокаНалоговаяБаза Из ТаблицаНалоговойБазы Цикл
		...
		Для Индекс = 0 По ПорядокКБК.ВГраница() Цикл
			...
			Для Каждого ДанныеЗаполнения Из ТаблицаДанныхЗаполнения.НайтиСтроки(ОтборДанныхЗаполнения) Цикл
				...
				СуммаРасхода = Макс(0, Мин(НалоговаяБаза, ДанныеНалоговаяБаза.СуммаРасходов));
				НалоговаяБаза = НалоговаяБаза - СуммаРасхода;
				ДанныеНалоговаяБаза.СуммаРасходов = ДанныеНалоговаяБаза.СуммаРасходов - СуммаРасхода;
...
Функция СуммаПоложенныхВычетов(ТаблицаДанныхЗаполнения)
	...
	Для Каждого ТекущаяСтрока Из ТаблицаДанныхЗаполнения Цикл
		...
		Если ТекущаяСтрока.Вид = Перечисления.ВычетыФизическихЛиц.НаПриобретениеИлиСтроительствоНедвижимости Тогда
			Результат = Результат + ТекущаяСтрока.СтруктураДанных.СуммаРасходовНаПриобретениеНедвижимостиКВычету;
...
Процедура ДобавитьДанныеЗаявленияНаВозвратНалога(ТаблицаДанныхЗаполнения, Организация, СчетОрганизации, Период, ВариантДействияСНалогомКВозврату, НомерКорректировки, ВыбраннаяФорма3НДФЛ)
	...
	Для Каждого СтрокаСведений Из ТаблицаДанныхЗаполнения Цикл
		...
		Если СтруктураДанных.Свойство("КБКИП") Тогда
			...
			СуммаКУплате = СтруктураДанных.СуммаНалогаИПКУплатеГод;
		...
		ИначеЕсли СтруктураДанных.Свойство("ПризнакУплатыВозврата") Тогда
			...
			СуммаКУплате = СтруктураДанных.СуммаНалога;
	...
	Если УчетнаяПолитика.ПлательщикНДФЛ(Организация, КонецГода(Период)) Тогда
		КБКПоУмолчанию = СписокКБК.ДоходыОтПредпринимательскойДеятельности;
	Иначе
		КБКПоУмолчанию = СписокКБК.ДоходыПриПолученииКоторыхНеУдержанНалог;
	КонецЕсли;
...
Процедура ЗаполнитьПоясненияРасчетаНалогаВДекларации(ПоясненияРасчетаНалога, Знач ТаблицаНалоговойБазы, ДанныеПомощникаЗаполнения, ВыбраннаяФорма3НДФЛ)
	...
	Если ТаблицаНалоговойБазы.Количество() = 0 Тогда
		...
		РасчетНалога.СтруктураДанных = НоваяСтруктураРасчетаНалоговойБазы(НалоговыеСтавки.ПоУмолчанию, ВидыДоходов.Иное);
	...
	Для Каждого СтрокаНалоговойБазы Из ТаблицаНалоговойБазы Цикл
		...
		Если СтрокаНалоговаяСтавка.ВидДохода = ВидыДоходов.Дивиденды Тогда
		...
		Если СтрокаНалоговаяСтавка.НалоговаяСтавка = НалоговыеСтавки.ПоУмолчанию
			И СтрокаНалоговаяСтавка.ВидДохода = ВидыДоходов.Иное Тогда
...
Функция НовыйТаблицаПлатежейНДФЛ()
...
// Возвращает массив созданных документов оплаты НДФЛ
//
// Параметры:
...
Функция СформироватьДокументыОплатыНалога(Показатели, Платежи = Неопределено) Экспорт
	...
	СтруктураПараметров = Новый Структура;
	СтруктураПараметров.Вставить("Правило",         Показатели.Правило);
	СтруктураПараметров.Вставить("ПериодСобытия",   Показатели.Период);
	СтруктураПараметров.Вставить("Организация",     Показатели.Организация);
	СтруктураПараметров.Вставить("НалоговыйПериод", Показатели.Период);
	СтруктураПараметров.Вставить("Платежи",         ПоместитьВоВременноеХранилище(ТаблицаПлатежей));
	СтруктураПараметров.Вставить("СпособОплаты",    Показатели.СпособОплаты);
	СтруктураПараметров.Вставить("СчетОрганизации", СчетОрганизации);
	
	СозданныеДокументы = Обработки.ФормированиеПлатежныхПорученийНаУплатуНалогов.СоздатьПлатежныеДокументы(
		СтруктураПараметров);
...
Функция НеоплаченныеНалоги(Показатели, ПлатежиНДФЛ)
