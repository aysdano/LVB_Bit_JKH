...
Процедура ПодготовитьФормуНаСервере()
	...
	ВидыОпераций = ПолучитьСписокВидовОпераций(ЗначенияЗаполнения, ОтборПоВидуОперации);
...
Функция ПолучитьСписокВидовОпераций(ЗначенияЗаполнения, ВидОперацииОтбор)
	...
	Если ЗначениеЗаполнено(ВидОперацииОтбор) Тогда
		ДобавитьВидОперации(СписокВидовОпераций, ВидОперацииОтбор, Организация, Склад, Ложь)
	Иначе
		...
		Для Каждого ЗначениеПеречисления Из ЗначенияПеречисления Цикл
			ДобавитьВидОперации(СписокВидовОпераций, 
				Перечисления.ВидыОперацийРозничнаяПродажа[ЗначениеПеречисления.Имя], 
				Организация, Склад, Истина)
		КонецЦикла;
	КонецЕсли; 
...
&НаСервереБезКонтекста
Процедура ДобавитьВидОперации(СписокВидовОпераций, ТекущийВидОперации, Организация, Склад, ОтображатьВидОперацииВСпискеВыбора)
	Если ТекущийВидОперации = Перечисления.ВидыОперацийРозничнаяПродажа.Продажа Тогда
		Если ЗначениеЗаполнено(Организация) И ПолучитьФункциональнуюОпцию("ИспользоватьФискальныйРегистратор") Тогда
			СписокНалоговыхРежимовДляПечатиЧека = Документы.РозничнаяПродажа.СписокНалоговыхРежимовДляПечатиЧека(Организация, Склад);
			
			Если СписокНалоговыхРежимовДляПечатиЧека.Количество() < 2 Тогда
