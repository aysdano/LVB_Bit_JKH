...
Процедура ТребуетсяЭлектроннаяПодписьРаздел4Часть3ОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	// В БСП нет программного интерфейса для открытия формы
	...
	ПараметрыСоздания.Вставить("Организация", ?(ДостаточноПодписиОсновнойОрганизации,
		Объект.Организация, ТекущийСубъект));
...
#Область ОбработчикиСобытийЭлементовГруппыЭлектроннаяПодпись

&НаКлиенте
Процедура ТребуетсяЭлектроннаяПодписьЗакрытьНажатие(Элемент)
	
	Отбор = Новый Структура;
	Отбор.Вставить("Субъект", ТекущийСубъект);
	РезультатПоиска = РезультатПоискаСертификатов.НайтиСтроки(Отбор);
	
	Если РезультатПоиска.Количество() > 0 Тогда
		СтрокаТаблицы = РезультатПоиска[0];
		СтрокаТаблицы.ОтображатьБаннер = Ложь;
	КонецЕсли;
	
	Элементы.ГруппаТребуетсяЭлектроннаяПодпись.Видимость = Ложь;
...
Процедура Подключаемый_ЭкономическиеСубъектыПриАктивизацииСтроки()
	...
	ОтобразитьБаннерТребуетсяЭлектроннаяПодпись();
...
Функция ВсеОтчетыПодписаны(ЭлементыДереваОтчетов)
	...
	Для каждого ЭлементДерева Из ЭлементыДереваОтчетов Цикл
		Если ЗначениеЗаполнено(ЭлементДерева.КлючСтрокиОтчета) Тогда // Это отчет
			Если ЭлементДерева.ДанныеОтсутствуют Тогда
...
Процедура ОчиститьДанныеСтрокиДерева(ИдентификаторИлиСтрокаДереваОтчетов, ДанныеОтсутствуют)
	...
	СтрокаДереваОтчетов.ОтчетЗаполнен          = ОтчетЗаполнен(СтрокаДереваОтчетов);
	СтрокаДереваОтчетов.РучнаяКорректировка    = Ложь;
...
Процедура ПослеЗакрытияОтчета(РезультатЗакрытия, ДополнительныеПараметры)
	...
	РучнаяКорректировка = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(РезультатЗакрытия, "РучнаяКорректировка", Ложь);
	
	ЗаписатьОтчетИСохранитьРезультатВПрисоединенныеФайлы(
		РезультатЗакрытия.Результат, СтрокаОтчета.КлючСтрокиОтчета, РучнаяКорректировка);
	
КонецПроцедуры

&НаСервере
Процедура ЗаписатьОтчетИСохранитьРезультатВПрисоединенныеФайлы(Результат, КлючСтрокиОтчета, РучнаяКорректировка)
	
	АдресХранилища = ПоместитьВоВременноеХранилище(Результат, УникальныйИдентификатор);
	
	СохранитьОтчетВПрисоединенныйФайл(КлючСтрокиОтчета, АдресХранилища, РучнаяКорректировка);
...
&НаСервере
Функция СохранитьОтчетВПрисоединенныйФайл(КлючСтрокиОтчета, АдресХранилища, РучнаяКорректировка)
	...
	СтрокаОтчета.РучнаяКорректировка = РучнаяКорректировка;
...
Процедура ОбновитьСтрокиПоРезультатуЗаполнения(Знач РезультатЗаполнения)
	...
	Для каждого СтрокаРезультата Из РезультатЗаполнения.Отчетность Цикл
		...
		ИначеЕсли ЗначениеЗаполнено(СтрокаРезультата.ФайлОтчета) Тогда
			...
			НайденнаяСтрока.РучнаяКорректировка = Ложь;
...
Процедура ПодписатьФинОтчет(ДополнительныеПараметры)
	...
	ВладелецПодписи = ?(ДостаточноПодписиОсновнойОрганизации, Объект.Организация, ТекущийСубъект);
	Отбор = Новый Структура;
	Отбор.Вставить("Субъект", ВладелецПодписи);
	РезультатПоиска = РезультатПоискаСертификатов.НайтиСтроки(Отбор);
	Если РезультатПоиска.Количество() > 0 Тогда
		РезультатПоискаСертификата = РезультатПоиска[0];
		
		Если Не РезультатПоискаСертификата.СертификатНайден Тогда
			ПоказатьПредупреждение(, Новый ФорматированнаяСтрока(
				Новый ФорматированнаяСтрока(СтрШаблон(НСтр("ru = 'Не найдена электронная подпись
				|%1'"), ВладелецПодписи), Новый Шрифт(,, Истина)),
				НСтр("ru = '
				|
				|Установите имеющуюся электронную подпись
				|'"),
				Новый ФорматированнаяСтрока(НСтр("ru = 'по инструкции'"),,,,
					УниверсальныйОбменСБанкамиФормыКлиентСервер.СсылкаНаИнструкциюПоУстановкеЭлектроннойПодписи()),
				НСтр("ru = ' или оформите '"),
				Новый ФорматированнаяСтрока(НСтр("ru = '1C:Подпись'"),,,,
					УниверсальныйОбменСБанкамиФормыКлиентСервер.СсылкаОписание1СПодпись())
			));
			Возврат;
		КонецЕсли;
	КонецЕсли;
...
&НаСервереБезКонтекста
Функция ДанныеСубъектаДляПоискаСертификата(Знач Субъект)
	
	Возврат ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Субъект, "ЮридическоеФизическоеЛицо, ИНН");
	
КонецФункции

&НаКлиенте
Процедура НайтиСертификат(Субъект)
	
	ДанныеСубъекта = ДанныеСубъектаДляПоискаСертификата(Субъект);
	
	ОтборОрганизация = ?(ТипЗнч(Субъект) = Тип("СправочникСсылка.Организации"), Субъект, Объект.Организация);
	
	ПараметрыОтбора = ЗаявкиНаКредитКлиентСервер.ПараметрыОтбораСертификата();
	ПараметрыОтбора.Сервис                    = ПредопределенноеЗначение("Перечисление.СервисыОбменаСБанками.ФинансоваяОтчетность");
	ПараметрыОтбора.Организация               = ОтборОрганизация;
	ПараметрыОтбора.Дата                      = ТекущаяДата();
	ПараметрыОтбора.ИНН                       = ДанныеСубъекта.ИНН;
	ПараметрыОтбора.ЮридическоеФизическоеЛицо = ДанныеСубъекта.ЮридическоеФизическоеЛицо;
	
	ДополнительныеПараметры = Новый Структура;
	ДополнительныеПараметры.Вставить("Субъект", Субъект);
	ОписаниеОповещения = Новый ОписаниеОповещения("НайтиСертификатЗавершение", ЭтотОбъект, ДополнительныеПараметры);
...
Процедура НайтиСертификатЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Субъект = ДополнительныеПараметры.Субъект;
	
	СтрокаТаблицы = РезультатПоискаСертификатов.Добавить();
	СтрокаТаблицы.Субъект = Субъект;
	СтрокаТаблицы.ОтображатьБаннер = Истина;
	
	СтрокаТаблицы.СертификатНайден = Ложь;
	Если ТипЗнч(Результат) = Тип("Структура") Тогда
		Если Результат.Свойство("Выполнено")
			...
			СтрокаТаблицы.СертификатНайден = Истина;
		КонецЕсли;
	КонецЕсли;
	
	Если НЕ ДостаточноПодписиОсновнойОрганизации
		И Элементы.ЭкономическиеСубъекты.ТекущиеДанные <> Неопределено
		И Элементы.ЭкономическиеСубъекты.ТекущиеДанные.Субъект <> Субъект Тогда
		Возврат; // На момент завершения поиска активен другой субъект.
	КонецЕсли;
	
	Элементы.ГруппаТребуетсяЭлектроннаяПодпись.Видимость = НЕ СтрокаТаблицы.СертификатНайден;
	
КонецПроцедуры

&НаКлиенте
Процедура ОтобразитьБаннерТребуетсяЭлектроннаяПодпись()
	
	Если ВсеОтчетыПодписаны(ДеревоОтчетов.ПолучитьЭлементы()) Тогда
		Элементы.ГруппаТребуетсяЭлектроннаяПодпись.Видимость = Ложь;
		Возврат;
	КонецЕсли;
	...
	Если ДостаточноПодписиОсновнойОрганизации Тогда
		Субъект = Объект.Организация;
	ИначеЕсли Элементы.ЭкономическиеСубъекты.ТекущиеДанные <> Неопределено Тогда
		Субъект = Элементы.ЭкономическиеСубъекты.ТекущиеДанные.Субъект;
	КонецЕсли;
	...
	Отбор = Новый Структура;
	Отбор.Вставить("Субъект", Субъект);
	РезультатПоиска = РезультатПоискаСертификатов.НайтиСтроки(Отбор);
	
	Если РезультатПоиска.Количество() > 0 Тогда
		РезультатПоискаСертификата = РезультатПоиска[0];
		Элементы.ГруппаТребуетсяЭлектроннаяПодпись.Видимость = РезультатПоискаСертификата.ОтображатьБаннер
			И НЕ РезультатПоискаСертификата.СертификатНайден;
	Иначе // Начнем поиск сертификата и покажем баннер, если не найдем.
		Элементы.ГруппаТребуетсяЭлектроннаяПодпись.Видимость = Ложь;
		НайтиСертификат(Субъект);
	КонецЕсли;
