...
Функция АктуализироватьРасчет(ПараметрыРасчета) Экспорт
	...
	ПараметрыПроверки.Период      = КонецКвартала(ПараметрыРасчета.Период);
...
Функция ОписаниеБлокаРасчетаСуммыВзносов(Организация, НалоговыйПериод, СтраховыеВзносыКУплате, ТипВзноса)
	...
	ИначеЕсли ТипВзноса = ВсеВзносы.ПФРсДоходов Тогда
		...
		ОписаниеБлока.Вставить(СловарьПолейJSON.Описание,
			СтрШаблон("%1 %2", ПоясненияРасчета.СуммаВзносаПояснение, ПоясненияРасчета.СуммаВзносаФормула));
	КонецЕсли;
