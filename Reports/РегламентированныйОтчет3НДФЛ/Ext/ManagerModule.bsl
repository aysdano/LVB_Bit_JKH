...
Функция ДанныеРеглОтчета(ЭкземплярРеглОтчета) Экспорт
	...
	Если    ЭкземплярРеглОтчета.ВыбраннаяФорма = "ФормаОтчета2018кв1"
		...
		УплаченныеАвансы = 0;
		Если ЭкземплярРеглОтчета.ВыбраннаяФорма = "ФормаОтчета2021кв1" Тогда
			ДанныеРаздела2 = ПрочитатьДанныеДекларации(ДанныеОтчета, ЭкземплярРеглОтчета.ВыбраннаяФорма, "Раздел2");
		...
		Для Каждого СтрокаДекларации Из ДанныеДекларации Цикл
			...
			Если ЭкземплярРеглОтчета.ВыбраннаяФорма = "ФормаОтчета2021кв1"
				И ЗначениеЗаполнено(СтрокаДекларации.КБКИП) Тогда
				Сумма = ТаблицаДанныхРеглОтчета.Добавить();
				Сумма.Период    = Период;
				Сумма.ВидНалога = ВидНалогаПоКБК(СтрокаДекларации.КБКИП, ЭкземплярРеглОтчета.ВыбраннаяФорма);
				Сумма.КБК       = СтрокаДекларации.КБКИП;
				Сумма.ОКАТО     = СтрокаДекларации.ОКТМОИПГод;
				
				Сумма.Сумма = СтрокаДекларации.СуммаНалогаИПКУплате1Квартал
					+ СтрокаДекларации.СуммаНалогаИПКУплатеПолугодие
					- СтрокаДекларации.СуммаНалогаИПКУменьшениюПолугодие
					+ СтрокаДекларации.СуммаНалогаИПКУплате9Месяцев
					- СтрокаДекларации.СуммаНалогаИПКУменьшению9Месяцев
					+ СтрокаДекларации.СуммаНалогаИПКУплатеГод
					- СтрокаДекларации.СуммаНалогаИПКУменьшениюГод
					- УплаченныеАвансы;
			КонецЕсли;
		КонецЦикла;
		...
		Если СвойствоРаздела.МногостраничностьВРазделе Тогда
			...
			Если РазделМногострочный Тогда
				Если Не НомераДобавленныхСтрок.Свойство(ИмяРаздела, НомерСтроки) Тогда
					...
					Если СтраницаРаздела.ДанныеДопСтрок.Количество() = МаксимальноеКоличествоСтрокНаСтранице
...
Функция ИмяФайлаСхемы(ВыбраннаяФорма, ДатаПодписи)
	...
	Если ВыбраннаяФорма = РеквизитыФормы2021().ИмяОбъекта Тогда
...
Функция ИмяМакетаДереваВыгрузкиXML(ВыбраннаяФорма)
	...
	Если ВыбраннаяФорма = РеквизитыФормыОтчета2021().ФормаОтчета Тогда
...
Функция СтруктураРеквизитовФормы(Организация, Период, ВыбраннаяФорма, ПараметрыОткрытойФормы = Неопределено) Экспорт
	...
	Если ПараметрыФормы.мВыбраннаяФорма = РеквизитыФормы2021().ИмяОбъекта Тогда
...
Функция КодыВидовДоходовРФ(ВыбраннаяФорма) Экспорт
	...
	Если ВыбраннаяФорма = РеквизитыФормыОтчета2020().ФормаОтчета
		Или ВыбраннаяФорма = РеквизитыФормыОтчета2021().ФормаОтчета Тогда
