...
#Если ТолстыйКлиентОбычноеПриложение Тогда
	бит_УправлениеПечатьюВызовСервера.СформироватьПечатныеФормыДляБыстройПечатиОбычноеПриложение(
	...
	КонецЕсли;
#Иначе
	бит_УправлениеПечатьюВызовСервера.СформироватьПечатныеФормыДляБыстройПечати(
			ИмяМенеджераПечати, ИменаМакетов, ПараметрКоманды, ПараметрыПечати,
...
Процедура ВыполнитьПодключаемуюКомандуПечати(Знач Команда, Знач Форма, Знач Источник) Экспорт
	...
	Если ТипЗнч(Команда) = Тип("КомандаФормы") Тогда
		ОписаниеКоманды = бит_УправлениеПечатьюКлиентПовтИсп.ОписаниеКомандыПечати(Команда.Имя, Форма.ИмяФормы);
	КонецЕсли;
	...
	Если Не ОписаниеКоманды.НеВыполнятьЗаписьВФорме И ТипЗнч(Источник) = Тип("ДанныеФормыСтруктура")
		...
		Если Источник.Ссылка.Пустая() Тогда
			...
			бит_ОбщегоНазначенияКлиентСервер.ВывестиСообщение("Данные записаны.");
...
Функция ПроверитьДокументыПроведены(ДокументыМассив, ФормаИсточник = Неопределено) Экспорт
	...
	Если КоличествоНепроведенныхДокументов > 0 Тогда
		...
		Если КоличествоНепроведенныхДокументов = 1 Тогда
			бит_ОбщегоНазначенияКлиентСервер.ВывестиСообщение("Выполнено проведение документа.");
		Иначе
			бит_ОбщегоНазначенияКлиентСервер.ВывестиСообщение("Выполнено проведение документов.");
		КонецЕсли;
...
// Формирует печатную форму документа с использованием макета в формате офисного документа (MS Word).
//
Функция ПечатьВФорматеОфисногоДокумента(ПараметрыПечати) Экспорт
	...
	СведенияОМакете = Новый Структура("ИмяМакета, ПользовательскиеМакетыПечати, ВнешнийМакет, Расширение");
	СведенияОМакете.ИмяМакета                    = ПараметрыПечати.Идентификатор;
	СведенияОМакете.ПользовательскиеМакетыПечати = ПараметрыПечати.ПользовательскиеМакетыПечати;
	СведенияОМакете.ВнешнийМакет                 = ПараметрыПечати.ВнешнийМакет;
	СведенияОМакете.Расширение                   = ПараметрыПечати.Расширение;
	...
	МакетИДанныеОбъекта = бит_УправлениеПечатьюВызовСервера.ПолучитьМакетыИДанныеОбъектов(ИмяМенеджераПечати, СведенияОМакете, СоставДокументов);
	
	Для Каждого ДокументСсылка Из СоставДокументов Цикл
		НапечататьОфисныйДокумент(ДокументСсылка, МакетИДанныеОбъекта, СведенияОМакете.ИмяМакета, МакетИДанныеОбъекта.ЛокальныйКаталогФайловПечати);
	КонецЦикла;
	
КонецФункции

Процедура НапечататьОфисныйДокумент(ДокументСсылка, МакетИДанныеОбъекта, ИмяМакета, ЛокальныйКаталогФайловПечати)
	
	ТипМакета				= МакетИДанныеОбъекта.Макеты.ТипыМакетов[ИмяМакета];
	ДвоичныеДанныеМакетов	= МакетИДанныеОбъекта.Макеты.ДвоичныеДанныеМакетов;
	ОписаниеОбластей    	= МакетИДанныеОбъекта.Макеты.ОписаниеОбластей;
	ДанныеОбъекта = МакетИДанныеОбъекта.Данные[ДокументСсылка][ИмяМакета];
	Попытка
		Макет = ИнициализироватьМакет(ДвоичныеДанныеМакетов[ИмяМакета], ТипМакета, ЛокальныйКаталогФайловПечати, ИмяМакета);
		Если Макет = Неопределено Тогда
			Возврат;
		КонецЕсли;
		ПечатнаяФорма = ИнициализироватьПечатнуюФорму(ТипМакета, Макет.НастройкиСтраницыМакета, Макет);
		Если ПечатнаяФорма = Неопределено Тогда
			ОчиститьСсылки(Макет);
			Возврат;
		КонецЕсли;
		
		Для каждого КлючИЗначение Из ОписаниеОбластей[ИмяМакета] Цикл
			
			ОписаниеОбласти = КлючИЗначение.Значение;
			
			ПараметрыЗаполнения = ПолучитьДанныеПарамеровОбласти(ОписаниеОбласти.Параметры, ДанныеОбъекта);
			
			Область = ПолучитьОбласть(Макет, ОписаниеОбласти);
			
			Если ОписаниеОбласти.ТипОбласти = "Таблица" ИЛИ ОписаниеОбласти.ТипОбласти = "Список" Тогда
				ПрисоединитьИЗаполнитьКоллекцию(ПечатнаяФорма, Область, ПараметрыЗаполнения[ОписаниеОбласти.Таблица], Ложь);
			Иначе
				ПрисоединитьОбластьИЗаполнитьПараметры(ПечатнаяФорма, Область, ПараметрыЗаполнения, Ложь);
			КонецЕсли;
	...
	Исключение
		...
		ОчиститьСсылки(ПечатнаяФорма);
		ОчиститьСсылки(Макет);
		Возврат;
	КонецПопытки;
	
	ОчиститьСсылки(ПечатнаяФорма, Ложь);
	ОчиститьСсылки(Макет);
	
КонецПроцедуры

// Функция заполняет Соответствие параметров и данных объекта печати. 
//
// Параметры:
//
// Параметры - Структура - Содержит перечень параметров области.
// Данные    - Стуркутра - Содержит данные объекта печати.
//
// Возвращаемое значение:
//  Результат – Строка.
//
Функция ПолучитьДанныеПарамеровОбласти(знач Параметры,  знач Данные) Экспорт
	
	ПарметрыОбласти = Новый Соответствие;
	
	Для каждого КлючИЗначение Из Параметры Цикл
		
		ИмяПараметра = КлючИЗначение.Значение.ИмяПараметра; // Здесь обязательно должен содержаться текст
		Выражение    = КлючИЗначение.Значение.Выражение;
		Таблица      = КлючИЗначение.Значение.Таблица;
		...
		Попытка
			...
			Если ЗначениеЗаполнено(Таблица) Тогда
				...
				ТаблицаМассив =  Данные[Таблица];
				ПоискМассив   = ПарметрыОбласти.Получить(Таблица);
				Массив        = ?(ПоискМассив = Неопределено, Новый Массив, ПоискМассив);
				
				КоличествоСтрок = ТаблицаМассив.Количество() -1;
				
				Для Значение = 0 По КоличествоСтрок Цикл
					
					СтрокаТаблицыМассива = ТаблицаМассив.Получить(Значение);
					
					ДобавитьВМассив = Ложь;
					Попытка
					    СтрокаМассива = Массив.Получить(Значение);
					Исключение
						ДобавитьВМассив = Истина;
						СтрокаМассива = Новый Соответствие;
					КонецПопытки;
					
					Результат = СтрЗаменить(Выражение,"#Р", "СтрокаТаблицыМассива");
					Значение  = Вычислить(Результат);
					
					СтрокаМассива.Вставить(ИмяПараметра, Значение);
					
					Если ДобавитьВМассив Тогда
						Массив.Добавить(СтрокаМассива);
					КонецЕсли; 
					
				КонецЦикла;
				
				Если ПоискМассив = Неопределено Тогда
					ПарметрыОбласти.Вставить(Таблица, Массив);
				КонецЕсли; 
				
			Иначе 
				
				Результат = СтрЗаменить(Выражение,"#Р", "Данные");
				Значение  = Вычислить(Результат);
				
				ПарметрыОбласти.Вставить(ИмяПараметра, Значение);
...
Функция ИнициализироватьМакет(знач ДвоичныеДанныеМакета, знач ТипМакета, знач ПутьККаталогу = "", знач ИмяМакета = "") Экспорт
	...
	Если ВРег(ТипМакета) = "DOC" ИЛИ ВРег(ТипМакета) = "DOCX" ИЛИ ВРег(ТипМакета) = "RTF" Тогда
		Макет = бит_УправлениеПечатьюMSWordКлиент.ПолучитьМакетMSWord(ДвоичныеДанныеМакета, ИмяВременногоФайла);
		Макет.Вставить("Тип", ВРег(ТипМакета));
		Возврат Макет;
	КонецЕсли;
...
Функция ИнициализироватьПечатнуюФорму(знач ТипДокумента, знач НастройкиСтраницыМакета = Неопределено, Макет = Неопределено) Экспорт
	...
	Если ВРег(ТипДокумента) = "DOC" ИЛИ ВРег(ТипДокумента) = "DOCX" ИЛИ ВРег(ТипДокумента) = "RTF" Тогда
		Параметр = ?(Макет = Неопределено, НастройкиСтраницыМакета, Макет); // для обратной совместимости
		ПечатнаяФорма = бит_УправлениеПечатьюMSWordКлиент.ИнициализироватьПечатнуюФормуMSWord(Параметр);
		ПечатнаяФорма.Вставить("Тип", ВРег(ТипДокумента));
...
// Функции и процедцры получения областей из макета,.
// Вывода в печатную форму областей макета и заполнение параметров в них.

#Область РаботаСОбластямиИПараметрамиМакета
...
Функция ПолучитьОбласть(знач СсылкаМакет, знач ОписаниеОбласти) Экспорт
	...
	Если СсылкаМакет.Тип = "DOC" ИЛИ СсылкаМакет.Тип = "DOCX" ИЛИ СсылкаМакет.Тип = "RTF" Тогда
		
		Если		ОписаниеОбласти.ТипОбласти = "ВерхнийКолонтитул" Тогда
			Область = бит_УправлениеПечатьюMSWordКлиент.ПолучитьОбластьВерхнегоКолонтитула(СсылкаМакет);
		ИначеЕсли	ОписаниеОбласти.ТипОбласти = "НижнийКолонтитул" Тогда
			Область = бит_УправлениеПечатьюMSWordКлиент.ПолучитьОбластьНижнегоКолонтитула(СсылкаМакет);
		ИначеЕсли	ОписаниеОбласти.ТипОбласти = "Таблица" Тогда
			Область = бит_УправлениеПечатьюMSWordКлиент.ПолучитьОбластьМакетаMSWord(СсылкаМакет, ОписаниеОбласти.ИмяОбласти);
		ИначеЕсли	ОписаниеОбласти.ТипОбласти = "Реквизиты" Тогда
			Область = бит_УправлениеПечатьюMSWordКлиент.ПолучитьОбластьМакетаMSWord(СсылкаМакет, ОписаниеОбласти.ИмяОбласти, 1, 0);
...
Процедура ПрисоединитьОбласть(знач ПечатнаяФорма,
	...
	Попытка
		...
		Если ПечатнаяФорма.Тип = "DOC" ИЛИ ПечатнаяФорма.Тип = "DOCX" ИЛИ ПечатнаяФорма.Тип = "RTF" Тогда
			
			ВыведеннаяОбласть = Неопределено;
			
			Если		ОписаниеОбласти.ТипОбласти = "ВерхнийКолонтитул" Тогда
				бит_УправлениеПечатьюMSWordКлиент.ДобавитьВерхнийКолонтитул(ПечатнаяФорма, ОбластьМакета);
			ИначеЕсли	ОписаниеОбласти.ТипОбласти = "НижнийКолонтитул" Тогда
				бит_УправлениеПечатьюMSWordКлиент.ДобавитьНижнийКолонтитул(ПечатнаяФорма, ОбластьМакета);
			ИначеЕсли	ОписаниеОбласти.ТипОбласти = "Реквизиты" Тогда
				ВыведеннаяОбласть = бит_УправлениеПечатьюMSWordКлиент.ПрисоединитьОбласть(ПечатнаяФорма, ОбластьМакета, ПереходНаСледующуюСтроку);
			ИначеЕсли	ОписаниеОбласти.ТипОбласти = "Список" Тогда
				ВыведеннаяОбласть = бит_УправлениеПечатьюMSWordКлиент.ПрисоединитьОбласть(ПечатнаяФорма, ОбластьМакета, ПереходНаСледующуюСтроку);
			ИначеЕсли	ОписаниеОбласти.ТипОбласти = "Таблица" Тогда
				Если ПечатнаяФорма.ПоследняяВыведеннаяОбласть <> Неопределено
				   И ПечатнаяФорма.ПоследняяВыведеннаяОбласть.ТипОбласти = "Таблица"
				   И НЕ ПечатнаяФорма.ПоследняяВыведеннаяОбласть.ПереходНаСледующуюСтроку Тогда
					ВыведеннаяОбласть = бит_УправлениеПечатьюMSWordКлиент.ПрисоединитьОбласть(ПечатнаяФорма, ОбластьМакета, ПереходНаСледующуюСтроку, Истина);
				Иначе
					ВыведеннаяОбласть = бит_УправлениеПечатьюMSWordКлиент.ПрисоединитьОбласть(ПечатнаяФорма, ОбластьМакета, ПереходНаСледующуюСтроку);
				КонецЕсли;
			Иначе
...
Процедура ЗаполнитьПараметры(знач ПечатнаяФорма, знач Данные) Экспорт
	...
	Если ПечатнаяФорма.Тип = "DOC" ИЛИ ПечатнаяФорма.Тип = "DOCX" ИЛИ ПечатнаяФорма.Тип = "RTF" Тогда
		Если		ОписаниеОбласти.ТипОбласти = "ВерхнийКолонтитул" Тогда
			бит_УправлениеПечатьюMSWordКлиент.ЗаполнитьПараметрыВерхнегоКолонтитула(ПечатнаяФорма, Данные);
		ИначеЕсли	ОписаниеОбласти.ТипОбласти = "НижнийКолонтитул" Тогда
			бит_УправлениеПечатьюMSWordКлиент.ЗаполнитьПараметрыНижнегоКолонтитула(ПечатнаяФорма, Данные);
		ИначеЕсли	ОписаниеОбласти.ТипОбласти = "Реквизиты"
				ИЛИ ОписаниеОбласти.ТипОбласти = "СтрокаТаблицы"
				ИЛИ ОписаниеОбласти.ТипОбласти = "Список" Тогда
			бит_УправлениеПечатьюMSWordКлиент.ЗаполнитьПараметры(ПечатнаяФорма.ПоследняяВыведеннаяОбласть.Область, Данные);
		Иначе
			ВызватьИсключение(НСтр("ru = 'Тип области не указан или указан не корректно'"));
		КонецЕсли;
	КонецЕсли;
...
Процедура ПрисоединитьИЗаполнитьКоллекцию(знач ПечатнаяФорма,
	...
	Если ПечатнаяФорма.Тип = "DOC" ИЛИ ПечатнаяФорма.Тип = "DOCX" ИЛИ ПечатнаяФорма.Тип = "RTF" Тогда
		Если		ОписаниеОбласти.ТипОбласти = "Таблица" Тогда
			бит_УправлениеПечатьюMSWordКлиент.ПрисоединитьИЗаполнитьОбластьТаблицы(ПечатнаяФорма, ОбластьМакета, Данные, ПереходНаСледСтроку);
		ИначеЕсли	ОписаниеОбласти.ТипОбласти = "Список" Тогда
			бит_УправлениеПечатьюMSWordКлиент.ПрисоединитьИЗаполнитьНабор(ПечатнаяФорма, ОбластьМакета, Данные, ПереходНаСледСтроку);
		Иначе
			ВызватьИсключение(НСтр("ru = 'Тип области не указан или указан не корректно'"));
		КонецЕсли;
	КонецЕсли;
...
Процедура ВставитьРазрывНаНовуюСтроку(знач ПечатнаяФорма) Экспорт
	...
	Если  ПечатнаяФорма.Тип = "DOC" ИЛИ ПечатнаяФорма.Тип = "DOCX" ИЛИ ПечатнаяФорма.Тип = "RTF" Тогда
		бит_УправлениеПечатьюMSWordКлиент.ВставитьРазрывНаНовуюСтроку(ПечатнаяФорма);
	КонецЕсли;
...
Процедура ОчиститьСсылки(Handler, знач ЗакрытьПриложение = Истина) Экспорт
	...
	Если Handler <> Неопределено Тогда
		Если Handler.Тип = "DOC" ИЛИ Handler.Тип = "DOCX" ИЛИ Handler.Тип = "RTF" Тогда
			бит_УправлениеПечатьюMSWordКлиент.ЗакрытьСоединение(Handler, ЗакрытьПриложение);
		КонецЕсли;
...
#Область ОтображениеПачатнойФормы
...
Процедура ПоказатьДокумент(знач Handler) Экспорт
	...
	Если Handler.Тип = "DOC" ИЛИ Handler.Тип = "DOCX" ИЛИ Handler.Тип = "RTF" Тогда
		бит_УправлениеПечатьюMSWordКлиент.ПоказатьДокументMSWord(Handler);
	КонецЕсли;
