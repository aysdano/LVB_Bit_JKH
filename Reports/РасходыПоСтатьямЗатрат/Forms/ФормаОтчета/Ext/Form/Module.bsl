...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	УстановитьЗаголовкиПолейГруппировок();
...
Процедура ПриЗагрузкеПользовательскихНастроекНаСервере(Настройки, ИспользуютсяСтандартныеНастройки)
	...
	Если ДопСвойства.Свойство("Счет") Тогда
		Отчет.Счет = ДопСвойства.Счет;
	КонецЕсли;
...
&НаСервере
Процедура УстановитьЗаголовкиПолейГруппировок()
	...
	Если Не Отчет.РежимРасшифровки Тогда
		...
		Если ПолеГруппировки1 <> Неопределено И ПолеГруппировки2 <> Неопределено Тогда
			...
			Если Отчет.Счет = ПланыСчетов.Хозрасчетный.Продажи_РасходыНаПродажу
				Или Отчет.Счет = ПланыСчетов.Хозрасчетный.Продажи_УправленческиеРасходы Тогда
			...
			ИначеЕсли Отчет.Счет = ПланыСчетов.Хозрасчетный.ПрочиеРасходы Тогда
			
				ПолеГруппировки1.Заголовок = НСтр("ru='Вид прочих доходов и расходов'");
				ПолеГруппировки2.Заголовок = НСтр("ru='Кор. аналитика'");
				
			ИначеЕсли Отчет.Счет = ПланыСчетов.Хозрасчетный.СебестоимостьПродаж Тогда
