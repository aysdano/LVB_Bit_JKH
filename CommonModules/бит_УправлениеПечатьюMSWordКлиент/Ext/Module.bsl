...
// Создает COM соединение с COM объектом Word.Application, создает в нем единственный документ.
//
...
//                              Следует символ перевода строки, который не нужно включать в получаемую область.
//  СмещениеОкончание - Число - переопределяет границу окончания области для случаев, когда область заканчивается не.
//   Перед операторной скобкой, а на один или несколько символов раньше. Значение должно быть отрицательным.
//   Значение по умолчанию:-1 - ожидается, что перед операторной скобкой закрытия области.
//   Есть символ перевода строки, который не нужно включать в получаемую область.
//
...
#Область ФункцииДляРаботыСКолонтитуламиДокументаMSWord
...
Процедура ПрисоединитьИЗаполнитьНабор(знач ПечатнаяФорма,
	...
	Если ДанныеОбъекта <> Неопределено Тогда
		Для Каждого ДанныеСтроки Из ДанныеОбъекта Цикл
			...
			Если ТипЗнч(ДанныеСтроки) = Тип("Соответствие") Тогда
...
Процедура ПрисоединитьИЗаполнитьОбластьТаблицы(знач ПечатнаяФорма,
	...
	Если ТипЗнч(ДанныеОбъекта[0]) = Тип("Соответствие") Тогда
	...
	Для Каждого ДанныеСтрокиТаблицы Из ДанныеОбъекта Цикл
		...
		Если ТипЗнч(ДанныеСтрокиТаблицы) = Тип("Соответствие") Тогда
