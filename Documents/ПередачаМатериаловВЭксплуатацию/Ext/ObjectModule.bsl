...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если Дата >= УчетМатериаловВЭксплуатации.ДатаОтменыСпецодежды() Тогда
		Для Каждого СтрокаСпецодежда Из Спецодежда Цикл 
			Если ЗначениеЗаполнено(СтрокаСпецодежда.НазначениеИспользования) Тогда
				СтрокаСпецодежда.НазначениеИспользования = Справочники.НазначенияИспользования.ПустаяСсылка();
			КонецЕсли;
		КонецЦикла;
		Для Каждого СтрокаСпецоснастка Из Спецоснастка Цикл 
			Если ЗначениеЗаполнено(СтрокаСпецоснастка.НазначениеИспользования) Тогда
				СтрокаСпецоснастка.НазначениеИспользования = Справочники.НазначенияИспользования.ПустаяСсылка();
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
