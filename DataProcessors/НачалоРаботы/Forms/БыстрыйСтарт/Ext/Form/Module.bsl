...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не НужноПодключатьИнтернетПоддержку Тогда
		
		ПодключенаИнтернетПоддержка = Истина;
		ПодключенСервис1СКонтрагент = Истина;
		
		Если ОрганизацияСуществует Тогда
			ПерейтиНаСтраницу(ЭтотОбъект, СледующаяСтраница(ЭтотОбъект, АктивнаяСтраница));
		Иначе
			ПерейтиНаСтраницу(ЭтотОбъект, СледующаяСтраница(ЭтотОбъект, АктивнаяСтраница));
		КонецЕсли;
	Иначе
...
Процедура ПродолжитьСИНН(Команда)
	...
	СледующаяСтраница = СледующаяСтраница(ЭтотОбъект, АктивнаяСтраница);
	Если СледующаяСтраница = "СтраницаВидБизнеса" Тогда
		ПерейтиНаСтраницуВидБизнеса();
	Иначе
		ПерейтиНаСтраницу(ЭтотОбъект, СледующаяСтраница);
	КонецЕсли;
...
Процедура Отмена(Команда)
	...
	ПерейтиНаСтраницу(ЭтотОбъект, НавигацияФормы[0]);
...
&НаКлиентеНаСервереБезКонтекста
Функция СледующаяСтраница(Форма, ТекущаяСтраница)
	
	НавигацияФормы = НавигацияФормы(Форма);
	
	Если ПустаяСтрока(Форма.АктивнаяСтраница) Тогда
		Возврат НавигацияФормы[0];
	КонецЕсли;
	
	ИндексТекущейСтраницы = НавигацияФормы.Найти(Форма.АктивнаяСтраница);
	...
	ИначеЕсли ИндексТекущейСтраницы = НавигацияФормы.ВГраница() Тогда
		Возврат "";
	Иначе
		Возврат НавигацияФормы[0];
	КонецЕсли;
		...
		Иначе
			...
			Если ЗапрещеноИзменятьВидОрганизации Тогда
				...
				РезультатПроверки = ИдентификационныеНомераНалогоплательщиков.ПроверитьСоответствиеТребованиямИНН(
					ИНН, ВидОрганизации = "ЮридическоеЛицо");
			Иначе
				// Вид организации определяется по ИНН
				РезультатПроверки = 
					ИдентификационныеНомераНалогоплательщиков.ПроверитьСоответствиеТребованиямИНН(ИНН, ЭтоЮрЛицо(ИНН));
			КонецЕсли;
...
Функция ПолучитьРеквизитыПоИНН(ИНН, ЭтоЮрЛицо)
	...
	Если ЭтоЮрЛицо Тогда
		...
		Если ЗначениеЗаполнено(СведенияОКомпании.ЕГРЮЛ) Тогда
		...
		Иначе
			...
			Результат.Вставить("ОписаниеОшибки", СтрШаблон(ШаблонОписанияОшибки, ИНН));
		КонецЕсли;
	...
	Если Результат.Свойство("ОткрытыеГосударственныеДанныеФНС") Тогда
		Результат.Удалить("ОткрытыеГосударственныеДанныеФНС");
	КонецЕсли;
	
	// Удаляем свойства типа "ТаблицаЗначений" перед передачей на клиент.
	Если Результат.Свойство("Руководители") Тогда
		Результат.Удалить("Руководители");
	КонецЕсли;
...
&НаКлиенте
Процедура ПерейтиНаСтраницуВидБизнеса()
