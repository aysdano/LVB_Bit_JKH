...
Процедура Справка(Команда)
	...
	ФайловаяСистемаКлиент.ОткрытьНавигационнуюСсылку("https://www.youtube.com/channel/UCfIp3isFhmeEuTlQa6ZzvhA");
...
Процедура ОбновитьДеревоДокументовИФайловВсеНаСервере(ПараметрыОтбора)
	...
	|
	|УПОРЯДОЧИТЬ ПО
	|	СвязанныеОбъектыРаспознаниеДокументов.РаспознанныйДокумент.Дата УБЫВ,
...
Процедура ДеревоДокументыИФайлыПеретаскивание(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, Строка, Поле)
	...
	ПараметрыОтбораСозданныхДокументов1С = ПолучитьПараметрыОтбораСозданныхДокументов1С();
	ДанныеДляПрикрепленияФайла = Новый Структура("МассивСсылок, ИменаФайлов, КудаДобавляемСсылка, ПараметрыОтбораСозданныхДокументов1С",
		МассивСсылок, ИменаФайлов, СтрокаДерева.Ссылка, ПараметрыОтбораСозданныхДокументов1С);
