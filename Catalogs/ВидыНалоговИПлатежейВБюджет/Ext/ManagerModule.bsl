...
// Устанавливает наименование вида налога для НДФЛ ИП, рассчитанного с превышения предельной величины налоговой базы
Процедура ЗаполнитьНаименованиеНДФЛ_ИП_СПревышения() Экспорт
	
	ВидНалога = Справочники.ВидыНалоговИПлатежейВБюджет.НалогПоВиду(
	...
	Если ЗначениеЗаполнено(ВидНалога) Тогда
		...
		Попытка
			...
			ВидНалогаОбъект.Наименование =
				НСтр("ru = 'НДФЛ индивидуального предпринимателя свыше предельной величины'");
			ВидНалогаОбъект.Записать();
		Исключение
