...
Процедура ЗаполнитьГоловнуюОрганизацию(ПараметрыОбновления = Неопределено) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		СтарыйНабор = РегистрыСведений.СведенияОбЭЛН.НачатьЗаписьНабора(Выборка.НомерЛН, ПустаяОрганизация, Ложь);
		...
		НовыйНабор = РегистрыСведений.СведенияОбЭЛН.НачатьЗаписьНабора(Выборка.НомерЛН, ТребуемаяГоловнаяОрганизация, Истина);
		...
		РегистрыСведений.СведенияОбЭЛН.ЗавершитьЗаписьНабора(СтарыйНабор);
		РегистрыСведений.СведенияОбЭЛН.ЗавершитьЗаписьНабора(НовыйНабор);
...
Функция ДоступенИсходныйXML(НомерЛН, ГоловнаяОрганизация) Экспорт
	...
	ЗначенияРесурсов = Получить(Фильтр);
	Возврат ЗначенияРесурсов.ДоступенИсходныйXML;
КонецФункции
...
Процедура ПриЗаписиБольничного(БольничныйОбъект) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ПодобратьДругойБольничный Тогда
			ЗначенияРеквизитов = БолееПодходящийБольничный(
				БольничныйОбъект,
...
Функция БолееПодходящийБольничный(БольничныйОбъект, НомерЛН, Кэш)
	ЗначениеИзКэша = Кэш[НомерЛН];
	...
	Если ЗначенияРеквизитов = Неопределено
		Или ЗначенияРеквизитов.НомерЛисткаНетрудоспособности <> НомерЛН
		Или ЗначенияРеквизитов.ПометкаУдаления Тогда
		// Не найден подходящий больничный.
		НоваяСсылка = Документы.БольничныйЛист.ПоследнийБольничныйПоНомеру(НомерЛН, БольничныйОбъект.Ссылка);
		Если НоваяСсылка <> Неопределено Тогда
	...
	Кэш.Вставить(НомерЛН, ЗначенияРеквизитов);
