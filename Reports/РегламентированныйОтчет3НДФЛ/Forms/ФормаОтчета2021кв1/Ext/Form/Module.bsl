...
Процедура СобратьДанныеВРаздел1Сервер()
	...
	Для Индекс = 0 По ПорядокРаспределенияПоКБК.ВГраница() Цикл
		...
		Для Каждого ДанныеРаздела2 Из ИтогиРаздела2 Цикл
			...
			Для Каждого СтрокаДохода Из НайденныеСтроки Цикл
				...
				МожноУменьшитьВычет = Макс(0, Мин(СтрокаДохода.СуммаДохода, ДанныеРаздела2.НалоговыйВычет));
				...
				Если СтрокаДохода.КБК = КБКИП Тогда
					НалоговаяБаза = НалоговаяБаза - СуммаДоходовНеПодлежащаяНалогообложению(
						СтрокаДохода.ВидДохода,
						СтрокаДохода.Ставка);
				КонецЕсли;
				
				// Считаем налог
				НалогИсчисленный = Окр(НалоговаяБаза * СтрокаДохода.Ставка / 100, 0);
	...
	Если ЗаполнятьПункт2 Тогда
		...
		НалогЗаГод = ИтогоНачисленоНалогаПоПредпринимательскойДеятельности
			 - СтруктураДанныхПриложение3Расчет.П002000001052;
...
Функция ИтогиРаздела2()
	...
	СводДанныхРаздела2.Колонки.Добавить("НалоговыйВычет", ТипЧисло);
	СводДанныхРаздела2.Колонки.Добавить("РасходыУменьшающиеНалог", ТипЧисло);
	СводДанныхРаздела2.Колонки.Добавить("ТорговыйСбор", ТипЧисло);
	СводДанныхРаздела2.Колонки.Добавить("АвансыПоПатенту", ТипЧисло);
	СводДанныхРаздела2.Колонки.Добавить("ПлатежиПоПатентуИП", ТипЧисло);
	...
	СводДанныхРаздела2.Свернуть("ВидДохода, Ставка",
		"НалоговыйВычет, РасходыУменьшающиеНалог, ТорговыйСбор, АвансыПоПатенту, ПлатежиПоПатентуИП");
...
&НаСервере
Функция СуммаДоходовНеПодлежащаяНалогообложению(ВидДохода, Ставка)
	
	Отбор = Новый Структура;
	Отбор.Вставить("ВидДохода", ВидДохода);
	Отбор.Вставить("Ставка", Ставка);
	Отбор.Вставить("ВидВычета", "СуммаДоходовНеПодлежащаяНалогообложению");
...
Процедура ПроверитьСтраницуПриложенияКРазделу1(ДанныеСтраницы, НомерСтраницы)
	...
	Для Индекс = 0 По мДанныеОтчета.Раздел1.Количество() - 1 Цикл
		...
		Пока ДанныеРаздела1.Свойство(СтрШаблон("П000100001030_%1", НомерСтроки), ЗначениеОКТМО) Цикл
			Если ЗначениеЗаполнено(ЗначениеОКТМО) Тогда
				МассивОКТМОРаздела1.Добавить(ЗначениеОКТМО);
			КонецЕсли;
			НомерСтроки = НомерСтроки + 1;
		КонецЦикла;
