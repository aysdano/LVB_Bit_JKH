...
Процедура ЗаполнитьСведенияОСтрахователеВМакете_2021(Макет, Документ)
	...
	ЗаполнитьВМакетеНаименованиеСтрахователя(Макет, Документ);
...
Процедура ЗаполнитьСведенияОЗастрахованномЛицеВМакете_2021(Макет, Документ)
	...
	ЗаполнитьВМакетеФИОЗаявителя(Макет, Сведения);
	ЗаполнитьВМакетеФИОПолучателя(Макет, Сведения);
	...
	ЗаполнитьВМакетеУдостоверениеЛичности_2017(Макет, Документ);
	
	ЗаполнитьВМакетеРазрешениеНаПроживание(Макет, Документ);
...
Функция СведенияОЗастрахованномЛицеДляФСС(Документ) Экспорт
	...
	Результат.Вставить("ФамилияПолучателя",  Документ.ФамилияПолучателя);
	Результат.Вставить("ИмяПолучателя",      Документ.ИмяПолучателя);
	Результат.Вставить("ОтчествоПолучателя", Документ.ОтчествоПолучателя);
	...
	ИначеЕсли Документ.СпособВыплатыПособия = Перечисления.СпособыВыплатыПособия.ПочтовымПереводом Тогда
		Результат.АдресПроживания = ИндексИПредставлениеАдреса(Документ.АдресПолучателяФактический);
	КонецЕсли;
	
	Результат.Вставить("АдресРегистрации",  ИндексИПредставлениеАдреса(Документ.АдресПолучателяРегистрации));
...
Функция ПредставлениеТелефона(ЗначениеТелефона, ОграничениеДлины) Экспорт
	Возврат УправлениеКонтактнойИнформациейЗарплатаКадры.ПредставлениеТелефона(ЗначениеТелефона, ОграничениеДлины, Истина);
КонецФункции
...
Процедура ЗаполнитьВМакетеСведенияОбАдресе_2021(Макет, ИндексИПредставлениеАдреса, Префикс) Экспорт
	Если ИндексИПредставлениеАдреса = Неопределено Тогда
		Возврат;
	КонецЕсли;
	ЗарплатаКадры.ВывестиДанныеПоБуквенно(ИндексИПредставлениеАдреса.Индекс, Макет, Префикс + "_Индекс_", 6);
	Макет.Параметры[Префикс + "_Представление"] = ИндексИПредставлениеАдреса.Представление;
КонецПроцедуры

Функция ИндексИПредставлениеАдреса(ЗначениеАдреса)
...
Функция ПредставлениеАдресаБезИндекса(СтруктураАдреса)
...
Процедура ЗаполнитьВМакетеНаименованиеСтрахователя(Макет, Заявление)
	...
	ДлиныСтрок = Новый Массив();
	ДлиныСтрок.Добавить(27);
	НаименованиеОрганизации = ЗарплатаКадры.РазбитьСтрокуНаПодСтроки(Заявление.НаименованиеОрганизации, ДлиныСтрок);
...
Функция ФИОЗаявителяДляПечатиПоБуквам(Документ)
	...
	Если ВывестиПоОтдельности Тогда
		Добивка = "                           ";
		Возврат Лев(Фамилия + Добивка, 27) + Лев(Имя + Добивка, 27) + Лев(Отчество + Добивка, 27)
	КонецЕсли;
	
	ДлиныСтрок = Новый Массив();
	ДлиныСтрок.Добавить(27);
	Возврат Лев(ЗарплатаКадры.РазбитьСтрокуНаПодСтроки(ФИОВРодительномПадеже, ДлиныСтрок), 81);
КонецФункции
...
Процедура ЗаполнитьВМакетеРазрешениеНаПроживание(Макет, Заявление) Экспорт
...
Функция СтруктураАдресаВМашиночитаемомФормате_2012(Адрес)
	ИменаПолей = "Индекс, Регион, Район, НаселенныйПункт, Улица, Дом, Корпуса, Квартира";
	СтруктураАдреса = ПрямыеВыплатыПособийСоциальногоСтрахования.СтруктураАдреса(Адрес, ИменаПолей);
	...
	КонецЕсли;
	// Заполнение полей "Корпус" и "Строение".
	Корпуса = ПрямыеВыплатыПособийСоциальногоСтрахования.СтруктураКорпусов(СтруктураАдреса.Корпуса);
	СтруктураАдреса.Вставить("Корпус",   Корпуса.Корпус);
	СтруктураАдреса.Вставить("Строение", Корпуса.Строение);
	// Строение и сооружение - одно и то же, одновременно их быть не может.
	Если ЗначениеЗаполнено(Корпуса.Сооружение) Тогда
		СтруктураАдреса.Строение = СтруктураАдреса.Строение + Корпуса.Сооружение;
	КонецЕсли;
	// Участок по возможности записываем в менее заполненное поле, по возможности через разделитель.
	Если ЗначениеЗаполнено(Корпуса.Участок) Тогда
		Если СтруктураАдреса.Строение = "" Тогда
			СтруктураАдреса.Строение = Корпуса.Участок;
		ИначеЕсли СтруктураАдреса.Корпус = "" Тогда
			СтруктураАдреса.Корпус = Корпуса.Участок;
		ИначеЕсли СтрДлина(СтруктураАдреса.Строение) < СтрДлина(СтруктураАдреса.Корпус) Тогда
			СтруктураАдреса.Строение = СтруктураАдреса.Строение + "/" + Корпуса.Участок;
		Иначе
			СтруктураАдреса.Корпус   = СтруктураАдреса.Корпус   + "/" + Корпуса.Участок;
		КонецЕсли;
	КонецЕсли;
