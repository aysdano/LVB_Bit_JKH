...
Функция ДвоичныеДанныеФайла(Знач ПрисоединенныйФайл, Знач ВызыватьИсключение = Истина) Экспорт
	...
	Если ФайлОбъект.ТипХраненияФайла = Перечисления.ТипыХраненияФайлов.ВИнформационнойБазе Тогда
		...
		// БИТ_ЖКХ 08.09.2017 14:32:42 Начало правки...
		бит_Результат = бит_ХранилищеФайлаИзИнформационнойБазы(ФайлОбъект.Ссылка);
		Если бит_Результат <> Неопределено Тогда
			Возврат бит_Результат.Получить();
		КонецЕсли;
		// БИТ_ЖКХ 08.09.2017 14:32:42 Конец правки.	   
...
// БИТ_ЖКХ 08.09.2017 12:00:13 Начало правки... 
// Возвращает двоичные данные файла из информационной базы.
//
// Параметры:
//   ФайлСсылка - ссылка на файл или его версию.
...
//   ХранилищеЗначения - двоичные данные файла.
//
Функция бит_ХранилищеФайлаИзИнформационнойБазы(ФайлСсылка) Экспорт
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ПрисоединенныеФайлы.ПрисоединенныйФайл КАК Файл,
	|	ПрисоединенныеФайлы.ХранимыйФайл КАК ДвоичныеДанныеФайла
	|ИЗ
	|	РегистрСведений.бит_ПрисоединенныеФайлы КАК ПрисоединенныеФайлы
	|ГДЕ
	|	ПрисоединенныеФайлы.ПрисоединенныйФайл = &ФайлСсылка";
