...
Процедура ДополнитьСтруктуруНавигацииПомощникаРегистрации() Экспорт
	...
	Если ТекущийНомерШагаДокументы < НомерШагаДокументы Тогда
		СтруктураНавигацииПомощника.Вставить(ИмяШагаДокументы, СтруктураШагаДокументы(НомерШагаДокументы));
		СтруктураНавигацииПомощника[ИмяШагаДокументыТекущее] = СтруктураШагаУстав(НомерШагаДокументы - 1);
	Иначе
	...
	Если Не ПустаяСтрока(ОписаниеДляЛичногоКабинета) Тогда
...
Процедура СформироватьДокументыОООВФоне(ПараметрыФормированияДокументов, АдресРезультата = Неопределено) Экспорт
	...
	Попытка
		...
		ПараметрыЗаявленияОРегистрацииООО.ИмяФормы = ФормаЗаявленияПоУмолчанию(
...
Процедура СформироватьДокументыИПВФоне(ПараметрыФормированияДокументов, АдресРезультата = Неопределено) Экспорт
	...
	Попытка
		...
		ИмяФормы = ФормаЗаявленияПоУмолчанию(Период, "РегламентированноеУведомлениеФормаР21001");
...
Функция КлючиПоказателейУведомленияРегистрацияЮЛ_2020(ИмяФормы)
	...
	Данные_ЛистЕ = Новый Структура;
	Данные_ЛистЕ.Вставить("Е01010100", "ОГРН");
	Данные_ЛистЕ.Вставить("Е01010200", "ИНН");
...
Процедура ЗаполнитьЗаявлениеОПостановкеНаУчетООО(ПараметрыОтчета, Контейнер) Экспорт
	...
	Для Каждого ТекущаяСтрока Из УчредителиЮридическиеЛица Цикл
		...
		НовыйУчредитель.ПолноеНаименование = ТекущаяСтрока.СведенияОЮридическомЛице.ПолноеНаименование;
	...
	Для Каждого СтрокаТаблицы Из ДанныеУчредителей.Учредители Цикл
		...
		НовыйЗаявитель.Вставить("ОграничитьДоступКСведениям_ЮЛ", "");
		
		// Реквизиты юридического лица
		Если СтрокаТаблицы.ЭтоЮридическоеЛицо И КоличествоУчредителей > 1 Тогда
			// В случае заявления заполняется в отношении единственного учредителя - юридического лица
			...
			НовыйЗаявитель.ПолноеНаименование = СтрокаТаблицы.СведенияОЮридическомЛице.ПолноеНаименование;
...
Функция НовыйПараметрыИнструкцииРегистрацииООО() Экспорт
	...
	Результат.Вставить("ВыборСпецрежимаНалогообложения", Новый Структура("УСН", Ложь));
...
Процедура ДополнитьИнформациейОПредставителяхУчредителей(ИнформацияОСвязях)
	...
	Для Каждого Учредитель Из ИнформацияОСвязях.Учредители Цикл
		...
		Иначе
			ДанныеУчредителя = Новый Структура;
			ШаблонОписанияОшибки = НСтр("ru = 'Не удалось найти данные для заполнения реквизитов по ИНН %1.'");
			ДанныеУчредителя.Вставить("ОписаниеОшибки", СтрШаблон(ШаблонОписанияОшибки, Учредитель.ИНН));
		КонецЕсли;
		...
		Для Каждого Учредитель Из Учредители Цикл
			...
			Если Учредитель.ТипУчредителя = Перечисления.ЮридическоеФизическоеЛицо.ЮридическоеЛицо Тогда
				Если Не ПустаяСтрока(Учредитель.ИНН) Тогда
					...
					Иначе
						ДанныеЗаполненияПоИНН = Новый Структура;
						ШаблонОписанияОшибки = НСтр("ru = 'Не удалось найти данные для заполнения реквизитов по ИНН %1.'");
						ДанныеЗаполненияПоИНН.Вставить("ОписаниеОшибки", СтрШаблон(ШаблонОписанияОшибки, Учредитель.ИНН));
					КонецЕсли;
...
Функция ПричинаПодачиЗаявленияИзмененийВЕГРЮЛ(ЭтоЗаявлениеР13014, ИзмененУстав, ИзмененияВносятсяВУчредительныеДокументы, НомерТиповогоУстава)
	...
	Если ИзмененУстав И ЗначениеЗаполнено(НомерТиповогоУстава) Тогда
		Результат = "3"; // в связи с изменением сведений о юридическом лице, в учредительных документах или в уставе
	ИначеЕсли ИзмененияВносятсяВУчредительныеДокументы Тогда
...
Процедура ЗаполнитьЗаявлениеРегистрацииИзмененийЕГРЮЛ(ПараметрыОтчета, Контейнер) Экспорт
	...
	ИначеЕсли Изменено.ПолноеНаименование Тогда
	...
	Если ПараметрыЗаполнения.ИзмененияВносятсяВУчредительныеДокументы И Не Изменено.Устав Тогда
	...
	Если Изменено.ЮридическийАдрес Тогда
		...
		Если ЭтоЗаявлениеР13014 Тогда
			ДобавитьСведенияОбАдресе(СведенияОбИзменениях, АдресСтруктурой, КлючиПоказателей.ЛистБ, Истина);
		Иначе
	...
	Если ВнестиИзмененияВУчредителей Тогда
		...
		Для Каждого СтрокаТаблицы Из УчастникиЮридическиеЛица Цикл
			...
			ИначеЕсли СтрокаТаблицы.СуммаВзноса = 0 Тогда
				...
				НовыйЛист.ПолноеНаименованиеТекущее = СведенияОЮридическомЛице.ПолноеНаименование;
	...
	Если ВнестиИзмененияВУчредителей Тогда
		...
		Для Каждого СтрокаТаблицы Из УчастникиФизическиеЛица Цикл
			...
			ИначеЕсли СтрокаТаблицы.СуммаВзносаДоИзменения <> СтрокаТаблицы.СуммаВзноса Тогда
				...
				НовыйЛист.ОтчествоТекущее = СведенияОФизическомЛице.Отчество;
				НовыйЛист.ИННТекущий = СведенияОФизическомЛице.ИНН;
				НовыйЛист.СуммаВзноса = СтрокаТаблицы.СуммаВзноса;
				НовыйЛист.ДоляУставногоКапитала = СтрокаТаблицы.ДоляУставногоКапитала;
			КонецЕсли;
...
Функция ИнструкцияРегистрацияИзмененийЕГРЮЛсУведомлением(ДанныеПомощникаЗаполнения, ДополнительныеПараметры)
	...
	ПараметрыИнструкции.ИспользуетсяИндивидуальныйУстав = ДанныеПомощникаЗаполнения.Изменено.Устав
		И Не ЗначениеЗаполнено(ДанныеПомощникаЗаполнения.НомерТиповогоУстава);
...
Функция ИнструкцияРегистрацияИзмененийЕГРЮЛбезУведомления(ДанныеПомощникаЗаполнения, ДополнительныеПараметры)
	...
	ПараметрыИнструкции.ЕстьПлатежныеРеквизиты = ЗначениеЗаполнено(ПлатежныеРеквизитыОплатаГоспошлины);
	ПараметрыИнструкции.ЭтоЗаявлениеР13014 = ЭтоЗаявлениеР13014(ДанныеПомощникаЗаполнения.ДатаСоставления);
	ПараметрыИнструкции.ИспользуетсяИндивидуальныйУстав =
		ДанныеПомощникаЗаполнения.Изменено.Устав И Не ЗначениеЗаполнено(ДанныеПомощникаЗаполнения.НомерТиповогоУстава)
		Или Не ДанныеПомощникаЗаполнения.Изменено.Устав И ДанныеПомощникаЗаполнения.ИзмененияВносятсяВУчредительныеДокументы;
