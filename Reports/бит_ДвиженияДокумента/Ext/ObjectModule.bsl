...
Перем ТонкаяСплошнаяЛиния; // Переменная хранит тип тонкой сплошной линии ячейки табличного документа.

Перем ТолстаяСплошнаяЛиния; // Переменная хранит тип толстой сплошной линии ячейки табличного документа.

Перем мНастройкиИзмерений; // Хранит настройки дополнительных измерений.
...
Функция ОпределитьВозможностьПолученияОтчета()
	...
	Если Не ЗначениеЗаполнено(Документ) Тогда
		...
		бит_ОбщегоНазначенияКлиентСервер.ВывестиСообщение("Запишите и проведите документ, по которому Вы фомируете отчет");
...
Процедура СформироватьОтчет(ДокументРезультат) Экспорт
	...
	// БИТ_ЖКХ изменение кода. Начало. 14.01.2010{{ 
	ТаблицаДвижений = бит_ОпределитьНаличиеДвиженийПоРегистратору(Документ);
	// БИТ_ЖКХ изменение кода. Конец. 14.01.2010}} 
	...
	Для Каждого СвойстваОбъекта Из ДвиженияДокумента Цикл
		...
		Если (ВидРегистра = "Сведений") 
			...
			// БИТ_ЖКХ добавление кода. Начало. 02.08.2010{{
			...
			КонецЕсли;
			// БИТ_ЖКХ добавление кода. Конец. 02.08.2010}} 
...
Функция бит_ОпределитьНаличиеДвиженийПоРегистратору(ДокументСсылка) Экспорт
	...
	Для Каждого Движение ИЗ МетаданнныеДокумента.Движения Цикл
		...
		// В запросе получаем имена регистров, по которым есть хотя бы одно движение например.
		
		// БИТ_ЖКХ 07.04.2015{{ 
		// Вместо ДЛЯ ИЗМЕНЕНИЯ добавлена управляемая блокировка.
		НачатьТранзакцию();
		
		Блокировка = Новый БлокировкаДанных;
		ЭлементБлокировки = Блокировка.Добавить(Движение.ПолноеИмя());
...
Процедура бит_ДобавитьПолеВСписокПолей(МетаданныеРесурса, СписокПолей, СтруктураПредставлениеПолей)
	...
	СтруктураИзмерения = Неопределено;//мНастройкиИзмерений.Получить(ИмяРесурса);
	ДобавитьПоле	   = Истина;
...
// БИТ_ЖКХ добавление кода. Начало. 14.01.2010{{
мНастройкиИзмерений = Неопределено; // Настройки измерений.
// БИТ_ЖКХ добавление кода. Конец. 14.01.2010}} 
