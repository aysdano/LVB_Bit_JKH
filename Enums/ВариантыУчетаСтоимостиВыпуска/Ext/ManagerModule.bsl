...
// Инициализирует коллекцию для глобального механизма настройки субконто - см. ОбщегоНазначенияБП.ПолучитьДействияИзмененияСубконто
//
// Параметры:
//  ПараметрыУчета - Структура - см. ОбщегоНазначенияБП.ОпределитьПараметрыУчета
//
Процедура УстановитьПараметрУчетаПродукция(ПараметрыУчета) Экспорт
	
	ОписаниеСубконтоНаСчете = ПланыСчетов.Хозрасчетный.ВыпускПродукции.ВидыСубконто.Найти(
		ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.Продукция);
	Значение = (ОписаниеСубконтоНаСчете <> Неопределено);
	
	ПараметрыУчета.Вставить(ГлобальноеИмяРазрезаАналитикиПродукция(), Значение);
	
КонецПроцедуры

// Устанавливает необходимость использования субконто для глобального механизма настройки субконто -
// см. НастройкиУчета.ВключитьРасширеннуюАналитику
//
// Параметры:
//  РазрезыАналитики - Массив - имена потребных разрезов аналитики, как указано в УстановитьПараметрУчетаПродукция
//  ВариантУчета - ПеречислениеСсылка.ВариантыУчетаСтоимостиВыпуска - применяемый вариант учета
//
Процедура УстановитьРазрезАналитикиПродукция(РазрезыАналитики, ВариантУчета) Экспорт
	
	Если Не ИспользоватьСубконтоПродукция(ВариантУчета) Тогда
		Возврат;
	КонецЕсли;
	
	РазрезыАналитики.Добавить(ГлобальноеИмяРазрезаАналитикиПродукция());
	
КонецПроцедуры

// Содержит описание разрезов учета для глобального механизма настройки субконто.
//
// Параметры - см. ОбщегоНазначенияБП.ПолучитьСоответствиеСубконтоПараметрамУчета
//
Процедура ДобавитьОписаниеРазрезовУчета(ОписаниеРазрезовУчета, ШаблонОписанияСчетов, ШаблонОписанияВидовСубконто, ШаблонОписанияПараметров, ШаблонОписанияИсключений) Экспорт
	...
	ОписаниеРазрезаУчета.Счета = ШаблонОписанияСчетов.СкопироватьКолонки();
	...
	ОписаниеСчета = ОписаниеРазрезаУчета.Счета.Добавить();
	ОписаниеСчета.Счет = ПланыСчетов.Хозрасчетный.ВыпускПродукции;
	
	ОписаниеСубконто = ОписаниеРазрезаУчета.Субконто.Добавить();
	ОписаниеСубконто.Вид            = ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.Продукция;
	ОписаниеСубконто.Суммовой       = Истина;
	ОписаниеСубконто.ТолькоОбороты  = Истина;
	ОписаниеСубконто.Параметр       = ГлобальноеИмяРазрезаАналитикиПродукция();
	
	ОписаниеПараметра = ОписаниеРазрезаУчета.Параметры.Добавить();
	ОписаниеПараметра.Имя        = ГлобальноеИмяРазрезаАналитикиПродукция();
	ОписаниеПараметра.Исключения = ШаблонОписанияИсключений.Скопировать();
...
Процедура УстановитьТранзитныйАлгоритмУчетаОтклоненийОтПлановойСебестоимости() Экспорт
	...
	|ГДЕ
	|	УчетнаяПолитика.СпособУчетаВыпускаГотовойПродукции = ЗНАЧЕНИЕ(Перечисление.ВариантыУчетаСтоимостиВыпуска.ОтклоненияСтоимости)";
...
Функция ГлобальноеИмяРазрезаАналитикиПродукция()
	Возврат "УчитыватьВыпускПродукцииПоПродукции";
КонецФункции
