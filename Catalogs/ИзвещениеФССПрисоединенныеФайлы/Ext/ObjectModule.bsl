		...
		Если Идентификатор = РеквизитыВладельца.ИдентификаторСообщения Тогда
			...
			Попытка
				...
				НайденоФизическоеЛицо = (
					...
					И РеквизитыВладельца.ФизическоеЛицо <> РезультатАнализа.ФизическиеЛица[0]);
...
Процедура ПриПоискеФИО(Результат, РеквизитыВладельца, ИзвлеченныйТекст, ИзвлеченныйТекстНРег, ДлинаТекста, РазделителиСлов, НомерШаблона)
	...
	КонецЗаголовкаФамилия = 1;
	Пока Истина Цикл
		// Поиск заголовка фамилии.
		НачалоЗаголовкаФамилия = СтрНайти(ИзвлеченныйТекстНРег, ЗаголовокФамилия, , КонецЗаголовкаФамилия);
		Если НачалоЗаголовкаФамилия = 0 Или НачалоЗаголовкаФамилия > МаксНачалоЗаголовкаФамилия Тогда
			Результат.ПодробныйЖурнал.Добавить(СтрШаблон(НСтр("ru = 'Заголовок фамилии (""%1"") не найден.'"), ЗаголовокФамилия));
			Прервать;
		КонецЕсли;
		КонецЗаголовкаФамилия = НачалоЗаголовкаФамилия + ДлинаЗаголовкаФамилия;
		Результат.ПодробныйЖурнал.Добавить(СтрШаблон(
			НСтр("ru = 'Заголовок фамилии (""%1"") найден в позиции %2.'"),
			ЗаголовокФамилия,
			ЧислоВСтроку(НачалоЗаголовкаФамилия)));
		
		// Проверка что найдено целое слово, а не часть другого слова.
		// Для этого необходимо проверить что слева разделитель слов, а справа - разделитель слов или цифра.
		Если НачалоЗаголовкаФамилия > 1 Тогда
			СимволСлева = Сред(ИзвлеченныйТекст, НачалоЗаголовкаФамилия - 1, 1);
			Если РазделителиСлов.Найти(СимволСлева) = Неопределено Тогда
				Результат.ПодробныйЖурнал.Добавить(СтрШаблон(
					НСтр("ru = 'Слева от заголовка фамилии (""%1"") находится символ ""%2"", не являющийся разделителем слов.'"),
					ЗаголовокФамилия,
					СимволСлева));
				Продолжить;
			КонецЕсли;
		КонецЕсли;
		СимволСправа = Сред(ИзвлеченныйТекст, КонецЗаголовкаФамилия, 1);
		Если РазделителиСлов.Найти(СимволСправа) = Неопределено Тогда
			Результат.ПодробныйЖурнал.Добавить(СтрШаблон(
				НСтр("ru = 'Справа от заголовка фамилии (""%1"") находится символ ""%2"", не являющийся разделителем слов.'"),
				ЗаголовокФамилия,
				СимволСправа));
			Продолжить;
		КонецЕсли;
		...
		// Наполнение фамилии.
		Фамилия = "";
		Для КонецФамилии = КонецЗаголовкаФамилия + 1 По ДлинаТекста Цикл
			ОчереднойСимвол = Сред(ИзвлеченныйТекст, КонецФамилии, 1);
			Если РазделителиСлов.Найти(ОчереднойСимвол) = Неопределено Тогда
				// Буква.
				Фамилия = Фамилия + ОчереднойСимвол;
			Иначе
				// Разделитель слов.
				Если Фамилия <> "" Тогда
					Прервать;
		...
		Если Фамилия = "" Тогда
			Результат.ПодробныйЖурнал.Добавить(СтрШаблон(
				НСтр("ru = 'Справа от заголовка фамилии (""%1"") находятся только разделители слов.'"),
				ЗаголовокФамилия));
			Продолжить;
		ИначеЕсли СтрСравнить(Фамилия, ЗаголовокИмя) = 0 Тогда
			// Пропуск фрагментов типа "Ф|И|О" и "Фамилия|Имя|Отчество".
			Результат.ПодробныйЖурнал.Добавить(
				НСтр("ru = 'Справа от заголовка фамилии (""%1"") находится заголовок имени ""%2"". Следовательно, это не ФИО.'"));
			Продолжить;
		КонецЕсли;
		
		
		// Поиск заголовка имени.
		КонецЗаголовкаИмя = КонецФамилии;
		Пока Истина Цикл
			НачалоЗаголовкаИмя = СтрНайти(ИзвлеченныйТекстНРег, ЗаголовокИмя, , КонецЗаголовкаИмя);
			Если НачалоЗаголовкаИмя = 0 Или НачалоЗаголовкаИмя > МаксНачалоЗаголовкаИмя Тогда
				КонецЗаголовкаИмя = 0;
				Результат.ПодробныйЖурнал.Добавить(СтрШаблон(НСтр("ru = 'Заголовок имени (""%1"") не найден.'"), ЗаголовокИмя));
				Прервать;
			КонецЕсли;
			КонецЗаголовкаИмя = НачалоЗаголовкаИмя + ДлинаЗаголовкаИмя;
			Результат.ПодробныйЖурнал.Добавить(СтрШаблон(
				НСтр("ru = 'Заголовок имени (""%1"") найден в позиции %2.'"),
				ЗаголовокИмя,
				ЧислоВСтроку(НачалоЗаголовкаИмя)));
			
			// Проверка что найдено целое слово, а не часть другого слова.
			// Для этого необходимо проверить что слева разделитель слов, а справа - разделитель слов или цифра.
			СимволСлева = Сред(ИзвлеченныйТекст, НачалоЗаголовкаИмя - 1, 1);
			Если РазделителиСлов.Найти(СимволСлева) = Неопределено Тогда
				Результат.ПодробныйЖурнал.Добавить(СтрШаблон(
					НСтр("ru = 'Слева от заголовка имени (""%1"") находится символ ""%2"", не являющийся разделителем слов.'"),
					ЗаголовокИмя,
					СимволСлева));
				Продолжить;
			КонецЕсли;
			СимволСправа = Сред(ИзвлеченныйТекст, КонецЗаголовкаИмя, 1);
			Если РазделителиСлов.Найти(СимволСправа) = Неопределено Тогда
				Результат.ПодробныйЖурнал.Добавить(СтрШаблон(
					НСтр("ru = 'Справа от заголовка имени (""%1"") находится символ ""%2"", не являющийся разделителем слов.'"),
					ЗаголовокИмя,
					СимволСправа));
				Продолжить;
			КонецЕсли;
			
			Прервать; // Проверки пройдены.
		КонецЦикла;
		Если КонецЗаголовкаИмя = 0 Тогда
			Продолжить;
		КонецЕсли;
		
		// Наполнение имени.
		Имя = "";
		Для КонецИмени = КонецЗаголовкаИмя + 1 По ДлинаТекста Цикл
			ОчереднойСимвол = Сред(ИзвлеченныйТекст, КонецИмени, 1);
			Если РазделителиСлов.Найти(ОчереднойСимвол) = Неопределено Тогда
				// Буква.
				Имя = Имя + ОчереднойСимвол;
			Иначе
				// Разделитель слов.
				Если Имя <> "" Тогда
					Прервать;
		...
		КонецЦикла;
		Если Имя = "" Тогда
			Результат.ПодробныйЖурнал.Добавить(НСтр("ru = 'Справа от заголовка имени находятся только разделители слов.'"));
			Продолжить;
		КонецЕсли;
		
		
		// Поиск заголовка отчества.
		КонецЗаголовкаОтчество = КонецИмени;
		Пока Истина Цикл
			НачалоЗаголовкаОтчество = СтрНайти(ИзвлеченныйТекстНРег, ЗаголовокОтчество, , КонецЗаголовкаОтчество);
			Если НачалоЗаголовкаОтчество = 0 Или НачалоЗаголовкаОтчество > МаксНачалоЗаголовкаОтчество Тогда
				КонецЗаголовкаОтчество = 0;
				Результат.ПодробныйЖурнал.Добавить(СтрШаблон(НСтр("ru = 'Заголовок отчества (""%1"") не найден.'"), ЗаголовокОтчество));
				Прервать;
			КонецЕсли;
			КонецЗаголовкаОтчество = НачалоЗаголовкаОтчество + ДлинаЗаголовкаОтчество;
			Результат.ПодробныйЖурнал.Добавить(СтрШаблон(
				НСтр("ru = 'Заголовок отчества (""%1"") найден в позиции %2.'"),
				ЗаголовокОтчество,
				ЧислоВСтроку(НачалоЗаголовкаОтчество)));
			
			// Проверка что найдено целое слово, а не часть другого слова.
			// Для этого необходимо проверить что слева разделитель слов, а справа - разделитель слов или цифра.
			СимволСлева = Сред(ИзвлеченныйТекст, НачалоЗаголовкаОтчество - 1, 1);
			Если РазделителиСлов.Найти(СимволСлева) = Неопределено Тогда
				Результат.ПодробныйЖурнал.Добавить(СтрШаблон(
					НСтр("ru = 'Слева от заголовка отчества (""%1"") находится символ ""%2"", не являющийся разделителем слов.'"),
					ЗаголовокОтчество,
					СимволСлева));
				Продолжить;
			КонецЕсли;
			СимволСправа = Сред(ИзвлеченныйТекст, КонецЗаголовкаОтчество, 1);
			Если РазделителиСлов.Найти(СимволСправа) = Неопределено Тогда
				Результат.ПодробныйЖурнал.Добавить(СтрШаблон(
					НСтр("ru = 'Справа от заголовка отчества (""%1"") находится символ ""%2"", не являющийся разделителем слов.'"),
					ЗаголовокОтчество,
					СимволСправа));
				Продолжить;
			КонецЕсли;
			...
			Прервать; // Проверки пройдены.
		КонецЦикла;
		Если КонецЗаголовкаОтчество = 0 Тогда
			Продолжить;
		КонецЕсли;
		
		// Наполнение отчества.
		Отчество = "";
		Для НомерСимвола = КонецЗаголовкаОтчество + 1 По ДлинаТекста Цикл
			ОчереднойСимвол = Сред(ИзвлеченныйТекст, НомерСимвола, 1);
			Если РазделителиСлов.Найти(ОчереднойСимвол) = Неопределено Тогда
				// Буква.
				Отчество = Отчество + ОчереднойСимвол;
			Иначе
				// Разделитель слов.
				Если Отчество <> "" Тогда
					Прервать;
