...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Для Каждого СтрокаСотрудника Из СуммыВозврата Цикл
		Если СтрокаСотрудника.Налог  = 0 И СтрокаСотрудника.НалогСПревышения = 0 Тогда
			СообщениеОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='В строке №%1 не указаны суммы возврата налога.'"), СтрокаСотрудника.НомерСтроки);
