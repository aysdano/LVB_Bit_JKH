...
Функция СформироватьЗаявлениеНаПолучениеПатента(ПараметрыВыполнения, АдресРезультата = Неопределено) Экспорт
	...
	Если Не ПараметрыВыполнения.Свойство("ИмяФормы") Тогда
		ИмяОтчета = Отчеты.РегламентированноеУведомлениеПолучениеПатента.ПолучитьФормуПоУмолчанию();
		ПараметрыВыполнения.Вставить("ИмяФормы", РедакцияФормыЗаявления(ИмяОтчета));
	КонецЕсли;
