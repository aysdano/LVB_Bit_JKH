...
Процедура ПодключениеПринтераЭтикетокЗавершение(Значение, ДополнительныеПараметры) Экспорт
	...
	РезультатПодготовкиДанных = ДанныеДляПечатиНаПринтереЭтикеток(СписокОбъектов.ВыгрузитьЗначения(), АдресХранилищаСКД, XMLОписаниеМакета, ТипЦен, Количество);
...
&НаСервереБезКонтекста
Функция ДанныеДляПечатиНаПринтереЭтикеток(СписокОбъектов, АдресХранилищаСКД, XMLОписаниеМакета, ТипЦен, КоличествоПоУмолчанию)
...
Процедура ЗаполнитьПоДаннымМакета()
	...
	ИначеЕсли МенеджерОборудованияВызовСервера.ПолучитьОписаниеМакета(ОписаниеМакетаСтрокой, СообщениеОбОшибке, СодержимоеXMLФайла) Тогда
		...
		ДанныеДляПечатиЭтикеток = Обработки.ПечатьЭтикеток.ДанныеДляПечатиЭтикеток(
			...
			АдресХранилищаСКД, ОписаниеМакетаСтрокой, ТипЦен,,Ложь);
