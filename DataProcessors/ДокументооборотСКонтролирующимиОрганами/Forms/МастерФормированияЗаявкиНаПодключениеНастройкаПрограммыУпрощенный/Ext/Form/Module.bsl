...
Функция ТребуетсяПолучениеРасписки()
	...
	Одобрено = НовыйСтатусДокумента = Перечисления.СтатусыЗаявленияАбонентаСпецоператораСвязи.Одобрено;
...
Функция ПодписатьПлатформой()
	...
	ПодписатьКомпонентой = 
		 ОбработкаЗаявленийАбонентаКлиентСервер.ИспользоватьСуществующий(ДокументЗаявление) И ЭтоСертификатКА 
		 ИЛИ ЭтоВторичное И ЭтоСертификатКА;
		 
	ПодписатьПлатформой = НЕ ПодписатьКомпонентой;
