...
Функция СтранаАдресаКонтактнойИнформации(Знач XMLСтрока) Экспорт
	...
	// БИТ_ЖКХ_17082015_начало_изменений
	...
	// БИТ_ЖКХ_17082015_окончание_изменений
...
Процедура ДобавитьСтрокуКонтактнойИнформации(Форма, Результат)
	...
	ПолеВвода = ПолеВвода(Форма, , ИнформацияОВидеКИ.Тип, ИмяРеквизита, ИнформацияОВидеКИ.Подсказка);
...
Функция Действие(Форма, Тип, ИмяРеквизита, ГруппаДействий, КоличествоАдресов, ЕстьКомментарий = Ложь)
	...
	Если МожноСоздаватьДействие И ((Тип = Перечисления.ТипыКонтактнойИнформации.ВебСтраница
		...
		// БИТ_ЖКХ 18.10.2016 15:55:24 Начало правки... 
		//// Есть действие
		...
		//	
		//КонецЕсли;
		// БИТ_ЖКХ 18.10.2016 15:55:27 Конец правки. 
...
Процедура ПроверитьНаличиеРеквизитовКонтактнойИнформации(Форма, МассивДобавляемыхРеквизитов)
	...
	Если СоздатьТаблицуКонтактнойИнформации Тогда
		...
		// БИТ_ЖКХ 25.02.2015{{
		МассивДобавляемыхРеквизитов.Добавить(Новый РеквизитФормы("ПредставлениеЗапятые", Строка500, ИмяОписания));
		// БИТ_ЖКХ 25.02.2015}}
...
// БИТ_ЖКХ 24.02.2015{{
Процедура бит_ПроверитьНаличиеРеквизитовКонтактнойИнформации(Форма, МассивДобавляемыхРеквизитов)
	...
	Если СоздатьТаблицуКонтактнойИнформации Тогда
		...
		МассивДобавляемыхРеквизитов.Добавить(Новый РеквизитФормы("ИмяРеквизита", Строка500, ИмяОписания));
		//9.11.2018
//		МассивДобавляемыхРеквизитов.Добавить(Новый РеквизитФормы("Вид", Новый ОписаниеТипов("СправочникСсылка.бит_ВидыКонтактнойИнформацииЖКХ"), ИмяОписания));
		МассивДобавляемыхРеквизитов.Добавить(Новый РеквизитФормы("Вид", Новый ОписаниеТипов("СправочникСсылка.ВидыКонтактнойИнформации"), ИмяОписания));
		...
		// БИТ_ЖКХ 16.08.2016 18:11:36 Начало правки...
		МассивДобавляемыхРеквизитов.Добавить(Новый РеквизитФормы("ДействуетС", Новый ОписаниеТипов("Строка"), ИмяОписания));
		МассивДобавляемыхРеквизитов.Добавить(Новый РеквизитФормы("ХранитьИсториюИзменений",  Новый ОписаниеТипов("Булево"), ИмяОписания));
		// БИТ_ЖКХ 16.08.2016 18:11:36 Конец правки.
...
//                                                             ПоложениеЗаголовкаЭлементаФормы.Верх (по умолчанию).
//
Процедура бит_ПриСозданииНаСервере(Форма, Объект, ИмяЭлементаДляРазмещения = "", ПоложениеЗаголовкаКИ = "",
	Знач ИсключаемыеВиды = Неопределено, ОтложеннаяИнициализация = Ложь) Экспорт
	...
	МассивДобавляемыхРеквизитов = Новый Массив;
	бит_ПроверитьНаличиеРеквизитовКонтактнойИнформации(Форма, МассивДобавляемыхРеквизитов);
	...
	ИмяГруппыВидовКИ = СтрЗаменить(ПолноеИмяОбъектаМетаданных, ".", "");
	//9.11.2018
//	ГруппаВидовКИ = Справочники.бит_ВидыКонтактнойИнформацииЖКХ[ИмяГруппыВидовКИ];
	ГруппаВидовКИ = Справочники.ВидыКонтактнойИнформации[ИмяГруппыВидовКИ];
	...
	Если ЗначениеЗаполнено(СсылкаНаОбъект) Тогда
		Запрос.Текст ="
			...
			|	ЕСТЬNULL(КонтактнаяИнформация.НомерСтроки, 0)         КАК НомерСтроки,
			|	" + ДанныеИдентификатораСтрокиТабличнойЧасти + "      КАК ИдентификаторСтроки,
			// БИТ_ЖКХ 16.08.2016 18:04:40 Начало правки...
			|	ЛОЖЬ КАК ХранитьИсториюИзменений,
			|	"""" КАК ДействуетС,
			// БИТ_ЖКХ 16.08.2016 18:04:40 Конец правки.
			|	ВЫРАЗИТЬ("""""""" КАК СТРОКА(200))                    КАК ИмяРеквизита,
			|	ВЫРАЗИТЬ("""" КАК СТРОКА)                             КАК Комментарий
			|ИЗ
//			|	Справочник.бит_ВидыКонтактнойИнформацииЖКХ КАК ВидыКонтактнойИнформации
			|	Справочник.ВидыКонтактнойИнформации КАК ВидыКонтактнойИнформации
			|ЛЕВОЕ СОЕДИНЕНИЕ 
			|	" +  ПолноеИмяОбъектаМетаданных + ".КонтактнаяИнформация КАК КонтактнаяИнформация
			|ПО 
...
//			|	Справочник.бит_ВидыКонтактнойИнформацииЖКХ КАК ВидыКонтактнойИнформации
	...
	// БИТ_ЖКХ 25.02.2015{{
	//КонтактнаяИнформация.Сортировать("РеквизитДопУпорядочивания, НомерСтроки");
	Если Строка(ТипЗнч(СсылкаНаОбъект)) = "Проживающие " Тогда
		...
		Пока Индекс >=0 Цикл
			...
			Если СтрокаТаблицы.ИмяПредопределенныхДанных = ""
				или СтрокаТаблицы.ИмяПредопределенныхДанных = "АдресУбытия"
				или СтрокаТаблицы.ИмяПредопределенныхДанных = "АдресПрибытия" Тогда
				
				КонтактнаяИнформация.удалить(СтрокаТаблицы);	
				
			ИначеЕсли СтрокаТаблицы.ИмяПредопределенныхДанных = "ЮрАдресПроживающего" Тогда
				СтрокаТаблицы.ИдентификаторСтроки = 1;
			ИначеЕсли СтрокаТаблицы.ИмяПредопределенныхДанных = "ФактАдресПроживающего" Тогда
				СтрокаТаблицы.ИдентификаторСтроки = 2;
			ИначеЕсли СтрокаТаблицы.ИмяПредопределенныхДанных = "ТелефонПроживающего" Тогда
				СтрокаТаблицы.ИдентификаторСтроки = 3;
			ИначеЕсли СтрокаТаблицы.ИмяПредопределенныхДанных = "ИнформАдресПроживающего" Тогда
				СтрокаТаблицы.ИдентификаторСтроки = 4;
			ИначеЕсли СтрокаТаблицы.ИмяПредопределенныхДанных = "ИнострАдресПроживающего" Тогда
				СтрокаТаблицы.ИдентификаторСтроки = 5;
			ИначеЕсли СтрокаТаблицы.ИмяПредопределенныхДанных = "ТелефонСлужебныйПроживающего" Тогда
				СтрокаТаблицы.ИдентификаторСтроки = 6;
			ИначеЕсли СтрокаТаблицы.ИмяПредопределенныхДанных = "EmailПроживающего" Тогда
				СтрокаТаблицы.ИдентификаторСтроки = 7;
			ИначеЕсли СтрокаТаблицы.ИмяПредопределенныхДанных = "АдресПрибытияДрРегион" Тогда
				СтрокаТаблицы.ИдентификаторСтроки = 8;
			ИначеЕсли СтрокаТаблицы.ИмяПредопределенныхДанных = "ДругаяИнформацияПроживающего" Тогда
		...
		ПолеКИ2           = ПолучитьДанныеИзКонтактнойИнформации(СсылкаНаОбъект,"Адрес"                , "АдресПроживания");
...
//		Отбор.Вставить("Вид",ПредопределенноеЗначение("Справочник.бит_ВидыКонтактнойИнформацииЖКХ.ФактАдресПроживающего"));
		...
		Для каждого СтрокаДерева из КонтактнаяИнформация Цикл
			...
			Пока Индекс >=0 Цикл
				...
				Если СтрокаТаблицы.ИмяПредопределенныхДанных = ПредИмяПредопределенныхДанных Тогда
				...
				Попытка
					...
					ПредИмяПредопределенныхДанных = СтрокаТаблицы.ИмяПредопределенныхДанных;
	...
	ИначеЕсли Строка(ТипЗнч(СсылкаНаОбъект)) = "Список зданий " Тогда
	...
	ИначеЕсли Строка(ТипЗнч(СсылкаНаОбъект)) = "Подразделения организаций (БИТ)" Тогда
		...
		Пока Индекс >=0 Цикл
			...
			Если СтрокаТаблицы.ИмяПредопределенныхДанных = "" Тогда
				
				КонтактнаяИнформация.удалить(СтрокаТаблицы);	
				
			ИначеЕсли СтрокаТаблицы.ИмяПредопределенныхДанных = "АдресПодразделенияОрганизации" Тогда
				СтрокаТаблицы.ИдентификаторСтроки = 1;
			ИначеЕсли СтрокаТаблицы.ИмяПредопределенныхДанных = "ТелефонПодразделенияОрганизации" Тогда
				СтрокаТаблицы.ИдентификаторСтроки = 2;
			КонецЕсли;
			Индекс = Индекс - 1;
	...
	ИначеЕсли Строка(ТипЗнч(СсылкаНаОбъект)) = "Органы ФМС, ЗАГС, милиции " Тогда
		...
		Пока Индекс >=0 Цикл
			...
			ИначеЕсли СтрокаТаблицы.ИмяПредопределенныхДанных = "ФактАдресОрганаФМС" Тогда
				СтрокаТаблицы.ИдентификаторСтроки = 1;
			ИначеЕсли СтрокаТаблицы.ИмяПредопределенныхДанных = "ТелефонОрганаФМС" Тогда
				СтрокаТаблицы.ИдентификаторСтроки = 2;
			ИначеЕсли СтрокаТаблицы.ИмяПредопределенныхДанных = "ВебСтраницаОрганаФМС" Тогда
				СтрокаТаблицы.ИдентификаторСтроки = 3;
			КонецЕсли;
			Индекс = Индекс - 1;
	...
	Для Каждого СтрокаКИ Из КонтактнаяИнформация Цикл
		...
		Если Не СтатическийЭлемент Тогда
			...
			// БИТ ЖКХ 05.02.2020
//			ПолеВвода(Форма, СтрокаКИ.РедактированиеТолькоВДиалоге, СтрокаКИ.Тип, ИмяРеквизита, СтрокаКИ.Подсказка, ЭтоНовыйВидКИ, СтрокаКИ.ОбязательноеЗаполнение);
			ПолеВвода(Форма, , СтрокаКИ.Тип, ИмяРеквизита, СтрокаКИ.Подсказка, ЭтоНовыйВидКИ, СтрокаКИ.ОбязательноеЗаполнение);
...
//БИТ_ЖКХ 15.10.2018
// параметры: 
//        1. ИсходныйОбъект (1. ЛицевойСчет 2. СписокЗданий 3. Строка)
//		  2. Что возвращать?( 1. Регион 2. Город 3. НасПункт 4. неопределено)		
// возврат: 1. регион/город/нас пункт 2. Структура
Функция ПолучитьАдресБСП(объектЛицевойСчет, ВидПоля = неопределено) экспорт
	...
	если ТипЗнч(объектЛицевойСчет) = Тип("СправочникСсылка.бит_ЛицевыеСчета") тогда 	
      ...
      иначе
	  //19.12.2018 55 
	   АдресРезультат = РаботаСАдресами.СведенияОбАдресе(объектЛицевойСчет.Здание.ПочтовыйАдрес);
      конецесли; 
	конецесли; 
	...
	если ВидПоля = "Регион" тогда
		возврат СтруктураАдресРегистрации.Регион; 
	конецесли;
	
	если ВидПоля = "Город" тогда
		возврат СтруктураАдресРегистрации.Город; 
	конецесли;

	если ВидПоля = "НасПункт" тогда
		возврат СтруктураАдресРегистрации.НаселенныйПункт; 
	конецесли;
