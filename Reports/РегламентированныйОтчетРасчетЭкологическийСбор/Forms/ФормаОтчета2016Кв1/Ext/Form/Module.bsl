...
Функция Сохранить(Автосохранение = Ложь, КодИФНС = "не применимо", Вариант, ВидДокументаНомерКорректировкиИзменен = Неопределено, НаимТекРаздела) Экспорт
	...
	СписокСохранения.Вставить("Приложение", Приложение);
	СписокСохранения.Вставить("ИмяФайлаПриложения", ИмяФайлаПриложения);
...
Процедура ВосстановитьСохраненныеДанные()
	...
	СписокСохранения.Свойство("Приложение", Приложение);
	СписокСохранения.Свойство("ИмяФайлаПриложения", ИмяФайлаПриложения);
...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	Если СтруктураРеквизитовФормы.мСохраненныйДок = Неопределено Тогда
		...
		Если СтруктураРеквизитовФормы.мСкопированаФорма <> Неопределено Тогда
			...
			ИмяФайлаПриложения = Неопределено;
		Иначе
			// Это новый документ.
...
Процедура ТабличныйДокументВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли Область.Имя = "SEP_DIV" Или Область.Имя = "FINDIVID" Тогда
		...
		СтандартнаяОбработка = Ложь;
	ИначеЕсли Область.Имя = "ДобавитьПриложение" Тогда 
		Если Не ЗначениеЗаполнено(Приложение) Тогда 
			Если Не ЗначениеЗаполнено(СтруктураРеквизитовФормы.мСохраненныйДок) Или Модифицированность Тогда 
				...
				ОО = Новый ОписаниеОповещения("ВопросПриложениеЗавершение", ЭтотОбъект);
				ПоказатьВопрос(ОО, ТекстВопроса, РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Нет);
			Иначе
				ВопросПриложениеЗавершение(КодВозвратаДиалога.Да, Неопределено);
			КонецЕсли;
		Иначе
			РаботаСФайламиКлиент.ОткрытьФайл(ДанныеФайла(Приложение), Ложь);
		КонецЕсли;
	ИначеЕсли Область.Имя = "УдалитьПриложение" Тогда
		УдалитьФайлПриложения();
...
&НаСервере
Процедура УдалитьФайлПриложения()
	Попытка
		ПрисоединенныйФайлОбъект = Приложение.ПолучитьОбъект();
		ПрисоединенныйФайлОбъект.ПометкаУдаления = Истина;
		ПрисоединенныйФайлОбъект.Записать();
		Приложение = Справочники.РегламентированныйОтчетПрисоединенныеФайлы.ПустаяСсылка();
		ИмяФайлаПриложения = "";
	Исключение
...
&НаКлиенте
Процедура ВопросПриложениеЗавершение(Ответ, ДополнительныеПараметры) Экспорт
...
Процедура ПослеСохраненияЗавершениеЗавершение(Результат, ДополнительныеПараметры) Экспорт
	АдресФайла  = "";
	ВыбИмяФайла = "";
	ОО = Новый ОписаниеОповещения("ДобавитьПриложениеЗавершение", ЭтотОбъект);
...
&НаКлиенте
Процедура ДобавитьПриложениеЗавершение(Результат, АдресФайла, ВыбИмяФайла, ДополнительныеПараметры) Экспорт
	Если НЕ Результат Тогда
		Возврат;
	КонецЕсли;
	
	ИмяФайлаНРЕГ = НРег(ВыбИмяФайла);
	Если Не СтрЗаканчиваетсяНа(ИмяФайлаНРЕГ, ".zip")
		И Не СтрЗаканчиваетсяНа(ИмяФайлаНРЕГ, ".rar")
		И Не СтрЗаканчиваетсяНа(ИмяФайлаНРЕГ, ".7z") Тогда 
		
		ОбщегоНазначенияКлиент.СообщитьПользователю("Файл должен иметь одно из расширений: .zip, .rar, .7z");
		Возврат;
	КонецЕсли;
	...
	Попытка
		ЗаполнитьСтруктуруДанныхФайла(АдресФайла, СтрЗаменить(ВыбИмяФайла, Каталог, ""), Каталог);
	Исключение
		ШаблонСообщения = НСтр("ru = 'При загрузке файла возникла ошибка.
									 |%1'");
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
Процедура ЗаполнитьСтруктуруДанныхФайла(АдресФайла, ПолноеИмяФайла, Каталог) Экспорт 
	...
	ПараметрыФайла.Вставить("ВладелецФайлов", СтруктураРеквизитовФормы.мСохраненныйДок.Ссылка);
	...
	Приложение = МодульРаботаСФайлами.ДобавитьФайл(ПараметрыФайла, АдресФайла, , "Файл создан автоматически из формы уведомления, редактирование запрещено.");
	ИмяФайлаПриложения = ИмяФайла;
	Сохранить(,,,,"Титульный");
	ОтображениеГиперссылокДоверенности(ЭтотОбъект);
КонецПроцедуры
...
Процедура ОтображениеГиперссылокДоверенности(Форма)
	ФайлДоверенностиУдалить = Форма.ТабличныйДокумент.Области.Найти("УдалитьПриложение");
	ФайлДоверенностиДобавить = Форма.ТабличныйДокумент.Области.Найти("ДобавитьПриложение");
	Если ФайлДоверенностиУдалить = Неопределено Тогда 
		Возврат;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(Форма.Приложение) Тогда 
		ФайлДоверенностиУдалить.Гиперссылка = Истина;
		ФайлДоверенностиУдалить.Текст = "х";
		ФайлДоверенностиДобавить.Текст = Форма.ИмяФайлаПриложения;
	Иначе
		ФайлДоверенностиУдалить.Гиперссылка = Ложь;
		ФайлДоверенностиУдалить.Текст = "";
		ФайлДоверенностиДобавить.Текст = "Добавить архив с приложениями";
	КонецЕсли;
КонецПроцедуры
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	СпПарам = "ОргРПН,КодОргРПН,ID_ORG,FNAME_ORG,INN_ORG,OGRN,KPP,EGRULIP_NUM,ADDR_JUR,";
	СпПарам = СпПарам + "ADDJ_INDEX,ADDJ_STREET";
	...
	Для Каждого Элт Из СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(СпПарам, ",", Истина) Цикл 
		Если Не ЗначениеЗаполнено(ПараметрыВыгрузки[Элт]) Тогда 
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "Не заполнен обязательный показатель!", "Титульный", Элт);
			СтатусПроверки = Ложь;
		КонецЕсли;
	КонецЦикла;
...
Процедура ЗаполнитьДанными(ДеревоВыгрузки, ПараметрыВыгрузки)
	...
	Для Каждого Стр Из ДанныеОтчета.ДанныеМногостраничныхРазделов.Товары Цикл
		Группа = СтрЗаменить(НРег(Стр.Данные.П100), "группа", "");
		Группа = СтрЗаменить(Группа, "n", "");
		Группа = СтрЗаменить(Группа, "№", "");
		Группа = ОТЧ.ПривестиЗначение(СтрЗаменить(Группа, " ", ""));
		Группа = ?(Группа = 0, ОТЧ.ПривестиЗначение(Прав(Стр.Данные.П100, 2)), Группа);
		Для Каждого СтрМнг Из Стр.ДанныеДопСтрок.П11000 Цикл
			...
			УстановитьЗначениеЭлемента(НовыйУзел_Вещество, "EI", СтрМнг.П1100013);
			...
			УстановитьЗначениеЭлемента(НовыйУзел_Вещество, "GRP_NUM", Группа);
		КонецЦикла;
	КонецЦикла;
	
	Для Каждого Стр Из ДанныеОтчета.ДанныеМногостраничныхРазделов.Упаковка Цикл
		Группа = СтрЗаменить(НРег(Стр.Данные.П200), "группа", "");
		Группа = СтрЗаменить(Группа, "n", "");
		Группа = СтрЗаменить(Группа, "№", "");
		Группа = ОТЧ.ПривестиЗначение(СтрЗаменить(Группа, " ", ""));
		Группа = ?(Группа = 0, ОТЧ.ПривестиЗначение(Прав(Стр.Данные.П200, 2)), Группа);
		Для Каждого СтрМнг Из Стр.ДанныеДопСтрок.П21000 Цикл
			...
			УстановитьЗначениеЭлемента(НовыйУзел_Вещество, "EI", СтрМнг.П2100013);
			...
			УстановитьЗначениеЭлемента(НовыйУзел_Вещество, "GRP_NUM", Группа);
			УстановитьЗначениеЭлемента(НовыйУзел_Вещество, "PM_CCODE", СтрМнг.П2100014);
		КонецЦикла;
	КонецЦикла;
...
Функция СформироватьИмяФайла(ПараметрыВыгрузки)
	...
	ИмяФайлаXML = ПараметрыВыгрузки.ОргКодСубъект + "_эко_"
				+ Лев(ПараметрыВыгрузки.НаименованиеОрг, 25) + "_"
				+ ПараметрыВыгрузки.ОтчетныйГод + "_год"
...
Функция СформироватьСтруктуруПараметров()
	...
	Если ЗначениеЗаполнено(Приложение) Тогда 
		МодульРаботаСФайлами = ОбщегоНазначения.ОбщийМодуль("РаботаСФайлами");
		ДвоичныеДанныеФайла = МодульРаботаСФайлами.ДвоичныеДанныеФайла(Приложение);
		Base64СтрокаДанныеАрхива = СтрЗаменить(Base64Строка(ДвоичныеДанныеФайла), Символы.ПС, "");
		Base64СтрокаДанныеАрхива = СтрЗаменить(Base64СтрокаДанныеАрхива, Символы.ВК, "");
		ПараметрыВыгрузки.Вставить("Аттач", Base64СтрокаДанныеАрхива);
	Иначе
		ПараметрыВыгрузки.Вставить("Аттач", "");
	КонецЕсли;
