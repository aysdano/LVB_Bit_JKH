...
Процедура ДобавитьДоверенностьЗавершение(Результат, АдресФайла, ВыбИмяФайла, Парам) Экспорт
	...
	ЭтоЭлектроннаяПодписьВМоделиСервиса = ЭлектроннаяПодписьВМоделиСервисаБРОВызовСервера.РеквизитыУчетнойЗаписи(СтруктураРеквизитовФормы.Организация).ЭлектроннаяПодписьВМоделиСервиса;
	...
	КриптографияЭДКОКлиент.ХешироватьДанные(ОО, ЭтоЭлектроннаяПодписьВМоделиСервиса, ВыбИмяФайла,, Ложь,, "GOST R 34.10-2012-256");
КонецПроцедуры
...
Процедура ЗаполнитьТовары(Узел)
	...
	Для Каждого Стр Из ТаблицаСтраницыТовары Цикл 
		...
		Для Каждого Стр2 Из ДанныеДопСтрок Цикл 
			...
			РегламентированнаяОтчетность.УстановитьЗначениеЭлемента(product_group, "group_name", Данные.П100);
...
Процедура ЗаполнитьУпаковка(Узел)
	...
	Для Каждого Стр Из ТаблицаСтраницыУпаковка Цикл 
		...
		Для Каждого Стр2 Из ДанныеДопСтрок Цикл 
			...
			РегламентированнаяОтчетность.УстановитьЗначениеЭлемента(product_group, "group_name", Данные.П200);
...
Процедура ПодписатьФайлГОСТ2012Завершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(Результат) = Тип("Структура") и Результат.Свойство("Хеш") Тогда 
		НовСтр.Комментарий = Результат.Хеш;
	Иначе
		НовСтр.Комментарий = "";
	КонецЕсли;
...
Процедура ПриложенияКОтчетуВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если УровеньГруппировки(ТекущиеДанные) = 0 Тогда
		...
		ИмяФормыГр = СтрЗаменить(ИмяФормы, "ФормаОтчета2020Кв1", "ФормаВводаРеквизитовДокумента");
		ОписаниеОповещения = Новый ОписаниеОповещения("ДобавитьДокументЗавершение", ЭтотОбъект, ПараметрыВводаРеквизитов);
		
		ФормаВводаРеквизитовДокумента = ПолучитьФорму(ИмяФормыГр, ПараметрыОткрытияФормы, ЭтотОбъект);
		ФормаВводаРеквизитовДокумента.ОписаниеОповещенияОЗакрытии = ОписаниеОповещения;
		ФормаВводаРеквизитовДокумента.РежимОткрытияОкна = РежимОткрытияОкнаФормы.БлокироватьОкноВладельца;
		ФормаВводаРеквизитовДокумента.Открыть();
	Иначе
