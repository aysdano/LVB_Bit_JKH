...
Процедура СформироватьXMLПакетДляЧекаКоррекции(ОбщиеПараметры, ПараметрыФискализации) Экспорт
	...
	Иначе
			...
			Для ИндексМассива = 0 По ОбщиеПараметры.ПозицииЧека.Количество() - 1 Цикл
				...
				Если ПозицияЧека.Свойство("ФискальнаяСтрока") Тогда
					...
					ИначеЕсли СтавкаНДС = 22 Тогда
						СуммаНДС22 = СуммаНДС22 + ?(СуммаНДС > 0, СуммаНДС, ПозицияЧека.Сумма / 122 * 22);
					ИначеЕсли СтавкаНДС = 122 Тогда
						СуммаНДС122 = СуммаНДС122 + ?(СуммаНДС > 0, СуммаНДС, ПозицияЧека.Сумма / 122 * 22);
						//--1бит Феденков 26.01.2026	
					Иначе 
		...
		//++1бит Феденков 26.01.2026
        ЗаписьXML.ЗаписатьАтрибут("SumTAX22"  , XMLСтрока(Окр(СуммаНДС22, 2))); 
		ЗаписьXML.ЗаписатьАтрибут("SumTAX122" , XMLСтрока(Окр(СуммаНДС122, 2))); 
        //--1бит Феденков 26.01.2026
		ЗаписьXML.ЗаписатьКонецЭлемента();
