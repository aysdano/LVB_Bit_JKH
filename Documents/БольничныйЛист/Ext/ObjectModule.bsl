...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Если ПособиеВыплачиваетсяФСС Тогда
		...
		Если ТекстОшибки <> "" Тогда
			СообщенияПользователюБЗК.СообщитьОбОшибкеВОбъекте(Отказ, ЭтотОбъект, "НомерЛисткаНетрудоспособности", ТекстОшибки);
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗаполнитьПоПервичномуБольничномуЛисту(НомерИлиСсылкаПервичногоБольничного) Экспорт
	...
	Если ПричинаНетрудоспособности = Перечисления.ПричиныНетрудоспособности.ПособиеПриДолечивании
		...
		Если ДатаНачалаСобытия <> ДатаНачалаОплаты Тогда
			ОплаченоРанее = (ДатаНачалаОплаты - ДатаНачалаСобытия) / 86400;
		Иначе
			ОплаченоРанее = 0;
		КонецЕсли;
		
		МаксимальноеКоличествоОплачиваемыхДней = УчетПособийСоциальногоСтрахования.ДнейОплатыЗаСлучайНетрудоспособности(
			ПричинаНетрудоспособности,
		...
		Если Не МаксимальноеКоличествоОплачиваемыхДней = Неопределено Тогда
			...
			Если ДнейОплаты > 0 И ЗначениеЗаполнено(ДатаНачалаОплаты) Тогда
				...
				ДатаОкончанияОплаты = ДатаНачалаОплаты + (ДнейОплаты - 1) * 86400;
			Иначе
