...
Функция ОписаниеДействия(Правило, ПериодСобытия, ПараметрыПереносаСроков)
	...
	Если Правило.СчитатьСрокВРабочихДнях Тогда
		...
		РабочаяДата = КалендарьБухгалтера.РабочаяДатаПоКалендарю(ПроизводственныйКалендарь, Срок, Правило.СрокДней);
	...
	Если Правило.ИсключатьПериодИзПредставленияЗадачи Тогда
		ШаблонТекста = НСтр("ru = '[Задача][Событие] [НалоговыйОрган]'");
	Иначе
