	...
	Для Каждого СтрокаПлатеж Из ТаблицаДвиженияВзаиморасчетов Цикл
		...
		ИначеЕсли ВидОперации = "Доход" Тогда
			...
			Если Не ДоходУчтенДоПереходаНаУСН Тогда
				...
				//Если Значениезаполнено(СтрокаПлатеж.РасчетныйДокумент) Тогда
				//	Попытка
				//		Если ТипЗнч(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаПлатеж.РасчетныйДокумент, "ДокументОснование")) = Тип("ДокументСсылка.бит_ОплатаКУ") 
				//			КУДиР.Графа5 = СтрокаПлатеж.РасчетныйДокумент.Графа5_УСН;
				//			СтрокаРасшифровки.Графа5 = СтрокаПлатеж.РасчетныйДокумент.Графа5_УСН;
				//		Иначе
				//			КУДиР.Графа5 = КУДиР.Графа5 + СуммаГрафа5;
