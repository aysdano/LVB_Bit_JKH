...
Процедура НастроитьОтправку(Команда)
	...
	БанкВыбран = ЗначениеЗаполнено(Банк);
	ТекущиеДанные = Элементы.Список.ТекущиеДанные;
	ЭтоСбербанк = Ложь;
	Если БанкВыбран Тогда
		ЭтоСбербанк = НЕ ФинОтчетностьВБанкиКлиентСервер.ЭтоБанкУниверсальногоОбмена(Банк);
	ИначеЕсли ТекущиеДанные <> Неопределено Тогда
		ЭтоСбербанк = НЕ ФинОтчетностьВБанкиКлиентСервер.ЭтоБанкУниверсальногоОбмена(Элементы.Список.ТекущиеДанные.Банк);
	КонецЕсли;
	...
	Иначе
		...
		Если СведенияОДлительнойОперации.ДлительнаяОперацияОбновленияДанныхСервиса <> Неопределено
			...
			ОбработчикПослеЗавершения = Новый ОписаниеОповещения("ОткрытьФормуНастроекОтправкиЗавершение", ЭтотОбъект);
			ОжидатьЗавершениеОбновленияДанныхСервиса(Истина, ОбработчикПослеЗавершения);
			Возврат;
		КонецЕсли;
		
		ОткрытьФормуНастроекОтправкиЗавершение(Истина, Неопределено);
	КонецЕсли;
...
Процедура ОткрытьФормуНовогоОтчета()
	...
	// Проверяем, что сервер был доступен и настройки комплектов отчетности сейчас актуальны.
	Если ЗначениеЗаполнено(ОписаниеОшибкиОбновленияДанныхСервиса) Тогда
...
Процедура УправлениеФормой(Форма)
	...
	Элементы.СписокНастроитьОтправку.Видимость = Форма.ЕстьПравоРедактированияОтчета;
