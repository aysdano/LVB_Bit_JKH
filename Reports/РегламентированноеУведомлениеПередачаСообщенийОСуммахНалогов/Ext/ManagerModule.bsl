...
Функция ПолучитьТаблицуФорм() Экспорт 
	...
	Стр = Результат.Добавить();
	Стр.ИмяФормы = "Форма2020_1";
	Стр.ОписаниеФормы = "Заявление о передаче сообщений об исчисленных налоговым органом суммах транспортного налога и земельного налога в соответствии с письмом ФНС России от 10.03.2021 N БС-4-21/3006@.";
...
Функция ЭлектронноеПредставление(Объект, ИмяФормы, УникальныйИдентификатор) Экспорт
	Если ИмяФормы = "Форма2020_1" Тогда
		ОбщегоНазначения.СообщитьПользователю("Выгрузка уведомления не предусмотрена");
		Возврат "";
...
Процедура НапечататьСтруктуру(СтруктураДанныхСтраницы, НомСтр, ИмяМакета, ПечатнаяФорма, ИННКПП)
	Попытка
		МакетПФ = Отчеты.РегламентированноеУведомлениеПередачаСообщенийОСуммахНалогов.ПолучитьМакет(ИмяМакета);
	Исключение
		Возврат;
	КонецПопытки;
