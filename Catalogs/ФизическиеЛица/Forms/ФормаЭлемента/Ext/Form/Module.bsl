...
&НаКлиенте
Процедура бит_ДоверенностиПриИзменении(Элемент)
	
	текСтрока=элементы.бит_Доверенности.ТекущиеДанные;
	пр = просклонятьВидДокумента(текСтрока.виддокумента);
	текСтрока.представление = "На основании "
	+ ?(пр<>"",пр,текСтрока.виддоговора)+" "
	+ ?(ЗначениеЗаполнено(текСтрока.номер),"№ "+текСтрока.номер+" ","")
	+ ?(ЗначениеЗаполнено(текСтрока.датас),"от "+формат(текСтрока.датас,"ДЛФ=Д")+" ","");
	
КонецПроцедуры

функция просклонятьВидДокумента(ВидДок)
	
	возврат(СклонениеПредставленийОбъектов.ПросклонятьПредставление(ВидДок,2));
