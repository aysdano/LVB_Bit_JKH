...
Процедура ОбновитьИтогиУСН(Форма) Экспорт
	...
	Если Форма.ПрименениеУСН Тогда
		Объект = Форма.Объект;
		// БИТ_ЖКХ 13.03.2017 14:55:53 Начало правки... 
		// БИТ_ЖКХ 28.04.2015{{
		//Если ФормаДокументаОднострочная(Форма) Тогда
		...
		//КонецЕсли;
		// БИТ_ЖКХ 28.04.2015}}
		Если НЕ ТипЗнч(Объект.ДокументОснование) = Тип("ДокументСсылка.бит_ОплатаКУ") Тогда
			Если ФормаДокументаОднострочная(Форма) Тогда
				Объект.Графа5_УСН = Форма.РасшифровкаПлатежаДоходыУСН;
			Иначе
				РасшифровкаПлатежа = ПолучитьРасшифровкаПлатежа(Форма);
			...
			КонецЕсли;
		КонецЕсли;
		// БИТ_ЖКХ 13.03.2017 14:55:59 Конец правки. 		
	КонецЕсли;
