...
Процедура СформироватьСтрокуПроверкиУИН()
	...
	СредстваПоступающиеВоВременноеРаспоряжение =
		ПлатежиВБюджетКлиентСервер.ЭтоСчетКазначействаСредствПоступающихВоВременноеРаспоряжение(
			Контекст.НомерСчетаПолучателя);
		Если Не ПустаяСтрока(ПлатежиВБюджетКлиентСервер.ПроверитьУИН(
				УИН,
				ПлатежНеВФедеральныйОрган,
				СредстваПоступающиеВоВременноеРаспоряжение,
				Контекст.Период)) Тогда
...
Процедура НастроитьФорму(Знач ЧастьФормы, Знач Реквизиты = Неопределено)
	...
	Если ЧастьФормы = "" Тогда
		...
		НастроитьСписокВыбора(
			...
			ПлатежиВБюджетКлиентСервер.СтатусыПлательщика(Мин(ПериодДокумента, Контекст.Период)));
