...
Процедура ПоказатьПериод(Форма)
	...
	Если ГодПериода = 2020 И Форма.РезультирующаяТаблица.Количество() = 2
		И СокрЛП(Форма.РезультирующаяТаблица[0].ФормаОтчета) = "ФормаОтчета2021Кв1"
		И СокрЛП(Форма.РезультирующаяТаблица[1].ФормаОтчета) = "ФормаОтчета2015Кв1" Тогда
