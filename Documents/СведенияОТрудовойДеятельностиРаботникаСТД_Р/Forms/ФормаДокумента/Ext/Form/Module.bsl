...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ЗаписьФайлаСТДР"
		...
		ПрисоединенныйФайл = Параметр;
		ТолькоПросмотр = Истина;
		УстановитьОтображениеПредупрежденияОПоследствияхРедактирования(ЭтотОбъект);
...
Процедура Подключаемый_ИзменитьДокумент(Команда)
	...
	Если УдалитьПрисоединенныйФайл(ПрисоединенныйФайл) Тогда
...
Функция УдалитьПрисоединенныйФайл(ПрисоединенныйФайл)
	
	Попытка
		ПрисоединенныйФайлОбъект = ПрисоединенныйФайл.ПолучитьОбъект();
		ПрисоединенныйФайлОбъект.УстановитьПометкуУдаления(Истина);
	Исключение
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Не удалось пометить на удаление файл СТД-Р'") + " (" + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке())+ ")");
		Возврат Ложь;
	КонецПопытки;
...
Процедура ПриПолученииДанныхНаСервере()
	...
	Иначе
		ПрисоединенныйФайл = Документы.СведенияОТрудовойДеятельностиРаботникаСТД_Р.ПрисоединенныйФайлСТДР(Объект.Ссылка);
	КонецЕсли;
	
	ТолькоПросмотр = ЗначениеЗаполнено(ПрисоединенныйФайл);
	УстановитьОтображениеПредупрежденияОПоследствияхРедактирования(ЭтотОбъект);
	
	Если ЗначениеЗаполнено(ПрисоединенныйФайл) Тогда
		// ЗарплатаКадрыПодсистемы.КадровыйУчет.ЭлектронныеТрудовыеКнижки
		ЭлектронныеТрудовыеКнижки.ДобавитьКомандуИзмененияПереданногоДокумента(ЭтотОбъект);
		// Конец ЗарплатаКадрыПодсистемы.КадровыйУчет.ЭлектронныеТрудовыеКнижки
	КонецЕсли;
...
Процедура УстановитьОтображениеПредупрежденияОПоследствияхРедактирования(УправляемаяФорма)
	
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		УправляемаяФорма.Элементы,
		"ПредупреждениеОПоследствияхРедактирования",
		"Видимость",
