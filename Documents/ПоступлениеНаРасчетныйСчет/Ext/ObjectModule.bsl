...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Попытка
		...
		//Если Строка(Типзнч(ДокументОснование)) = "Оплата КУ"	Тогда
			...
			Если ЗначениеЗаполнено(ТаблицаВзаиморасчетов) Тогда
				
				Если ТаблицаВзаиморасчетов.Количество() > 0 Тогда 
					
					Если НЕ ЗначениеЗаполнено(ТаблицаВзаиморасчетов[0].ДокументРасчетов) Тогда
						
						Для Каждого Строка из ТаблицаВзаиморасчетов Цикл
							
							Строка.ДокументРасчетов = Ссылка;
							// БИТ_ЖКХ 15.10.2020
						    Строка.ДокументРасчетов = Документы.ПоступлениеНаРасчетныйСчет.ПустаяСсылка();
