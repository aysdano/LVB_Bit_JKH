...
Процедура ТоварыНоменклатураПриИзменении(Элемент)
	...
	ДанныеОбъекта = Новый Структура(
		"Дата, Организация, Основание, ВидОперации, Склад, ТипЦен, СуммаВключаетНДС, ДокументБезНДС");
...
Процедура АгентскиеУслугиНоменклатураПриИзменении(Элемент)
	...
	ДанныеОбъекта = Новый Структура(
		"Дата, Организация, Основание, ВидОперации, ТипЦен, СуммаВключаетНДС, ДокументБезНДС");
	ЗаполнитьЗначенияСвойств(ДанныеОбъекта, Объект);
	ЗаполнитьЗначенияСвойств(ДанныеОбъекта, ЭтаФорма);
...
Функция ТекстЗапросаТаблицаТоваров(НомераТаблиц)
	...
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(ТаблицаЧека.ШтрихкодыУпаковокТоваровСсылка.ТипУпаковки, &ЕдиницаТовара) = &ЕдиницаТовара
	|			ТОГДА 0
	|		ИНАЧЕ 1
	|	КОНЕЦ КАК КоличествоУпаковок,
	|	ЕСТЬNULL(ТаблицаЧека.ШтрихкодыУпаковокТоваровСсылка.ТипУпаковки, &ЕдиницаТовара) <> &ЕдиницаТовара КАК ЭтоУпаковка,
	|	ВЫБОР
	...
	|	ВЫРАЗИТЬ(ЕСТЬNULL(ТаблицаЧека.ШтрихкодыУпаковокТоваровСсылка.ЗначениеШтрихкода, """") КАК СТРОКА(200)) КАК Штрихкод,
...
Функция ПараметрыПечатиЧека(ПараметрыОплаты, ИдентификаторУстройства, ВерсияФормата, АдресУстановкиККТ, ОписаниеОшибки)
	...
	Если ПолучитьФункциональнуюОпцию("ВестиУчетМаркируемойПродукцииИСМП") Тогда
		Если НЕ ОбработатьМаркированнуюПродукцию(ТаблицаДокумента, ОписаниеОшибки) Тогда
	...
	Для каждого СтрокаТовары Из ТаблицаДокумента Цикл
		...
		// Упаковки в форме документа отражаются как количество единиц продукции в их составе, 
		// но в чеке всегда должно быть 1
		Если СтрокаТовары.ЭтоУпаковка Тогда
...
Функция ПараметрыПечатиЧекаНаличныеКарта(ПараметрыОплаты, ИдентификаторУстройства, ВерсияФормата, АдресУстановкиККТ, ОписаниеОшибки)
	...
	// для наличных перед печатью чека очистим таблицу оплат
	Если ПараметрыОплаты.ОплатаНаличные <> 0 Тогда
...
Функция СписокОборудованияПечатьЧека(РабочееМесто)
	...
	СписокОборудованияПоТипу = СписокОборудованияПоТипу(МассивТиповОборудования, РабочееМесто);
	ТипыОборудования = ОбщегоНазначения.ЗначениеРеквизитаОбъектов(СписокОборудованияПоТипу, "ТипОборудования");
	
	Для каждого ЭкземплярОборудования Из СписокОборудованияПоТипу Цикл
		Если ТипыОборудования[ЭкземплярОборудования] <> Перечисления.ТипыПодключаемогоОборудования.ККТ Тогда
			СписокОборудованияПечатьЧека.Добавить(ЭкземплярОборудования);
		Иначе
			...
			Если НалоговыеРежимыРазрешенныеВНастройкахККТ.Найти(Объект.СистемаНалогообложения) <> Неопределено Тогда
				СписокОборудованияПечатьЧека.Добавить(ЭкземплярОборудования);
			КонецЕсли; 
		КонецЕсли; 
	КонецЦикла; 
	
	Возврат СписокОборудованияПечатьЧека;
КонецФункции
...
Процедура НапечататьЧек(ПараметрыОплаты)
	...
	ИначеЕсли СписокОборудования.Количество() > 0 Тогда
		СтруктураОтбора = Новый Структура("Ссылка", СписокОборудования);
...
&НаСервере
Функция ОбработатьМаркированнуюПродукцию(ТаблицаДокумента, СообщениеОбОшибке)
	...
	МассивШтрихкодыУпаковок = Новый Массив;
	Для каждого СтрокаТаблицыДокумента  Из ТаблицаДокумента Цикл
		Если ЗначениеЗаполнено(СтрокаТаблицыДокумента.ШтрихкодыУпаковокТоваровСсылка) Тогда
			ЗаполнитьЗначенияСвойств(ТаблицаТовары.Добавить(), СтрокаТаблицыДокумента);
			МассивШтрихкодыУпаковок.Добавить(СтрокаТаблицыДокумента.ШтрихкодыУпаковокТоваровСсылка);
		КонецЕсли; 
	КонецЦикла; 
	
	ТаблицаТовары.Свернуть("Номенклатура, Характеристика, Серия, Склад", "Количество, КоличествоУпаковок");
	...
	Иначе
		Для каждого СтрокаТовары Из ТаблицаТовары Цикл
			...
			Если ШтрихкодыПоТовару = Неопределено Тогда
				Продолжить;
			КонецЕсли; 
			...
			Для каждого СтрокаДокумента Из СтрокиДокумента Цикл
				Если ШтрихкодыПоТовару.Количество() > 0 Тогда
					СтрокаДокумента.Штрихкод = ШтрихкодыПоТовару[0].Штрихкод;
...
Функция ВидыПродукцииГОСИС()
	...
	Если ПолучитьФункциональнуюОпцию("ВестиУчетМаркируемойПродукцииИСМП") Тогда
		...
		ВидыПродукции.Добавить(Перечисления.ВидыПродукцииИС.АльтернативныйТабак);
		ВидыПродукции.Добавить(Перечисления.ВидыПродукцииИС.УпакованнаяВода);
	КонецЕсли; 
...
Процедура ТоварыНоменклатураПриИзмененииНаСервере(СтрокаТабличнойЧасти, Знач ДанныеОбъекта)
	...
	Если ДанныеОбъекта.ВидОперации = Перечисления.ВидыОперацийРозничнаяПродажа.Возврат
...
Процедура ОбновитьСписокНалоговыхРежимов()
	...
	СписокНалоговыхРежимов = Документы.РозничнаяПродажа.СписокНалоговыхРежимовДляПечатиЧека(Объект.Организация, Объект.Склад, Объект.Дата);
	
	СписокНалоговыхРежимовДляПечатиЧека.Очистить();
	Для каждого НалоговыйРежим Из СписокНалоговыхРежимов Цикл
