...
Процедура ПроверитьПереносУбыткаНаБудущее(Период, Организация, Ошибки, Отказ)
	...
	Если ВариантУчетаОтложенногоНалога = Перечисления.ВариантыУчетаОтложенногоНалога.Нет Тогда
		// Эта процедура опирается на данные учета по ПБУ 18
		Возврат;
	КонецЕсли;
	...
	Если Перечисления.ВариантыУчетаОтложенногоНалога.ЭтоБалансовыйМетод(ВариантУчетаОтложенногоНалога) Тогда
		
		// Строго говоря, при балансовом методе такая проверка не требуется именно в этом контексте -
		// это не препятствует ни расчету налога, ни расчету для целей ПБУ 18.
		// Но так как нет других средств напомнить пользователю о праве на перенос убытка, она выполняется.
		
		ТекстСообщения = НСтр(
			"ru = 'Не перенесен убыток прошлого года.
             |
             |На счете 09 на конец года осталось сальдо по виду ""Убыток текущего периода"".
             |Этот вид предназначен для использования только в течение года.
             |То, что он остался на конец года, указывает, что в программе не отражено намерение перенести налоговый убыток прошлого года на будущее.
             |
             |Порядок переноса убытка см. в Справочнике хозяйственных операций'");
		СсылкаСообщения = Новый Структура;
		СсылкаСообщения.Вставить("Представление", НСтр("ru = 'Перенос убытков на будущее'"));
		СсылкаСообщения.Вставить("Ссылка",        "https://its.1c.ru/db/hoosn#content:482:hdoc");
	...
	Иначе
		...
		СсылкаСообщения = "e1cib/list/Документ.ОперацияБух";
		
	КонецЕсли;
	
	ВыводСообщенийОбОшибках.ДобавитьПростоеОписаниеОшибки(Ошибки, ТекстСообщения, СсылкаСообщения);
...
Процедура СформироватьДвижения(СтруктураШапкиДокумента, Отказ, Ошибки)
	...
	ИначеЕсли СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыРегламентныхОпераций.ОбесценениеЗапасов Тогда
		...
		УчетОбесцененияЗапасов.СформироватьДвиженияРегламентнойОперации(
			Движения,
			СтруктураШапкиДокумента.Дата,
			СтруктураШапкиДокумента.Организация,
			СтруктураШапкиДокумента.Ссылка);
