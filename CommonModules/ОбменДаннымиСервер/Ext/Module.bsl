...
Функция ПараметрыИнформационнойБазы(Знач ИмяПланаОбмена, Знач КодУзла, СообщениеОбОшибке) Экспорт
	...
	Результат.Вставить("ВерсияКонфигурации",                        бит_ОбщегоНазначенияПовтИсп.ПолучитьНомерРелиза_1С());
	...
	Результат.ВерсияКонфигурации                   = бит_ОбщегоНазначенияПовтИсп.ПолучитьНомерРелиза_1С();
...
Процедура ПриЗаполненииСпискаТекущихДелПроверитьСовместимостьСТекущейВерсией(ТекущиеДела)
	...
	Если ПровереноНаВерсию <> Неопределено Тогда
		ВерсияМассив  = СтрРазделить(бит_ОбщегоНазначенияПовтИсп.ПолучитьНомерРелиза_1С(), ".");
