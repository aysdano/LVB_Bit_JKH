...
Функция ПроверитьДокументСВыводомВТаблицу_Форма2020_1(Данные, УникальныйИдентификатор)
	...
	Для Каждого Лист0Итер Из Данные.ДанныеМногостраничныхРазделов.Транспорт Цикл
		...
		КонецЕсли;
		Если Не ЗначениеЗаполнено(Лист0.ДатаРегТС)
			Или Год(Лист0.ДатаРегТС) < 1900 Тогда 
			ТаблицаОшибок.Добавить(Документы.УведомлениеОСпецрежимахНалогообложения.ПолучитьПустуюСтруктуруНавигацииПоОшибкам(
				"Не указана/неправильно указана дата регистрации транспортного средства", "Транспорт", "ДатаРегТС", Лист0.УИД));
		КонецЕсли;
		...
		КонецЕсли;
		Если Не ЗначениеЗаполнено(Лист0.ДатаДокРегТС)
			Или Год(Лист0.ДатаДокРегТС) < 1900 Тогда 
			ТаблицаОшибок.Добавить(Документы.УведомлениеОСпецрежимахНалогообложения.ПолучитьПустуюСтруктуруНавигацииПоОшибкам(
				"Не указана/неправильно указана дата выдачи документа, подтверждающего регистрацию транспортного средства", "Транспорт", "ДатаДокРегТС", Лист0.УИД));
		КонецЕсли;
		Если ЗначениеЗаполнено(Лист0.ДатаСнУчетТС)
			И Год(Лист0.ДатаСнУчетТС) < 1900 Тогда 
			ТаблицаОшибок.Добавить(Документы.УведомлениеОСпецрежимахНалогообложения.ПолучитьПустуюСтруктуруНавигацииПоОшибкам(
				"Неправильно указана дата снятия с учета", "Транспорт", "ДатаСнУчетТС", Лист0.УИД));
		КонецЕсли;
		Если ЗначениеЗаполнено(Лист0.ДатаПасп)
			И Год(Лист0.ДатаПасп) < 1900 Тогда 
			ТаблицаОшибок.Добавить(Документы.УведомлениеОСпецрежимахНалогообложения.ПолучитьПустуюСтруктуруНавигацииПоОшибкам(
				"Неправильно указана дата выдачи ПТС", "Транспорт", "ДатаПасп", Лист0.УИД));
		КонецЕсли;
	КонецЦикла;
	...
	Для Каждого Лист0Итер Из Данные.ДанныеМногостраничныхРазделов.Земля Цикл
		...
		КонецЕсли;
		Если Не ЗначениеЗаполнено(Лист0.ДатаРегПрав) 
			Или Год(Лист0.ДатаРегПрав) < 1900 Тогда 
			ТаблицаОшибок.Добавить(Документы.УведомлениеОСпецрежимахНалогообложения.ПолучитьПустуюСтруктуруНавигацииПоОшибкам(
			"Не заполнена/неправильно заполнена дата регистрации (возникновения) права", "Земля", "ДатаРегПрав", Лист0.УИД));
		КонецЕсли;
		Если ЗначениеЗаполнено(Лист0.ДатаПрекрПрав)
			И Год(Лист0.ДатаПрекрПрав) < 1900 Тогда 
			ТаблицаОшибок.Добавить(Документы.УведомлениеОСпецрежимахНалогообложения.ПолучитьПустуюСтруктуруНавигацииПоОшибкам(
				"Неправильно указана дата прекращения", "Земля", "ДатаПрекрПрав", Лист0.УИД));
		КонецЕсли;
	КонецЦикла;
