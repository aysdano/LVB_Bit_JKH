...
Процедура ВставитьДополнительнуюСтроку(ИмяГруппы, ИндексВставляемойСтроки)
	...
	ДеревоМнЧ = ДеревоРазд.Строки[0].ДанныеМногострочныхЧастей[ИмяГруппы];
...
Функция ВыполнитьДокументированиеНаСервере()
	...
	СтруктураРезультата.Вставить("НаименованиеОтчета"	, "6-НДФЛ (с 2021 года)");
...
Процедура СформироватьДанныеОтчета(ОтчетСсылка, СтруктураОтчета)
	...
	СтруктураДанных = ОбъектОтчета(ЭтаФорма.ИмяФормы).СохраненныеДанныеОтчета(
	СтруктураРеквизитовФормы.мВыбраннаяФорма, СтруктураРеквизитовФормы, мСтруктураМногоуровневыхРазделов);
...
Процедура СформироватьСлужебныеДанные()
	...
	МассивПолейСписочныхЗначений.Добавить("П000010001001");
	МассивПолейСписочныхЗначений.Добавить("П000020010501");
...
Процедура ПроверкаКС_1_02()
	...
	Для каждого СтраницаРаздела Из ДанныеКС.Раздел2 Цикл
		...
		Настройки.ЭтоОшибка = НЕ Показатели.Р02С110.Значение >= Показатели.Р02С130.Значение;
		Настройки.Расшифровка = ПодставитьПараметрыВСтроку(Настройки.ШаблонРасшифровки, Показатели.Р02С110.Значение,
...
Процедура ПроверкаКС_1_03()
	...
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru='%1 = (%2 - %3) х %4 / 100 Итог [%5]'"));
	...
	Для каждого СтраницаРаздела Из ДанныеКС.Раздел2 Цикл
		...
		Настройки.ЭтоОшибка = НЕ Показатели.Р02С140.Значение = РасчетнаяСумма;
		Настройки.Расшифровка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Настройки.ШаблонРасшифровки, 
...
Процедура ПроверкаКС_1_04()
	...
	Для каждого СтраницаРаздела Из ДанныеКС.Раздел2 Цикл
		...
		Настройки.ЭтоОшибка = НЕ Показатели.Р02С140.Значение >= Показатели.Р02С150.Значение;
		Настройки.Расшифровка = ПодставитьПараметрыВСтроку(Настройки.ШаблонРасшифровки, Показатели.Р02С140.Значение,
...
Процедура ПроверкаКС_1_14()
	...
	Заголовки.Вставить("ОписаниеКС"			, 
		...
		НСтр("ru='""Сумма налога исчисленная"" Раздела 2 Приложения № 1 к 6-НДФЛ <= 1 руб. '") +
		НСтр("ru='(соотношение применяется к 6-НДФЛ за налоговый период)'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru='%1 = %2'"));
	Заголовки.Вставить("ОписаниеОшибки"		, НСтр("ru='Ошибочная сумма дохода'"));
	Настройки = ОбработатьНастройки(		, Заголовки);
...
Процедура ПроверкаКС_1_22_доп()
	...
	Для каждого СтраницаРаздела Из ДанныеКС.Раздел1 Цикл
		...
		Настройки.Расшифровка = ПодставитьПараметрыВСтроку(Настройки.ШаблонРасшифровки, Показатели.Р01С020.Значение, 
...
Процедура ПроверкаКС_1_24_доп()
	...
	СформироватьПоказатели(Показатели, ИменаПоказателей);
	Список	= ДанныеКС.СоответствиеСписков.Получить(Показатели.Р02С105.ИмяПоказателя);
	...
	Если НЕ Список = Неопределено Тогда
		Для каждого СтраницаРаздела Из ДанныеКС.Раздел2 Цикл
			...
			Настройки.ЭтоОшибка = Список.Получить(Показатели.Р02С105.Значение) = Неопределено;
...
Процедура ПроверкаКС_1_25_доп()
	...
	Для каждого СтраницаРаздела Из ДанныеКС.Раздел2 Цикл
		...
		РасчетнаяСумма = СуммаПоказателей(Показатели) - Показатели.Р02С110.Значение;
		Настройки.ЭтоОшибка = НЕ Показатели.Р02С110.Значение >= РасчетнаяСумма;
		Настройки.Расшифровка = ПодставитьПараметрыВСтроку(Настройки.ШаблонРасшифровки, Показатели.Р02С110.Значение,
			РасчетнаяСумма);
		ДополнитьСтруктуруОшибок(Настройки);
