...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.СобытиеНаступило("Отпр.дв.ден.б.за пр.20к4") Тогда
...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	Если СтруктураРеквизитовФормы.мСохраненныйДок = Неопределено Тогда
		...
		Иначе
			...
			СтруктураРеквизитовФормы.Вставить("мПечатьБезШтрихкодаРазрешена", Истина);
...
Процедура Выгрузить(Команда)
	...
	Если НЕ ДвижениеСредствПоСчетуВБанкеЗаПределамиРФВНовойРедакцииПредставляетсяВЭлектронномВиде() Тогда
...
&НаСервере
Функция ДвижениеСредствПоСчетуВБанкеЗаПределамиРФВНовойРедакцииПредставляетсяВЭлектронномВиде()
	
	Возврат ТекущаяДатаСеанса() >= '20210415';
