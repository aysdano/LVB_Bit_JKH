...
Функция СформироватьРядыБлоков(ВключенныеЗадачи, НалоговыеИндикаторы, ОтчетныеИндикаторы) Экспорт
	...
	МассивЗадачТребущихПомощника = ОтчетныеИндикаторы.НайтиСтроки(ОтборНуженПомощникОтчетовПрошлыхПериодов);
	...
	Для Каждого СтрокаЗадачи Из ВключенныеЗадачи Цикл
		...
		Если СтрокаЗадачи.КодЗадачи = ЗадачиБухгалтераКлиентСервер.КодЗадачиУСН()
			...
			Для Каждого ЗаписьМассива ИЗ МассивЗадачТребущихПомощника Цикл
				Если ЗаписьМассива.Владелец.Код = СтрокаЗадачи.КодЗадачи Тогда
...
Функция СформироватьТекущиеПредстоящиеЗадачи(Организация, ТекущийДень, ДеятельностьОтсутствует)
	...
	|	ВТПолучателиОтчета.ТипПолучателя КАК Получатель
	|ИЗ
	...
	|				ИЛИ ВТПолучателиОтчета.ИдентификаторПравила = """"
...
Процедура ПрименитьАлгоритмДляВзносовИПСДоходов(НалоговыеИндикаторы, ЗадачиПоНалогам, Организация, ТекущийДень, ИндикаторВзносовИП, СчетаУчетаДС)
	...
	Иначе
		// Отсутствует задача для постоянной части взносов ИП.
		// Здесь, вероятнее всего, критичная ошибка в наполнении НСИ списка задач.
		// Если есть задача по взносам ИП с доходов, должна быть и задача для постоянной части взносов, и индикатор для нее.
		ТекстСообщения
			= НСтр("ru = 'Служебные данные списка задач заполнены некорректно или повреждены:
				|для задачи ""взносы за себя"" не подобрано правило уплаты фиксированных страховых взносов.
				|Рекомендуется проверить заполнение справочника ""Правила представления отчетов, уплаты налогов"".'");
...
//
Процедура ПрименитьАлгоритмПоОтчетам(ОтчетныеИндикаторы, ЗадачиПоОтчетам, Организация, ТекущийДень, ЭтоНулевка, ДатаНачалаДеятельности)
