...
Функция ПолучитьТаблицуОсобыхПолей()
	...
	НовСтр.ПутьXML = "Файл/Документ/ЗаявГибТС/СведТС/ДатаГибТС";
...
Процедура ОсобаяОбработкаЭлементов(Путь, СтрЗагружаемоеЗначение, Страница) Экспорт 
	Если Путь = "Файл/Документ/ЗаявГибТС/СведТС/ДатаГибТС" Тогда
		Страница.ДатаГибелиТС = РегламентированнаяОтчетностьЗагрузка.ДатаИзСтрокиЛюбогоФормата("01." + СтрЗагружаемоеЗначение.ЗначениеЭлемента);
	КонецЕсли;
КонецПроцедуры
