...
Процедура ЗаполнитьФормаОтчета2011Кв4_Пояснения5Строки5510и5530(КонтекстВыполнения, НомерСтроки)
	...
	// БИТ_ЖКХ 05.04.2016 11:51:57 Начало правки... 
	СчетаРасчетов = СчетаРасчетов + ", 76.КУ";
	// БИТ_ЖКХ 05.04.2016 11:52:01 Конец правки.
	СчетаАвансов  = "60.02, 60.22, 60.32";
	СчетаНДСАвансов = "76.ВА";
	...
	// БИТ_ЖКХ 04.07.2018 12:11:03 Начало правки...
	СчетаПеревода = СчетаПеревода + ", 76.КУ";
	// БИТ_ЖКХ 04.07.2018 12:11:03 Конец правки.
	...
	// БИТ_ЖКХ 05.04.2016 11:57:51 Начало правки... 
	// БИТ_ЖКХ добавление кода. Начало. 12.11.2012{{
	СчетаГрафы09 = СчетаГрафы09 + ", 76.КУ";
	// БИТ_ЖКХ добавление кода. Конец. 12.11.2012}}
	// БИТ_ЖКХ 05.04.2016 11:57:54 Конец правки. 
...
Процедура ЗаполнитьФормаОтчета2011Кв4_Пояснения5Строки5511и5531(КонтекстВыполнения, НомерСтроки)
	...
	СчетаРасчетов = "46, 62.01, 62.21, 62.31, 62.03, 76.06, 76.26, 76.36";
	// БИТ_ЖКХ 09.09.2015 14:10:37 Начало правки... 
	СчетаРасчетов = СчетаРасчетов + ", 76.КУ";
	// БИТ_ЖКХ 09.09.2015 14:10:41 Конец правки. 
	СчетаРезервов = "63";
	СчетаПеревода = "76.06, 76.26, 76.36";
	// БИТ_ЖКХ 03.07.2018 18:38:02 Начало правки...
	СчетаПеревода = СчетаПеревода + ", 76.КУ"; 
	// БИТ_ЖКХ 03.07.2018 18:38:02 Конец правки. 
	...
	// БИТ_ЖКХ 05.04.2016 13:18:16 Начало правки...
	// БИТ_ЖКХ добавление кода. Начало. 12.11.2012{{
	//СчетаРасчетов = СчетаРасчетов +", 76.КУ";
	СчетаГрафы09 = СчетаГрафы09 + ", 76.КУ";
	// БИТ_ЖКХ добавление кода. Конец. 12.11.2012}}
	// БИТ_ЖКХ 05.04.2016 13:18:16 Конец правки.
...
Процедура ЗаполнитьФормаОтчета2011Кв4_Пояснения51_Краткосрочная(КонтекстВыполнения)
	...
	Запрос = Новый Запрос;
	Запрос.МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;
	// БИТ_ЖКХ 05.04.2016 11:48:54 Начало правки... 
	// БИТ_ЖКХ добавление кода. Начало. 12.11.2012{{
	СчетаПоЛицевымСчетам = Новый Массив;
	СчетаПоЛицевымСчетам.Добавить(Счет("76.КУ"));
	СубконтоПоЛицевымСчетам = ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.ЛицевыеСчета;
	
	Запрос.УстановитьПараметр("СчетаПоЛицевымСчетам"   , СчетаПоЛицевымСчетам);
	Запрос.УстановитьПараметр("СубконтоПоЛицевымСчетам", СубконтоПоЛицевымСчетам);
	// БИТ_ЖКХ добавление кода. Конец. 12.11.2012}}
	// БИТ_ЖКХ 05.04.2016 11:48:58 Конец правки. 
	...
	Пока Выборка.Следующий() Цикл
		...
		// БИТ_ЖКХ 04.07.2018 9:56:29 Начало правки...
		//Если ОбработкаСчетов.АПСчетаПоСрочности.Найти(СчетСоставляющей) <> Неопределено Тогда
		Если СчетаПоЛицевымСчетам.Найти(СчетСоставляющей) <> Неопределено Тогда
			НаименованиеСоставляющей = "Развернутое сальдо на начало периода по дебету счета " + СчетСоставляющей.Код + ", по субконто """ + СубконтоПоЛицевымСчетам + """";
		ИначеЕсли ОбработкаСчетов.АПСчетаПоСрочности.Найти(СчетСоставляющей) <> Неопределено Тогда
		// БИТ_ЖКХ 04.07.2018 9:56:31 Конец правки.а
	...
	Пока Выборка.Следующий() Цикл
		...
		//Если ОбработкаСчетов.АПСчетаПоСрочности.Найти(СчетСоставляющей) <> Неопределено Тогда
		Если СчетаПоЛицевымСчетам.Найти(СчетСоставляющей) <> Неопределено Тогда
			НаименованиеСоставляющей = "Развернутое сальдо на конец периода по дебету счета " + СчетСоставляющей.Код + ", по субконто """ + СубконтоПоЛицевымСчетам + """";
		ИначеЕсли ОбработкаСчетов.АПСчетаПоСрочности.Найти(СчетСоставляющей) <> Неопределено Тогда
		// БИТ_ЖКХ 04.07.2018 9:56:31 Конец правки. 
	...
	Пока Выборка.Следующий() Цикл
	    ...
	    //Если ОбработкаСчетов.АПСчетаПоСрочности.Найти(СчетСоставляющей) <> Неопределено Тогда
		Если СчетаПоЛицевымСчетам.Найти(СчетСоставляющей) <> Неопределено Тогда
			НаименованиеСоставляющей = "Обороты по дебету счета " + СчетСоставляющей.Код + ", отражаемые в дебиторской задолженности, развернутый по субконто """ + СубконтоПоЛицевымСчетам + """";
		ИначеЕсли ОбработкаСчетов.АПСчетаПоСрочности.Найти(СчетСоставляющей) <> Неопределено Тогда
		// БИТ_ЖКХ 04.07.2018 9:56:31 Конец правки.
	...
	Пока Выборка.Следующий() Цикл
		...
		//Если ОбработкаСчетов.АПСчетаПоСрочности.Найти(СчетСоставляющей) <> Неопределено Тогда
		Если СчетаПоЛицевымСчетам.Найти(СчетСоставляющей) <> Неопределено Тогда
			НаименованиеСоставляющей = "Обороты по кредиту счета " + СчетСоставляющей.Код + ", отражаемые в дебиторской задолженности, развернутый по субконто """ + СубконтоПоЛицевымСчетам + """";
		ИначеЕсли ОбработкаСчетов.АПСчетаПоСрочности.Найти(СчетСоставляющей) <> Неопределено Тогда
		// БИТ_ЖКХ 04.07.2018 9:56:31 Конец правки.
	...
	Пока Выборка.Следующий() Цикл
		...
		Если СчетаПоЛицевымСчетам.Найти(СчетСоставляющей) <> Неопределено Тогда
			НаименованиеСоставляющей = "Оборот с кредита счета " + СчетСоставляющейКт.Код + " развернутый по субконто """ + СубконтоПоЛицевымСчетам + """ в дебет счета "
			                         + СчетСоставляющейДт.Код + " по статьям субконто """ + СубконтоПрочиеДоходыРасходы
			                         + """ с видом """ + ВидПДРСписание + """";
		ИначеЕсли ОбработкаСчетов.АПСчетаПоСрочности.Найти(СчетСоставляющейКт) <> Неопределено
		// БИТ_ЖКХ 04.07.2018 9:56:31 Конец правки.
	...
	Пока Выборка.Следующий() Цикл
		...
		//Если ОбработкаСчетов.АПСчетаПоСрочности.Найти(СчетСоставляющейДт) <> Неопределено
		Если СчетаПоЛицевымСчетам.Найти(СчетСоставляющей) <> Неопределено Тогда
			НаименованиеСоставляющей = "Оборот с дебета счета " + СчетСоставляющейДт.Код + " развернутый по субконто """ + СубконтоПоЛицевымСчетам + """ в кредит счета "
			                         + СчетСоставляющейКт.Код + " по статьям субконто """ + СубконтоПрочиеДоходыРасходы
			                         + """ с видом """ + ВидПДРНачисление + """";
		ИначеЕсли ОбработкаСчетов.АПСчетаПоСрочности.Найти(СчетСоставляющейДт) <> Неопределено
		// БИТ_ЖКХ 04.07.2018 9:56:31 Конец правки.
		 ИЛИ ОбработкаСчетов.ОсобыеСчетаПоСрочности.Найти(СчетСоставляющейДт) <> Неопределено
		 ИЛИ ОбработкаСчетов.СчетаПоСрочности.Найти(СчетСоставляющейДт) <> Неопределено Тогда
			НаименованиеСоставляющей = "Оборот с дебета счета " + СчетСоставляющейДт.Код + " по контрагентам и договорам со сроком оплаты не превышающим 365 дней в кредит счета "
	...
	Пока Выборка.Следующий() Цикл
		...
		//Если ОбработкаСчетов.АПСчетаПоСрочности.Найти(СчетСоставляющей) <> Неопределено Тогда
		Если СчетаПоЛицевымСчетам.Найти(СчетСоставляющей) <> Неопределено Тогда
			НаименованиеСоставляющей = "Переведено из дебиторской в кредиторскую задолженность по счету " + СчетСоставляющей.Код + " по субконто """ + СубконтоПоЛицевымСчетам + """";
		ИначеЕсли ОбработкаСчетов.АПСчетаПоСрочности.Найти(СчетСоставляющей) <> Неопределено Тогда
		// БИТ_ЖКХ 04.07.2018 9:56:31 Конец правки. 
	...
	Пока Выборка.Следующий() Цикл
		...
		//Если ОбработкаСчетов.АПСчетаПоСрочности.Найти(СчетСоставляющей) <> Неопределено Тогда
		Если СчетаПоЛицевымСчетам.Найти(СчетСоставляющей) <> Неопределено Тогда
			НаименованиеСоставляющей = "Переведено из кредиторской в дебиторскую задолженность по счету " + СчетСоставляющей.Код + " по субконто """ + СубконтоПоЛицевымСчетам + """";
		ИначеЕсли ОбработкаСчетов.АПСчетаПоСрочности.Найти(СчетСоставляющей) <> Неопределено Тогда
		// БИТ_ЖКХ 04.07.2018 9:56:31 Конец правки. ь по счету " + СчетСоставляющей.Код + " по контрагентам и договорам со сроком оплаты не превышающим 365 дней";
...
Функция ТекстЗапросаПояснения51_Краткосрочная() Экспорт
	...
	ТекстЗапроса = "ВЫБРАТЬ
	               ...
	               |	Договор
	               |;
				   // БИТ_ЖКХ 03.07.2018 17:40:34 Начало правки...	             				   |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
	               |	ХозрасчетныйОстаткиИОбороты.Счет КАК Счет,
				   |	ВЫРАЗИТЬ(ХозрасчетныйОстаткиИОбороты.Субконто1 КАК Справочник.бит_ЛицевыеСчета) КАК ЛицевойСчет,
	               |	ХозрасчетныйОстаткиИОбороты.СуммаНачальныйОстатокДт КАК СНД,
	               |	ХозрасчетныйОстаткиИОбороты.СуммаНачальныйОстатокКт КАК СНК,
	               |	ХозрасчетныйОстаткиИОбороты.СуммаОборотДт КАК ДО,
	               |	ХозрасчетныйОстаткиИОбороты.СуммаОборотКт КАК КО,
	               ...
	               |		ИНАЧЕ 0
	               |	КОНЕЦ КАК ПереводКвД
	               |ПОМЕСТИТЬ ВТ_ВсеДанныеСчетовПоЛицевымСчетам
	               |ИЗ
	               |	РегистрБухгалтерии.Хозрасчетный.ОстаткиИОбороты(&НачалоПериода, &КонецПериода, Период, , Счет В (&СчетаПоЛицевымСчетам), &СубконтоПоЛицевымСчетам, Организация = &Организация) КАК ХозрасчетныйОстаткиИОбороты
	               |;
	               |
				   |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
				   |	ВТ_ВсеДанныеСчетовПоЛицевымСчетам.Счет КАК Счет,
	               |	ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка) КАК Контрагент,
	               |	ЗНАЧЕНИЕ(Справочник.ДоговорыКонтрагентов.ПустаяСсылка) КАК Договор,
	               |	ВТ_ВсеДанныеСчетовПоЛицевымСчетам.СНД КАК СНД,
	               |	ВТ_ВсеДанныеСчетовПоЛицевымСчетам.СНК КАК СНК,
	               |	ВЫБОР
	               |		КОГДА ВТ_ВсеДанныеСчетовПоЛицевымСчетам.ОтражатьВДебиторской
	               |			ТОГДА ВТ_ВсеДанныеСчетовПоЛицевымСчетам.ДО
	               |		ИНАЧЕ 0
	               |	КОНЕЦ КАК ДО,
	               |	ВЫБОР
	               |		КОГДА ВТ_ВсеДанныеСчетовПоЛицевымСчетам.ОтражатьВДебиторской
	               |			ТОГДА ВТ_ВсеДанныеСчетовПоЛицевымСчетам.КО
	               |		ИНАЧЕ 0
	               |	КОНЕЦ КАК КО,
	               |	ВТ_ВсеДанныеСчетовПоЛицевымСчетам.СКД КАК СКД,
	               |	ВТ_ВсеДанныеСчетовПоЛицевымСчетам.СКК КАК СКК,
	               |	ВЫБОР
	               |		КОГДА ВТ_ВсеДанныеСчетовПоЛицевымСчетам.ОтражатьВДебиторской
	               |			ТОГДА ВТ_ВсеДанныеСчетовПоЛицевымСчетам.Дельта
	               |		ИНАЧЕ 0
	               |	КОНЕЦ КАК Дельта,
	               |	ВТ_ВсеДанныеСчетовПоЛицевымСчетам.ПереводДвК КАК ПереводДвК,
	               |	ВТ_ВсеДанныеСчетовПоЛицевымСчетам.ПереводКвД КАК ПереводКвД
	               |ПОМЕСТИТЬ ВТ_ДанныеСчетовПоЛицевымСчетам
	               |ИЗ
	               |	ВТ_ВсеДанныеСчетовПоЛицевымСчетам КАК ВТ_ВсеДанныеСчетовПоЛицевымСчетам
				   // БИТ_ЖКХ 03.07.2018 17:40:34 Конец правки.
	               |;	                              
				   |
	               |
	               |////////////////////////////////////////////////////////////////////////////////
	               ...
	               // БИТ_ЖКХ 03.07.2018 18:20:11 Начало правки... 
				   |
	               |	ОБЪЕДИНИТЬ ВСЕ
				   |
	               |ВЫБРАТЬ
	               |	ВТ_ДанныеСчетовПоЛицевымСчетам.Счет,
	               |	ВТ_ДанныеСчетовПоЛицевымСчетам.Контрагент,
	               |	ВТ_ДанныеСчетовПоЛицевымСчетам.Договор,
	               |	ВТ_ДанныеСчетовПоЛицевымСчетам.СНД,
	               |	ВТ_ДанныеСчетовПоЛицевымСчетам.СНК,
	               |	ВТ_ДанныеСчетовПоЛицевымСчетам.ДО,
	               |	ВТ_ДанныеСчетовПоЛицевымСчетам.КО,
	               |	ВТ_ДанныеСчетовПоЛицевымСчетам.СКД,
	               |	ВТ_ДанныеСчетовПоЛицевымСчетам.СКК,
	               |	ВТ_ДанныеСчетовПоЛицевымСчетам.Дельта,
	               |	ВТ_ДанныеСчетовПоЛицевымСчетам.ПереводДвК,
	               |	ВТ_ДанныеСчетовПоЛицевымСчетам.ПереводКвД
	               |ИЗ
	               |	ВТ_ДанныеСчетовПоЛицевымСчетам КАК ВТ_ДанныеСчетовПоЛицевымСчетам
				   // БИТ_ЖКХ 03.07.2018 18:20:15 Конец правки. 
				   |;
	               |
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
...
Процедура ЗаполнитьФормаОтчета2011Кв4_Пояснения5Строки5560и5580(КонтекстВыполнения, НомерСтроки)
	...
	// БИТ_ЖКХ 04.07.2018 12:19:24 Начало правки...
	СчетаПеревода = СчетаПеревода + ", 76.КУ";
	// БИТ_ЖКХ 04.07.2018 12:19:24 Конец правки.
...
Процедура ЗаполнитьФормаОтчета2011Кв4_Пояснения5Строки5562и5582(КонтекстВыполнения, НомерСтроки)
	...
	// БИТ_ЖКХ 03.07.2018 18:46:21 Начало правки...
	СчетаАвансов = СчетаАвансов + ", 76.КУ";
	// БИТ_ЖКХ 03.07.2018 18:46:21 Конец правки.
	СчетаНДСАвансов = "76.АВ";
	СчетаПеревода = "76.06, 76.26, 76.36";
	// БИТ_ЖКХ 03.07.2018 18:46:21 Начало правки...
	СчетаПеревода = СчетаПеревода + ", 76.КУ";
...
Процедура ЗаполнитьФормаОтчета2011Кв4_Пояснения53_Краткосрочная(КонтекстВыполнения)
	...
	Запрос = Новый Запрос;
	Запрос.МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;
	// БИТ_ЖКХ 03.07.2018 18:47:51 Начало правки... 
	СчетаПоЛицевымСчетам = Новый Массив;
	СчетаПоЛицевымСчетам.Добавить(Счет("76.КУ"));
	СубконтоПоЛицевымСчетам = ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.ЛицевыеСчета;
	...
	Запрос.УстановитьПараметр("СчетаПоЛицевымСчетам"   , СчетаПоЛицевымСчетам);
	Запрос.УстановитьПараметр("СубконтоПоЛицевымСчетам", СубконтоПоЛицевымСчетам);
	// БИТ_ЖКХ 03.07.2018 18:47:54 Конец правки. 
	...
	Пока Выборка.Следующий() Цикл
		...
		//Если ОбработкаСчетов.АПСчетаПоСрочности.Найти(СчетСоставляющей) <> Неопределено Тогда
		Если СчетаПоЛицевымСчетам.Найти(СчетСоставляющей) <> Неопределено Тогда
			НаименованиеСоставляющей = "Развернутое сальдо на начало периода по кредиту счета " + СчетСоставляющей.Код + ", по субконто """ + СубконтоПоЛицевымСчетам + """";
		ИначеЕсли ОбработкаСчетов.АПСчетаПоСрочности.Найти(СчетСоставляющей) <> Неопределено Тогда
		// БИТ_ЖКХ 04.07.2018 9:56:31 Конец правки.
	...
	Пока Выборка.Следующий() Цикл
		...
		//Если ОбработкаСчетов.АПСчетаПоСрочности.Найти(СчетСоставляющей) <> Неопределено Тогда
		Если СчетаПоЛицевымСчетам.Найти(СчетСоставляющей) <> Неопределено Тогда
			НаименованиеСоставляющей = "Развернутое сальдо на конец периода по кредиту счета " + СчетСоставляющей.Код + ", по субконто """ + СубконтоПоЛицевымСчетам + """";
		ИначеЕсли ОбработкаСчетов.АПСчетаПоСрочности.Найти(СчетСоставляющей) <> Неопределено Тогда
		// БИТ_ЖКХ 04.07.2018 9:56:31 Конец правки.
	...
	Пока Выборка.Следующий() Цикл
		...
		//Если ОбработкаСчетов.АПСчетаПоСрочности.Найти(СчетСоставляющей) <> Неопределено Тогда
		Если СчетаПоЛицевымСчетам.Найти(СчетСоставляющей) <> Неопределено Тогда
			НаименованиеСоставляющей = "Обороты по кредиту счета " + СчетСоставляющей.Код + ", отражаемые в кредиторской задолженности, развернутый по субконто """ + СубконтоПоЛицевымСчетам + """";
		ИначеЕсли ОбработкаСчетов.АПСчетаПоСрочности.Найти(СчетСоставляющей) <> Неопределено Тогда
		// БИТ_ЖКХ 04.07.2018 9:56:31 Конец правки.
	...
	Пока Выборка.Следующий() Цикл
		...
		//Если ОбработкаСчетов.АПСчетаПоСрочности.Найти(СчетСоставляющей) <> Неопределено Тогда
		Если СчетаПоЛицевымСчетам.Найти(СчетСоставляющей) <> Неопределено Тогда
			НаименованиеСоставляющей = "Обороты по дебету счета " + СчетСоставляющей.Код + ", отражаемые в кредиторской задолженности, развернутый по субконто """ + СубконтоПоЛицевымСчетам + """";
		ИначеЕсли ОбработкаСчетов.АПСчетаПоСрочности.Найти(СчетСоставляющей) <> Неопределено Тогда
		// БИТ_ЖКХ 04.07.2018 9:56:31 Конец правки.
	...
	Пока Выборка.Следующий() Цикл
		...
		Если СчетаПоЛицевымСчетам.Найти(СчетСоставляющей) <> Неопределено Тогда
			НаименованиеСоставляющей = "Переведено из дебиторской в кредиторскую задолженность по счету " + СчетСоставляющей.Код + " по субконто """ + СубконтоПоЛицевымСчетам + """";
		ИначеЕсли ОбработкаСчетов.АПСчетаПоСрочности.Найти(СчетСоставляющей) <> Неопределено
		// БИТ_ЖКХ 04.07.2018 9:56:31 Конец правки. 
		 ИЛИ ОбработкаСчетов.ОсобыеСчетаПоСрочности.Найти(СчетСоставляющей) <> Неопределено
		 ИЛИ ОбработкаСчетов.СчетаПоСрочности.Найти(СчетСоставляющей) <> Неопределено Тогда
			НаименованиеСоставляющей = "Переведено из дебиторской в кредиторскую задолженность по счету " + СчетСоставляющей.Код + " по контрагентам и договорам со сроком оплаты не превышающим 365 дней";
...
Функция ТекстЗапросаПояснения53_Краткосрочная() Экспорт
	...
	ТекстЗапроса = "ВЫБРАТЬ
	               ...
	               |	Договор
	               |;
				   // БИТ_ЖКХ 03.07.2018 18:51:45 Начало правки... 
				   |
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
	               |	ХозрасчетныйОстаткиИОбороты.Счет КАК Счет,
	               ...
	               |	ВТ_ВсеДанныеСчетовПоЛицевымСчетам.СНК КАК СНК,
	               |	ВЫБОР
	               |		КОГДА ВТ_ВсеДанныеСчетовПоЛицевымСчетам.ОтражатьВКредиторской
	               |			ТОГДА ВТ_ВсеДанныеСчетовПоЛицевымСчетам.ДО
	               |		ИНАЧЕ 0
	               |	КОНЕЦ КАК ДО,
	               |	ВЫБОР
	               ...
	               |ИЗ
	               |	ВТ_ВсеДанныеСчетовПоЛицевымСчетам КАК ВТ_ВсеДанныеСчетовПоЛицевымСчетам
				   // БИТ_ЖКХ 03.07.2018 18:51:52 Конец правки. 
	               |;
	               |
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
	               ...
	               |ГДЕ
	               |	ВТ_ДолгосрочныеДоговоры.Договор ЕСТЬ NULL
	               | // БИТ_ЖКХ 03.07.2018 18:57:37 Начало правки...
				   |
	               |	ОБЪЕДИНИТЬ ВСЕ
				   |
	               |ВЫБРАТЬ
	               ...
	               |ИЗ
	               |	ВТ_ДанныеСчетовПоЛицевымСчетам КАК ВТ_ДанныеСчетовПоЛицевымСчетам
				   // БИТ_ЖКХ 03.07.2018 18:57:37 Конец правки.
				   |;
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
	               |	ХозрасчетныйОстаткиИОбороты.Счет КАК Счет,
...
Функция СпособыОбработкиСчетовПояснений5(ПараметрыОбработкиСчетов)
	...
	// БИТ_ЖКХ 05.04.2016 13:20:47 Начало правки... 
	СпособыОбработкиСчетовПояснений5.Вставить("76.КУ", "АПСчетаПоСрочности");
	// БИТ_ЖКХ 05.04.2016 13:20:51 Конец правки. 
	...
	//// БИТ_ЖКХ 05.04.2016 13:20:47 Начало правки... 
	//СпособыОбработкиСчетовПояснений5.Вставить("76.КУ", "СчетаБезОбработки");
	//// БИТ_ЖКХ 05.04.2016 13:20:51 Конец правки.
