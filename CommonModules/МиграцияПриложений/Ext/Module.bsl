...
Процедура ЗаданиеВыгрузка(ЗавершитьМиграцию = Ложь) Экспорт
	...
	Если СостояниеЗагрузки.ВерсияКонфигурации <> бит_ОбщегоНазначенияПовтИсп.ПолучитьНомерРелиза_1С() Тогда
...
Функция СоздатьПриложениеДляМиграции(ПараметрыДоступа, Наименование, ЧасовойПояс, ПраваПользователей) Экспорт
	...
	ПараметрыМетода.Вставить("version", бит_ОбщегоНазначенияПовтИсп.ПолучитьНомерРелиза_1С());
...
Функция ЗагрузитьДанныеИзКаталога(КаталогЗагрузки, СоответствиеСсылок, ОбъектыДляОчистки)
	...
	Если Манифест.ВерсияКонфигурации <> бит_ОбщегоНазначенияПовтИсп.ПолучитьНомерРелиза_1С() Тогда
...
Функция ВыгрузитьДанныеВКаталог(КаталогВыгрузки, УзелОбмена, НомерСообщения, ПоследнийОбъектМетаданных, ВыгружатьНастройкиПользователей, ЗавершитьМиграциюАвтоматически)
	...
	Манифест.Вставить("ВерсияКонфигурации", бит_ОбщегоНазначенияПовтИсп.ПолучитьНомерРелиза_1С());
...
Функция СостояниеЗагрузки()
	...
	Статус.Вставить("ВерсияКонфигурации", бит_ОбщегоНазначенияПовтИсп.ПолучитьНомерРелиза_1С());
