...
&НаКлиенте
Процедура КартинкаЗакрытьБаннерОтменаДекларации_ТранспортныйНалогНажатие(Элемент)
	СкрытьБаннерИзмененияПоНалогу(Элемент.Имя);
КонецПроцедуры

&НаКлиенте
Процедура КартинкаЗакрытьБаннерИзменениеСроков_ТранспортныйНалогНажатие(Элемент)
	СкрытьБаннерИзмененияПоНалогу(Элемент.Имя);
КонецПроцедуры

&НаКлиенте
Процедура КартинкаЗакрытьБаннерТранспортныйНалогНажатие(Элемент)
	
	СкрытьБаннерОтменаДекларацииПоТранспортномуЗемельномуНалогу("ТранспортныйНалог");
...
&НаКлиенте
Процедура КартинкаЗакрытьБаннерОтменаДекларации_ЗемельныйНалогНажатие(Элемент)
	СкрытьБаннерИзмененияПоНалогу(Элемент.Имя);
КонецПроцедуры

&НаКлиенте
Процедура КартинкаЗакрытьБаннерИзменениеСроков_ЗемельныйНалогНажатие(Элемент)
	СкрытьБаннерИзмененияПоНалогу(Элемент.Имя);
КонецПроцедуры

&НаКлиенте
Процедура КартинкаЗакрытьБаннерЗемельныйНалогНажатие(Элемент)
	
	СкрытьБаннерОтменаДекларацииПоТранспортномуЗемельномуНалогу("ЗемельныйНалог");	
...
Процедура ПодготовитьФормуНаСервере()
	...
	Если НастройкиСистемыНалогообложения.ПрименяетсяУСН
		...
		НастройкиУчетаУСНФормы.ПодготовитьФормуНаСервере(ЭтотОбъект, СписокНедействующихНастроек);
	КонецЕсли;
...
Процедура УправлениеФормой(Форма, ИмяРаздела = Неопределено)
	...
	Если ИмяРаздела = Неопределено Или ИмяРаздела = "ТранспортныйНалог" Тогда
		Элементы.ГруппаБаннерОтменаДекларации_ТранспортныйНалог.Видимость = ВидимостьБаннераОтменаДекларацииПоНалогу("ТранспортныйНалог");
		Элементы.ГруппаБаннерИзменениеСроков_ТранспортныйНалог.Видимость = ВидимостьБаннераИзменениеСроковУплатыНалога("ТранспортныйНалог");
	КонецЕсли;
	
	Если ИмяРаздела = Неопределено Или ИмяРаздела = "ЗемельныйНалог" Тогда
		Элементы.ГруппаБаннерОтменаДекларации_ЗемельныйНалог.Видимость = ВидимостьБаннераОтменаДекларацииПоНалогу("ЗемельныйНалог");
		Элементы.ГруппаБаннерИзменениеСроков_ЗемельныйНалог.Видимость = ВидимостьБаннераИзменениеСроковУплатыНалога("ЗемельныйНалог");
	КонецЕсли;
...
Процедура НастроитьБаннерыИзмененияПоНалогам()
	...
	// Транспортный налог
	ТекстБаннераОтменаДекларации_ТранспортныйНалог = 
		УправлениеВнеоборотнымиАктивами.ТекстБаннераОтменаДекларацииПоНалогу("ТранспортныйНалог");
	ТекстБаннераИзменениеСроковУплаты_ТранспортныйНалог = 
		УправлениеВнеоборотнымиАктивами.ТекстБаннераИзменениеСроковУплатыНалога("ТранспортныйНалог");
		
	Элементы.ГруппаБаннерОтменаДекларации_ТранспортныйНалог.Видимость = ВидимостьБаннераОтменаДекларацииПоНалогу("ТранспортныйНалог");
	Элементы.ГруппаБаннерИзменениеСроков_ТранспортныйНалог.Видимость = ВидимостьБаннераИзменениеСроковУплатыНалога("ТранспортныйНалог");
	
	// Земельный налог
	ТекстБаннераОтменаДекларации_ЗемельныйНалог = 
		УправлениеВнеоборотнымиАктивами.ТекстБаннераОтменаДекларацииПоНалогу("ЗемельныйНалог");
	ТекстБаннераИзменениеСроковУплаты_ЗемельныйНалог = 
		УправлениеВнеоборотнымиАктивами.ТекстБаннераИзменениеСроковУплатыНалога("ЗемельныйНалог");
...
Процедура СкрытьБаннерИзмененияПоНалогу(Знач ИмяЭлемента)
	...
	Если СтрНайти(ИмяЭлемента, "КартинкаЗакрытьБаннер") = 0 Тогда
		Возврат;
	КонецЕсли;
	
	ИмяЭлемента = СтрЗаменить(ИмяЭлемента, "КартинкаЗакрытьБаннер", "");
	ЧастиИмени = СтрРазделить(ИмяЭлемента, "_", Ложь);
	...
	Если Тема = "ОтменаДекларации" Тогда
		УправлениеВнеоборотнымиАктивами.СохранитьНастройкуБаннераОтменаДекларацииПоНалогу(Налог, Ложь);
		Элементы["ГруппаБаннерОтменаДекларации_" + Налог].Видимость = ВидимостьБаннераОтменаДекларацииПоНалогу(Налог);
	ИначеЕсли Тема = "ИзменениеСроков" Тогда	
		УправлениеВнеоборотнымиАктивами.СохранитьНастройкуБаннераИзменениеСроковУплатыНалога(Налог, Ложь);
		Элементы["ГруппаБаннерИзменениеСроков_" + Налог].Видимость = ВидимостьБаннераИзменениеСроковУплатыНалога(Налог);
	КонецЕсли;	
				
КонецПроцедуры

&НаСервереБезКонтекста
Функция ВидимостьБаннераОтменаДекларацииПоНалогу(Налог)

	Возврат УправлениеВнеоборотнымиАктивами.ВидимостьБаннераОтменаДекларацииПоНалогу(Налог);

КонецФункции 

&НаСервереБезКонтекста
Функция ВидимостьБаннераИзменениеСроковУплатыНалога(Налог)

	Возврат УправлениеВнеоборотнымиАктивами.ВидимостьБаннераИзменениеСроковУплатыНалога(Налог);
...
&НаСервере
Процедура НастроитьБаннерОтменаДекларацииПоТранспортномуЗемельномуНалогу()
	
	ТекстБаннераТранспортныйНалог = 
		УправлениеВнеоборотнымиАктивами.ТекстБаннераОтменаДекларацииПоТранспортномуЗемельномуНалогу("ТранспортныйНалог");
	ТекстБаннераЗемельныйНалог = 
		УправлениеВнеоборотнымиАктивами.ТекстБаннераОтменаДекларацииПоТранспортномуЗемельномуНалогу("ЗемельныйНалог");
		
	Элементы.ГруппаБаннерТранспортныйНалог.Видимость = ВидимостьБаннераОтменаДекларацииПоТранспортномуЗемельномуНалогу("ТранспортныйНалог");
	Элементы.ГруппаБаннерЗемельныйНалог.Видимость = ВидимостьБаннераОтменаДекларацииПоТранспортномуЗемельномуНалогу("ЗемельныйНалог");
	
КонецПроцедуры

&НаСервере
Процедура СкрытьБаннерОтменаДекларацииПоТранспортномуЗемельномуНалогу(Налог)
	
	УправлениеВнеоборотнымиАктивами.СохранитьНастройкуБаннераОтменаДекларацииПоТранспортномуЗемельномуНалогу(Налог, Ложь);
	Элементы["ГруппаБаннер" + Налог].Видимость = ВидимостьБаннераОтменаДекларацииПоТранспортномуЗемельномуНалогу(Налог);
		
КонецПроцедуры

&НаСервереБезКонтекста
Функция ВидимостьБаннераОтменаДекларацииПоТранспортномуЗемельномуНалогу(Налог)

	Возврат УправлениеВнеоборотнымиАктивами.ВидимостьБаннераОтменаДекларацииПоТранспортномуЗемельномуНалогу(Налог);
