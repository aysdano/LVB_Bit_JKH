...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если СпособВыплатыПособия = Перечисления.СпособыВыплатыПособия.ЧерезБанк Тогда
		...
		Если Не РезультатПроверки.Успех Тогда
			ЗарплатаКадрыОтображениеОшибок.СообщитьОбОшибке(Отказ, РезультатПроверки.Пояснение, "НомерЛицевогоСчета");
		КонецЕсли;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ИзвещениеИзФССНомер) Или ЗначениеЗаполнено(ИзвещениеИзФССДата) Тогда
		...
		ИначеЕсли СтрДлина(ИзвещениеИзФССНомер) > 12 Тогда
			...
			СообщитьОбОшибкеВОбъекте(Отказ, "ИзвещениеИзФССНомер", Текст);
		Иначе
...
Процедура ПроверитьРеквизитыПолучателя(Отказ)
	...
	Если СпособВыплатыПособия = Перечисления.СпособыВыплатыПособия.ПочтовымПереводом Тогда
		...
		Если ЗначениеЗаполнено(ТекстОшибки) Тогда
			...
			ЗарплатаКадрыОтображениеОшибок.СообщитьОбОшибке(Отказ, ТекстОшибки, "АдресПолучателяФактический");
		КонецЕсли;
	КонецЕсли;
	Если Справочники.СтатусыНалогоплательщиковПоНДФЛ.ЭтоРезидент(СтатусНалогоплательщикаПолучателя) Тогда
		...
		Если ЗначениеЗаполнено(ТекстОшибки) Тогда
			...
			ЗарплатаКадрыОтображениеОшибок.СообщитьОбОшибке(Отказ, ТекстОшибки, "АдресПолучателяРегистрации");
		КонецЕсли;
	КонецЕсли;
...
Процедура ПроверитьКодУдостоверенияЛичности(Отказ, ROW)
	...
	Если ЭтоДокументНаПроживание Тогда
		...
		Если ДопустимыеВидыУдостоверенийЛичности.Найти(ВидУдостоверенияЛичности) = Неопределено Тогда
			...
			ЗарплатаКадрыОтображениеОшибок.СообщитьОбОшибке(Отказ, Текст, "ВидДокументаНаПроживание");
		КонецЕсли;
	Иначе
		...
		Если ДопустимыеВидыУдостоверенийЛичности.Найти(ВидУдостоверенияЛичности) = Неопределено Тогда
			...
			Текст = СтрШаблон(Текст, ВидУдостоверенияЛичности);
			ЗарплатаКадрыОтображениеОшибок.СообщитьОбОшибке(Отказ, Текст, "ВидДокументаУдостоверяющегоЛичностьПолучателя");
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
Процедура ПроверитьЗаполнениеРеквизитаОбъекта(Отказ, ИмяРеквизита, Знач Текст = "")
	СообщенияПользователюБЗК.ПроверитьЗаполнениеРеквизитаОбъекта(
		Отказ,
		ЭтотОбъект,
		ИмяРеквизита,
		Текст);
...
Процедура СообщитьОбОшибкеВОбъекте(Отказ, ИмяРеквизита, Текст)
...
Процедура ЗаполнитьДанныеОрганизации(ПараметрыФиксации, РеквизитыДокумента, РайонныйКоэффициентЗаполненИзОснования)
	...
	ИменаЗаполняемыхПолей = "НаименованиеОрганизации, НаименованиеТерриториальногоОрганаФСС,
		|УполномоченныйПоПрямымВыплатамФСС, ДолжностьУполномоченногоПоПрямымВыплатамФСС, ОснованиеПодписиУполномоченногоПоПрямымВыплатамФСС, ТелефонУполномоченногоПредставителя, АдресЭлектроннойПочтыСтрахователя";
	...
	ОргСведения.Свойство("АдресЭлектроннойПочтыОрганизации", РеквизитыДокумента.АдресЭлектроннойПочтыСтрахователя);
...
Функция ЗаполнитьКонтактыФизическогоЛица(ПараметрыФиксации)
	...
	ВладельцыКИ = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ФизическоеЛицо);
	ТаблицаКИ = УправлениеКонтактнойИнформацией.КонтактнаяИнформацияОбъектов(ВладельцыКИ, , ВидыКИ);
	
	Найденные = ТаблицаКИ.НайтиСтроки(Новый Структура("Вид", ВидАдресПоПрописке));
	Для Каждого СтрокаКИ Из Найденные Цикл
		Реквизиты.АдресПолучателяРегистрации = СтрокаКИ.Значение;
	КонецЦикла;
	
	Найденные = ТаблицаКИ.НайтиСтроки(Новый Структура("Вид", ВидТелефонДомашний));
	Для Каждого СтрокаКИ Из Найденные Цикл
		Реквизиты.ТелефонПолучателя = СтрокаКИ.Значение;
	КонецЦикла;
	
	Если Не ЗначениеЗаполнено(Реквизиты.ТелефонПолучателя) Тогда
		Найденные = ТаблицаКИ.НайтиСтроки(Новый Структура("Вид", ВидТелефонМобильный));
		Для Каждого СтрокаКИ Из Найденные Цикл
			Реквизиты.ТелефонПолучателя = СтрокаКИ.Значение;
		КонецЦикла;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ВидАдресаПочтовогоПеревода) Тогда
		Найденные = ТаблицаКИ.НайтиСтроки(Новый Структура("Вид", ВидАдресаПочтовогоПеревода));
		Для Каждого СтрокаКИ Из Найденные Цикл
...
Процедура ПроверитьОтсутствиеДваждыИзмененныхСведенийОЗастрахованномЛице(Отказ)
	...
	Если Не ЭтоНовый() И ПредшествующиеСведения.Ссылка = Ссылка Тогда
		...
		СообщитьОбОшибкеВОбъекте(Ложь, "Дата", Текст);  // Не останавливающее предупреждение, Отказ не включается.
		Возврат;
	КонецЕсли;
	...
	Текст = СтрШаблон(
		...
		СутьОтличийВСведенияхОЗастрахованномЛице(ПредшествующиеСведения),
		ВКакомОставитьФлажок,
		ВКакомОтключитьФлажок);
	ИмяРеквизита = "ИзмененыСведенияОЗастрахованномЛице";
	СообщенияПользователюБЗК.СообщитьОбОшибкеВОбъекте(Отказ, ПредшествующиеСведения.Ссылка, ИмяРеквизита, Текст);
КонецПроцедуры

Функция ПредшествующиеСведенияОЗастрахованномЛице()
	Если Не ЗначениеЗаполнено(ФизическоеЛицо) Или Не ЗначениеЗаполнено(Дата) Тогда
	...
	|	И Сведения.Дата < &НачалоСледующегоДня
	...
	Запрос.УстановитьПараметр("НачалоСледующегоДня", НачалоДня(Дата + 86400));
