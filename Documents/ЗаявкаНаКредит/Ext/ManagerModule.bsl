...
//
Функция ОборотыПоКредитамИЗаймам(Дата, Знач КоличествоМесяцев = 0, Организация = Неопределено) Экспорт
	...
	|ИЗ
	|	РегистрБухгалтерии.Хозрасчетный.Обороты(&НачалоПериода, &КонецПериода, Месяц, Счет В ИЕРАРХИИ (&СчетаРасчетовПоКредитамИЗаймам), , &ОтборПоОрганизации, , ) КАК ХозрасчетныйОбороты
	|
	|СГРУППИРОВАТЬ ПО
	|	ХозрасчетныйОбороты.Организация");
	
	СчетаРасчетовПоКредитамИЗаймам = Новый Массив;
	СчетаРасчетовПоКредитамИЗаймам.Добавить(ПланыСчетов.Хозрасчетный.РасчетыПоДолгосрочнымКредитамИЗаймам);
	СчетаРасчетовПоКредитамИЗаймам.Добавить(ПланыСчетов.Хозрасчетный.РасчетыПоКраткосрочнымКредитамИЗаймам);
	Запрос.УстановитьПараметр("СчетаРасчетовПоКредитамИЗаймам", СчетаРасчетовПоКредитамИЗаймам);
...
Процедура ПодготовитьФайлыДляСохранения(Параметры, АдресХранилища, АдресАрхива) Экспорт
	...
	УниверсальныйОбменСБанками.ПодготовитьАрхивыВЖурналеОперацийСФайлами(
		Результат.ИдентификаторВременногоХранилищаТранзакций, "", Новый Структура);
...
// Количество полных месяцев, включаемых в расчет показателей деятельности.
//
Функция ГлубинаРасчетаПоказателей()
...
Функция ПодготовитьФайлыДляОтправки_ЗаявкаНаКредит(ЗаявкаНаКредит, КредитныеПродукты, ЭтоЭлектроннаяПодписьВМоделиСервиса)
	...
	Для каждого КредитныйПродукт Из КредитныеПродукты Цикл
		...
		УсловияКредитованияXDTO = ЗаявкиНаКредит.УсловияКредитованияXDTOдляБанка(
			Банк, Перечисления.ХарактеристикиСервисаЗаявкиНаКредит.УсловияКредитовЗаймов);

		КредитныйПродуктXDTO = ЗаявкиНаКредит.НайтиКредитныйПродукт(
			УсловияКредитованияXDTO, КредитныйПродукт.ИдентификаторКредитногоПродукта, СведенияОЗаявке.ДанныеЗаявки.Дата);

		Если СведенияОБанке = Неопределено ИЛИ КредитныйПродуктXDTO = Неопределено Тогда
			Продолжить;
		КонецЕсли;

		// 1. xml-файл самой заявки.
		ИмяФайлаЗаявки = ЗаявкиНаКредит.ИмяФайлаДляКонтейнера(
		...
		Вложение.ИсходноеИмяФайла = ИмяФайлаЗаявки;
		Вложение.ИсходныеДанные   = ВыгрузитьЗаявкуНаКредитXML(
			СведенияОЗаявке, КредитныйПродуктXDTO, СведенияОБанке, ДополнительныеПоказателиБанка);
		...
		Вложение.ТипДокумента     = Перечисления.ТипыДокументовОбменаСБанкамиЗаявкиНаКредит.АнкетаЗаемщика;
		Вложение.ИсходноеИмяФайла = СтрЗаменить(ИмяФайлаЗаявки, ".xml", ".xlsx");
	...
	ПараметрыПомещения.ЭтоЭлектроннаяПодписьВМоделиСервиса = ЭтоЭлектроннаяПодписьВМоделиСервиса;
	...
	Для Каждого ТребуемыйОтчетЗаемщика Из ТребуемаяОтчетностьЗаемщика Цикл
		...
		ПараметрыПомещения.Банки = ТребуемыйОтчетЗаемщика.Банки;
		...
		Для Каждого НайденныйОтчет Из НайденныеОтчеты Цикл
			...
			Если Не ПустаяСтрока(РезультатВыгрузкиОтчета.ОписаниеОшибки) Тогда
				...
				Если ЭтоРасшифровкиБухОтчетности Тогда
					...
					РезультатВыгрузкиОтчета.ОписаниеОшибки = СтрШаблон(
						НСтр("ru = 'Не заполнен отчет ""%1"".
									|%2'"),
						ПредставлениеОтчета, РезультатВыгрузкиОтчета.ОписаниеОшибки);
				КонецЕсли;
				ДобавитьОписаниеОшибки(Результат.ОписаниеОшибки, РезультатВыгрузкиОтчета.ОписаниеОшибки);
				...
				Если ЗначениеЗаполнено(РезультатВыгрузкиОтчета.ТаблицаСообщений) Тогда
					...
					Для Каждого Ошибка Из РезультатВыгрузкиОтчета.ТаблицаСообщений Цикл
						Если Не ЗначениеЗаполнено(Ошибка.ОтчетДок) Тогда
							Ошибка.ОтчетДок = НайденныйОтчет.РегламентированныйОтчет;
					...
					ОбщегоНазначенияКлиентСервер.ДополнитьМассив(
						...
						ОбщегоНазначения.ТаблицаЗначенийВМассив(РезультатВыгрузкиОтчета.ТаблицаСообщений));
			...
			Если ЭтоРасшифровкиБухОтчетности Тогда
				...
				РезультатПодготовки = ПодготовитьРасшифровкуБухгалтерскойОтчетности(
					...
					КешРасшифровокВПользовательскомФормате, ТребуемыйОтчетЗаемщика, СведенияОЗаявке);
				
			Иначе

				ПараметрыПомещения.ИдентификаторАрхива = Строка(НайденныйОтчет.РегламентированныйОтчет.УникальныйИдентификатор());
			...
			Если Не РезультатПодготовки.Подготовлен Тогда
				...
				ТекстСообщения = СтрСоединить(РезультатПодготовки.Ошибки, Символы.ПС);
				Если Не ЗначениеЗаполнено(ТекстСообщения) Тогда
					ПредставлениеОтчета = ПредставлениеОтчета(
						...
						ТребуемыйОтчетЗаемщика.ДатаНачала,
						ТребуемыйОтчетЗаемщика.ДатаОкончания);
					ТекстСообщения = СтрШаблон(НСтр("ru = 'Не заполнен отчет ""%1""'"), ПредставлениеОтчета);
				КонецЕсли;
				ДобавитьОписаниеОшибки(Результат.ОписаниеОшибки, ТекстСообщения);
				
				Если ЗначениеЗаполнено(РезультатПодготовки.ТаблицаСообщений) Тогда

					// Дополняем описание ранее найденных ошибок в других отчетах.
					ОбщегоНазначенияКлиентСервер.ДополнитьМассив(
						Результат.ТаблицаСообщений,
						ОбщегоНазначения.ТаблицаЗначенийВМассив(РезультатПодготовки.ТаблицаСообщений));
			...
			Если Не ЭтоРасшифровкиБухОтчетности
			   И Не ЗаполнениеФинОтчетностиВБанки.ЭтоИмяФайлаАрхива(РезультатВыгрузкиОтчета.ИмяФайлаВыгрузки) Тогда
		...
		Если Не ОтчетНайден Тогда
			...
			ТекстСообщения = "";
			Если ОтборПоИсточникуОтчета.ИсточникОтчета = "БухгалтерскаяОтчетность" Тогда
	...
	ОценкаПроизводительности.ЗакончитьЗамерВремени("ПодготовкаФайловДляОтправкиЗаявкиНаКредит", ВремяНачала, Банки.Количество());
...
Функция ПодготовитьРасшифровкуБухгалтерскойОтчетности(РезультатВыгрузкиОтчета, ПараметрыПомещения,
	...
	Вложение.ТипСодержимого   = УниверсальныйОбменСБанками.ТипСодержимогоПоИмениФайла(РезультатВыгрузкиОтчета.ИмяФайлаВыгрузки);
	Вложение.ИсходноеИмяФайла = ЗаявкиНаКредит.ИмяФайлаДляКонтейнера(
...
Функция ЗапомнитьОтчетыВПользовательскомФормате(РезультатВыгрузкиОтчета)
	...
	КешРасшифровокВПользовательскомФормате.Колонки.Добавить("ИмяФайла",        Новый ОписаниеТипов("Строка"));
...
//	ЗаявкаНаКредит - ДокументСсылка.ЗаявкаНаКредит - Заполненная заявка.
//
Функция ВыгрузитьЗаявкуНаКредитXML(СведенияОЗаявке, КредитныйПродуктXDTO, СведенияОБанке, ДополнительныеПоказатели)
	...
	ПараметрыВыгрузки.ИдентификаторКредитногоПродукта = КредитныйПродуктXDTO.Идентификатор;
	...
	Иначе
		...
		Если КредитныйПродуктXDTO.ТребуютсяДанныеУчредителей = Истина Тогда // В объекте XDTO может быть Неопределено, поэтому явно сравниваем с Истина.
		...
		Иначе
			...
			ОтправительЮЛ.Учредители    = СведенияОЗаявке.Учредители.СкопироватьКолонки();
		КонецЕсли;
...
//
Функция ВыгрузитьЗаявкуНаКредитExcel(СведенияОЗаявке, КредитныйПродуктXDTO, СведенияОБанке, ДополнительныеПоказатели)
	...
	Если ЗначениеЗаполнено(КредитныйПродуктXDTO.Наименование) Тогда
		Область = Макет.ПолучитьОбласть("ПродуктНаименование");
		Область.Параметры.КредитныйПродукт = КредитныйПродуктXDTO.Наименование;
		ТабличныйДокумент.Вывести(Область);
	КонецЕсли;
	
	// Цель, сумма и срок
	Область = Макет.ПолучитьОбласть("ПродуктПодвал");
	...
	Если ДанныеЗаемщика.ЮридическоеФизическоеЛицо = Перечисления.ЮридическоеФизическоеЛицо.ФизическоеЛицо Тогда
		...
		Область.Параметры.Телефон               = СоединитьСтроки(ДанныеЗаемщика.Телефоны, ДанныеЗаемщика.ТелефонЮридическогоАдреса);
	...
	Иначе
		...
		Область.Параметры.Телефон               = СоединитьСтроки(
			ДанныеРуководителя.ТелефонРабочийФизическиеЛица, ДанныеРуководителя.ТелефонДомашнийФизическиеЛица);
	    ...
	    Если КредитныйПродуктXDTO.ТребуютсяДанныеУчредителей = Истина Тогда // В объекте XDTO может быть Неопределено, поэтому явно сравниваем с Истина.
		    ТребуютсяДанныеУчредителей = СведенияОЗаявке.Учредители.Количество() > 0 
		    		ИЛИ ДанныеЗаявки.СоставУчредителей = Перечисления.СоставУчредителейЮридическогоЛица.РуководительЕдинственныйУчредитель;
		Иначе
			ТребуютсяДанныеУчредителей = Ложь;
		КонецЕсли;
	    
	    Если ТребуютсяДанныеУчредителей Тогда
	    	...
	    	Иначе
	    		...
	    		Для каждого СтрокаУчредителя Из СведенияОЗаявке.Учредители Цикл
	    			...
	    			Область.Параметры.Телефон = СоединитьСтроки(СтрокаУчредителя.ТелефонРабочий, СтрокаУчредителя.ТелефонДомашний);
	...
	Если ДополнительныеПоказатели.Количество() > 0 Тогда
		...
		ДополнительныеПоказателиXDTO = НайтиДополнительныеПоказателиXDTOКредитногоПродукта(
			СведенияОБанке.Банк,
			КредитныйПродуктXDTO.Идентификатор,
			ДанныеЗаявки.Дата);
...
Функция НайтиДополнительныеПоказателиXDTOКредитногоПродукта(Банк, Знач ИдентификаторКредитногоПродукта, Знач ДатаАктуальности)
	...
	Для каждого ДополнительныеПоказателиXDTO Из УсловияКредитованияXDTO.ДополнительныеПоказатели Цикл
		...
		Если ДополнительныеПоказателиXDTO.ДатаНачала > ДатаАктуальности
			ИЛИ ДополнительныеПоказателиXDTO.ДатаОкончания < ДатаАктуальности Тогда
			Продолжить;
		КонецЕсли;
